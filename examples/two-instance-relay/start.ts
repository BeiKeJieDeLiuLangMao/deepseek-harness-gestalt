...
export async function apply(_ctx: Context, config: Config): Promise<void> {
  ...
  await withResources(async (resources) => {
    ...
    const pending = await pairingA.submitEndpointMessage1({
      ...
      completionId,
      device: { name: 'Snow phone', platform: 'ios' },
      message1,
    })
    ...
    let desktopAttachmentId = parseRelayAttachmentId('desktop-snow-1')
    ...
    const desktopLifecycle = new DesktopRelayEndpointLifecycle({
      attachmentId: () => {
        desktopGeneration += 1
        desktopAttachmentId = parseRelayAttachmentId(`desktop-snow-${String(desktopGeneration)}`)
