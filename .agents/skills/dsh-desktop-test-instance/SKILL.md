...
8. **Report evidence for product acceptance.** After the complete background experience route passes, freeze the build identity and prepare the first user-facing handoff with the existing screenshots, route, exact version, application path, and starting state. For a pull request, record and attach the required GIF only after the user accepts this version and semantic Standards and Spec review plus affected regressions pass:
   ...
   - Link the real product-path E2E screenshots in the initial handoff. Capture the actual application window through computer use and list those screenshot paths. After user acceptance and review/regression completion, follow [record-browser-gif](../record-browser-gif/SKILL.md) from the exact reviewed commit, append the verified GIF to the pull request, and retain screenshots, GIF, and command logs in a gitignored artifact directory outside the scratch root before cleanup. Every product-user-visible GUI pull request still requires the final GIF.
   ...
   The initial handoff is complete when its screenshots and instructions are readable, carry truthful provenance, and match the frozen build. Give the user that report, route, exact application path, and starting state without waiting for the later code review or GIF. Pull-request evidence completes only after the accepted, reviewed, regression-passing commit has its verified GIF attached. Record the observed UI-route result separately from teardown results so a passing interaction cannot hide a failed shutdown.
