...
- Read [docs/AGENTS.md](../../../../docs/AGENTS.md) and the parent [documentation skill](../SKILL.md) when deciding where content belongs or changing product documentation prose.
...
- Read the current `DocsPage` type, entries, `localeCollections`, and `sections` in [website/docs.ts](../../../../website/docs.ts) before changing the manifest; do not rely on a remembered field set.
- Read [website/.vitepress/config.ts](../../../../website/.vitepress/config.ts) before changing VitePress layout behavior or top-level navigation.
...
- `sidebar`: use the matching locale collection: `zh-guide`, `zh-develop`, `zh-reference`, `en-guide`, `en-develop`, or `en-reference`.
- `section`: reuse a label declared in the matching locale's `sections` list in `website/docs.ts`. Add a new entry there when the information architecture requires a new group.
