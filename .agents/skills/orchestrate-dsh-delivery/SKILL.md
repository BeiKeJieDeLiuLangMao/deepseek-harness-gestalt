...
3. Group related tickets on their stable module owner's branch; use separate branches only for independent module boundaries. Stack pull requests only when a leftover dependency still needs GitHub's official stack after this topology cannot express it.
...
## Integrate through the Delivery owner

1. Reuse the Delivery owner to integrate each completed module branch into the specification branch; do not create an extra merger role. Fast-forward when histories allow it; otherwise create a merge commit. Push the specification branch and report the new head. Give Delivery `/opt/homebrew/bin` on PATH. `pnpm: command not found` is a PATH miss, not a sandbox denial. When the module SHA is already verified and the specification update is a fast-forward of only that SHA, Delivery may push with `LEFTHOOK=0`; GitHub CI on that SHA owns typecheck. Do not rewrite lefthook in the same product PR.
2. After successful integration, recompute the ready frontier and dispatch newly unblocked owners.
3. Before each batch, Delivery merges current `origin/master` forward into the specification branch once and pushes it. Affected in-flight owners then merge that updated remote head forward into their module branches, audit semantic conflicts, and republish the exact head. Sibling owners do not merge master independently.
