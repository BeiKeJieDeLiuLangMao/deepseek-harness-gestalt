...
6. Have the acceptance-environment session prepare the evidence report and headed product handoff through [dsh-desktop-test-instance](../dsh-desktop-test-instance/SKILL.md). Start the headed instance only after that report is complete and tied to the frozen head.
7. Ask each writer session to run [`retro`](../retro/SKILL.md) on its own session. Collect the candidates in the root task, present a synthesized list to the user, and wait for an explicit keep-or-drop decision per item. Dispatch a writer to land only accepted environment changes on the specification branch, then accept the writer's re-run of the affected checks. Do not merge to `master` before that decision.
...
9. Mark the specification pull request ready. Its body carries `Closes` for the specification and every delivered ticket. Merge it only after local evidence, CI, review, fidelity, the acceptance walk, the retro decision, and merge requirements pass.
10. Confirm GitHub reports that pull request as merged and its tickets closed. Resume a failed or interrupted worker from GitHub state. Ask the user only for missing credentials, permissions, a material product decision, a retro keep-or-drop, a blocked experience-route step, conflicting official stack metadata, or release authorization.
