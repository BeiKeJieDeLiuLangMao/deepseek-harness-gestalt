...
4. Freeze the exact candidate after E2E, fidelity, and the agent acceptance walk pass. For a product-user-visible GUI change, record and publish the required GIF from that head with `record-browser-gif`. The evidence report ties the exact head to source identity, launch results, screenshots and GIF, TDD evidence or an explicit missing-RED statement, a readable `show-me` diff, and reproducible steps. Only then give the user the application path, starting state, and acceptance route.
