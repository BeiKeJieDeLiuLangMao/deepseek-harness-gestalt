...
Use this mapping only when the user permits `cliproxyapi` and the live catalog confirms the exact id and required capabilities. The root has no fallback: the user chooses its model, and the root never routes or changes itself.
