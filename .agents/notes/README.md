...
Each Agent Note belongs to one path-encoded class from the closed set in `scripts/agent-note-tree.ts`; the classification gate rejects other folders. Class is the nested folder from the closed six; GitHub `area/*` / "platform/infra" is not a class. Adding a class requires updating the canonical set and this section. See the [classification Agent Note](implemented/process/2026-06-20-agent-note-classification.md).
...
The archive is path-encoded as `archived/{class}/yyyy-mm-dd-topic-title.md`; `implemented` is deliberately absent because only implemented notes can enter it. An archival change moves the complete English/Chinese/sidecar triplet, retains `Status: implemented`, inserts the same `Archived: YYYY-MM-DD` line immediately below that status in both language files with no extra blank line, re-records the sidecar with `git hash-object`, and repairs or deletes inbound links. These are the only permitted content changes during archival. The header shape is in [`dsh-archive-agent-notes`](../skills/dsh-archive-agent-notes/SKILL.md).
