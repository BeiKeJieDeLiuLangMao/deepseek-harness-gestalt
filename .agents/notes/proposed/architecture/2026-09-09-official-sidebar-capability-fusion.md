...
The Web composition mounts two right-side workbenches. `ui-sidebar-right` owns the frame's `rightbar` seat, a DockKit surface, `ctx.sidebarRight`, `ctx.sidebarRightTabs`, and the guide, Files, and text-preview types. `ui-better-sidebar` separately appends a React root to `document.body`, owns another right panel, bottom panel, free windows, registry, layout store, and `dsh-sidebar:v1:*` state. The duplicate panels expose different capabilities and route file, Browser, Phone, Side Chat, Terminal, Changes, Tasks, Member Question, and model-initiated opens through different state owners.
...
| Bottom workbench | Independent tabs and split tree, resizable height, per-Session open state, first-open Terminal option, and type-only opens in the last touched bottom pane; content opens land in the visible right workbench. | The retained Better tests cover focused landing and layout calculations, not a complete assembled path. Implement the bottom seat before removing the Better root. |
