...
The adapter will derive its model catalog from the local core's `/v1/models` response and will register, replace, or withdraw its single route atomically through `ctx.llm`. Provider-topology notifications will cause Models and Composer consumers to re-read the existing provider and model directories. An empty account pool, an unavailable core, or a catalog that cannot substantiate a usable model will not publish a falsely usable route. The product-owned route id is `gestalt-cliproxyapi`; it is distinct from the conventional user-configured `cliproxyapi` id and will fail loud rather than replace any existing route with the same product id.
...
- The LLM integration publishes only `gestalt-cliproxyapi` from the live local model catalog, withdraws or marks it unusable when the core cannot serve models, leaves a user's `cliproxyapi` route untouched, and can complete a separately authorized real model request.
