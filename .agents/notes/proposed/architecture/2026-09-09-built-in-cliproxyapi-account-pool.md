...
The manager implementations establish the provider matrix and parsing basis but have not been validated against live services in this delivery. Initial product behavior will therefore label unverified, failed, or expired observations as unknown or stale, preserve the last valid snapshot on authentication or transport failure, and avoid claiming that every account currently returns every documented field. Refresh cadence and cache lifetime will be bounded Host policy chosen to avoid repeated upstream calls; implementation tickets will fix their values with provider-focused tests rather than exposing a renderer polling interval. Quota observations are display and diagnostic facts only: Gestalt will not invent a remaining-percentage threshold that disables an account or changes routing. CLIProxyAPI's standard eligibility, cooldown, and scheduler behavior remains authoritative. The UI prototype may use labeled fixtures for every state but may not imply that a fixture field has passed live-service validation.
