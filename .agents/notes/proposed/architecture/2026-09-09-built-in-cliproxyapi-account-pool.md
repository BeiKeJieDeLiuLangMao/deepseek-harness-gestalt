...
The remaining provider probe matrix, refresh cadence, cache lifetime, rate limits, and side effects must be recorded before the proposal freezes. The UI prototype may use labeled fixtures for every state but may not imply that a fixture field exists upstream.
