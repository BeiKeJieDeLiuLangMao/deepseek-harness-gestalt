...
The replacement must make the account pool part of the Desktop Bundle. It must supervise one local CLIProxyAPI core, present a Gestalt-owned account and quota experience, and publish one usable model provider without exposing management credentials to the renderer. The product also needs a durable source relationship with the upstream core while carrying the required GLM Coding Plan capability through a separately reviewed clean-room implementation in the Gestalt fork.

## Proposal

The remote coordination record is [gestaltrun/deepseek-harness-gestalt#649](https://github.com/gestaltrun/deepseek-harness-gestalt/issues/649). It tracks this proposal and its delivery graph but remains an `enhancement` that is neither frozen nor `ready-for-agent`; linking it from a planning pull request does not claim that the GUI draft or experience route is frozen.

DeepSeek Gestalt will ship CLIProxyAPI as a built-in Desktop component. The initial upstream baseline is lightweight tag `v7.2.155`, which resolves to commit `7fac6b15bcfe5ea55c18c9eaec8e5b7e6457d974`. The Desktop Bundle will contain a platform binary built from an exact commit of [`gestaltrun/CLIProxyAPI`](https://github.com/gestaltrun/CLIProxyAPI), a GitHub fork whose parent and source remain [`router-for-me/CLIProxyAPI`](https://github.com/router-for-me/CLIProxyAPI). The harness repository will record that core as a Git submodule pin. Application startup will not download, install, or enable the core, and Settings will not retain an Offer card.

The manager UI source will be imported from `router-for-me/Cli-Proxy-API-Management-Center` commit `ed5f1c48e11ba7335f1e8f676f228c280196af85`, licensed under MIT with the verified `Copyright (c) 2026 Router-For.ME` notice, into a first-party client package in this repository and then evolve as Gestalt source. The import will preserve the applicable copyright and permission notice. It will use the existing Desktop Settings shell, slots, components, locale, and theme. It will not be a UI submodule, a runtime download, an iframe, or a remote management page. Prototype conclusions and the experience route will specify its final presentation separately; this proposal fixes ownership and trust boundaries rather than visual layout.
...
The process will bind only to loopback on a per-instance dynamic port. The inspected upstream configuration defaults to port `8317`, and port-zero behavior is not established because the core does not report an assigned port back through its config. Releasing a Host-reserved port before spawn does not prove exclusive ownership and is not accepted as a race-free design.
...
The management gateway will expose only product operations: read the redacted account roster, begin a supported login, observe its status, cancel its OAuth session, apply supported auth-file mutations, and request quota refresh where a verified provider probe exists. It will not expose CLIProxyAPI's generic management request facility to the renderer. Active quota probes will use Host-owned, fixed provider endpoint policies adapted from the imported manager logic; the renderer will choose an account and request refresh, not supply an arbitrary URL, method, headers, or management payload.

Anthropic, Codex, and Antigravity start PKCE redirect flows by default. Kimi and xAI use device authorization: their auth start returns `flow=device`, `user_code`, `expires_in`, `url`, and `state`. Login URLs, user codes, expiry, and state will be returned only in the minimum form required for the Desktop Host and UI to complete the verified flow. The UI will show waiting, success, failure, expiry, cancellation, and retry without treating a device flow as an embedded redirect.
...
The adapter will derive its model catalog from the local core's `/v1/models` response and will register, replace, or withdraw its single route atomically through `ctx.llm`. Provider-topology notifications will cause Models and Composer consumers to re-read the existing provider and model directories. An empty account pool, an unavailable core, or a catalog that cannot substantiate a usable model will not publish a falsely usable route. The product-owned route id is `gestalt-account-pool`; it is distinct from the conventional user-configured `cliproxyapi` id and will fail loud rather than replace any existing route with the same product id.
...
CLIProxyAPI does not provide one uniform active quota endpoint. The imported manager source contains provider-specific active quota implementations whose React-free data modules and shared quota utilities will be adapted into Host-owned policies: `src/features/quota/providers/*/data.ts`, `src/utils/quota/{constants,parsers,builders,resetInstants}`, and `src/types/quota.ts`. The minimum portable closure also includes the quota-facing `AuthFileItem` type, `normalizeAuthIndex`, and the `DAY_MS`, `HOUR_MS`, and `MINUTE_MS` constants; its i18next dependency is type-only and will become a local translation-function signature rather than importing that UI framework. `QuotaTimeline`, `quotaTimelineModel`, `QuotaMeter`, and `AuthFileQuotaSection` are direct UI foundations to adapt into the first-party account cards, not proof that the original manager's browser-direct management architecture should remain.

The initial active matrix covers all five login providers. Anthropic reads OAuth usage windows including `five_hour` and `seven_day` utilization and `resets_at`. Codex reads wham usage with `used_percent`, `limit_window_seconds`, and reset values. Antigravity reads `retrieveUserQuotaSummary` buckets with `remainingFraction`, window, and `resetTime`; `5h`, `five-hour`, and `five_hour` normalize to five hours, while `weekly` and `week` normalize to 168 hours. Every other window keeps `periodHours=null`, preserving balance and reset facts without drawing a time tick. Kimi reads `/coding/v1/usages` usage and `limits.window` duration/time unit. xAI reads period usage from `cli-chat-proxy.grok.com/v1/billing`; the manager's paid `POST` chat ping with `max_tokens: 1` will not be imported. When billing or `/me` cannot establish quota, the result remains unknown rather than sending an inference request. The Host will normalize only verified response fields and retain their provider, provenance, collection time, and last successful observation.
...
GLM quota prefers `TOKENS_LIMIT` entries and uses `CREDIT_LIMIT` only when no token limit exists. Unit `3` identifies the five-hour window and unit `6` the weekly window. The projection retains `used_percent`, `reset_at`, and `updated_at`; it derives remaining capacity from verified used percentage and draws the time comparison only when the window duration and reset position are established. The subscription base's `/models` response is authoritative for GLM model availability, so the integration will not invent a static GLM catalog.

The manager implementations establish the complete provider matrix and parsing basis, while live-service validation remains acceptance evidence rather than a missing design input. Initial product behavior will therefore label unverified, failed, or expired observations as unknown or stale, preserve the last valid snapshot on authentication or transport failure, and avoid claiming that every account currently returns every documented field. Refresh cadence and cache lifetime will be bounded Host policy chosen to avoid repeated upstream calls; implementation tickets will fix their values with provider-focused tests rather than exposing a renderer polling interval. Quota observations are display and diagnostic facts only. Codex reset-credit count may be read and displayed, but reset-credit consumption and reset operations are outside this delivery, and Gestalt will not invent a remaining-percentage threshold that disables an account or changes routing. CLIProxyAPI's standard eligibility, cooldown, and scheduler behavior remains authoritative. The UI prototype may use labeled fixtures for every state but may not imply that a fixture field has passed live-service validation.
...
The Gestalt UI will therefore offer a dedicated GLM Coding Plan key entry rather than adding GLM to the five OAuth login actions. That entry will explicitly select region (China or international) and account scope (personal or team); it will not infer either from the key or silently fall back to pay-as-you-go. Team accounts additionally require an organization id and may name a project id. The Host will store these values through the credential-owning path and will pass only the resulting authority to the core. GLM will remain an account source behind the single CLIProxyAPI provider unless verified protocol constraints require a separate route. Its OpenAI-compatible effort normalization maps ordinary `low`, `medium`, and `high` requests to GLM `high`, and `xhigh` or `max` to GLM `max`; the exact `glm-5.3` model preserves explicit `low`. Anthropic-compatible GLM 5.3 requests preserve the corresponding `low`, `high`, and `max` levels rather than passing unsupported spellings through.

The initial product path prioritizes China personal and international personal Coding Plan keys. The same credential model will support China team and international team when the user supplies the existing organization id and optional project id fields; the product will not add subscription purchase, organization discovery, project discovery, invitation, or billing administration. The verified China Chat Completions base is `https://open.bigmodel.cn/api/coding/paas/v4`; the ordinary `https://open.bigmodel.cn/api/paas/v4` pay-as-you-go base is a different product and must not be substituted. The verified China Anthropic-compatible base is `https://open.bigmodel.cn/api/anthropic`. International quota uses the `api.z.ai` origin. International inference and Anthropic-compatible bases are explicit verified core settings rather than guessed Gestalt defaults; an absent setting makes that account unavailable instead of redirecting it to China or pay-as-you-go.
...
- The LLM integration publishes only `gestalt-account-pool` from the live local model catalog, withdraws or marks it unusable when the core cannot serve models, leaves a user's `cliproxyapi` route untouched, and can complete a separately authorized real model request.
