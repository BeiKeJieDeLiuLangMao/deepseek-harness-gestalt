...
CLIProxyAPI does not provide one uniform active quota endpoint. The imported manager source contains provider-specific active quota implementations whose React-free data modules and shared quota utilities will be adapted into Host-owned policies: `src/features/quota/providers/*/data.ts`, `src/utils/quota/{constants,parsers,builders,resetInstants}`, and `src/types/quota.ts`. `QuotaTimeline`, `quotaTimelineModel`, `QuotaMeter`, and `AuthFileQuotaSection` are direct UI foundations to adapt into the first-party account cards, not proof that the original manager's browser-direct management architecture should remain.

The initial active matrix covers all five login providers. Anthropic reads OAuth usage windows including `five_hour` and `seven_day` utilization and `resets_at`. Codex reads wham usage with `used_percent`, `limit_window_seconds`, and reset values. Antigravity reads `retrieveUserQuotaSummary` buckets with `remainingFraction`, window, and `resetTime`; its parser must preserve an unrecognized window enum as unknown and must not assign a guessed duration. Kimi reads `/coding/v1/usages` usage and `limits.window` duration/time unit. xAI reads period usage from `cli-chat-proxy.grok.com/v1/billing`; any xAI liveness path that creates an inference request is excluded from default quota refresh and may run only as an explicitly authorized diagnostic. The Host will normalize only verified response fields and retain their provider, provenance, collection time, and last successful observation.
...
The manager implementations establish the provider matrix and parsing basis but have not been validated against live services in this delivery. Initial product behavior will therefore label unverified, failed, or expired observations as unknown or stale, preserve the last valid snapshot on authentication or transport failure, and avoid claiming that every account currently returns every documented field. Refresh cadence and cache lifetime will be bounded Host policy chosen to avoid repeated upstream calls; implementation tickets will fix their values with provider-focused tests rather than exposing a renderer polling interval. Quota observations are display and diagnostic facts only. Codex reset-credit consumption and reset operations are outside this delivery, and Gestalt will not invent a remaining-percentage threshold that disables an account or changes routing. CLIProxyAPI's standard eligibility, cooldown, and scheduler behavior remains authoritative. The UI prototype may use labeled fixtures for every state but may not imply that a fixture field has passed live-service validation.
