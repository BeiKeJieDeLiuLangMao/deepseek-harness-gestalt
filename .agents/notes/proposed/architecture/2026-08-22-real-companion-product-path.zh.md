...
Mobile Companion 不提供推送能力。删除 APNs 与 FCM adapter、token、payload、持久化、配置、配额、指标、secret、原生依赖与验收要求。进入后台会暂停 Relay 连接；打开应用或回到前台时重新连接，并在启用 mutation 前完成 Desktop 权威同步。只有不携带过期交互权威且不依赖推送投递的 deep link 才可保留。
