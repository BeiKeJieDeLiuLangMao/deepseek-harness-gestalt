...
已附加摘要可以折叠实时事件日志并选择最新的真人 `user/message`，但冷路径有意不读取任何日志：冷摘要只来自 projection cache，因此冷近期度只能与 cache 一样新鲜。
...
交付的 JSONL 后端决定了具体的存储约束。它的 header 就是第 1 行，在物化时一次写就，此后这份日志永远以追加方式打开；`jsonl.spec.ts` 钉住了「已提交的字节绝不重写」。一个每次追加都要改的 header 字段，违反的是一条被断言的持久性不变式，而不只是让写入方变复杂。因此，要与「让 JSONL 保持近似」相比较的形态，是每会话一个伴随文件。树外后端只有在为自身表示定义了更新原子性、版本与恢复语义时，才可以把该值存入自己的索引；本提案不规定其他提供方的 schema。

实现之前必须回答三个问题，本文对它们都没有定论：

**共享谓词由谁拥有？** 已存储字段在写入时编码规则，写入方只看到一个批次，而已附加摘要折叠整份日志。两者必须使用同一个导出的事件谓词或 reducer，避免新的消息来源变体让已附加排序与冷排序发生分歧。

**该字段引入之前的日志表现如何？** 既有产物里没有这个值。回退到 mtime 能让它们保持既有的 mtime 近似准确度；回退到 `createdAt` 是诚实的，但会把选择器和会话树里每一个既有会话都重新排一次序。
...
- 选定的 JSONL 表示会保留已提交的日志字节，并与对应的追加操作原子地更新活动时间值，或定义一种保守且可观察的陈旧值失败模式。
...
- [有界冷空白验证](../../implemented/bug-fix/2026-08-13-bounded-cold-blank-verification.zh.md)——移除 mtime 排序，并定义本文将使其精确的过渡性 cache-only 冷摘要。
...
- [基于句柄的会话持久化](../../implemented/architecture/2026-08-27-handle-based-session-persistence.zh.md)——已存储字段将挂入的写入句柄追加路径。
