...
Mobile Companion has no push capability. APNs and FCM adapters, tokens, payloads, persistence, configuration, quotas, metrics, secrets, native dependencies, and acceptance requirements are deleted. Backgrounding pauses the Relay connection; opening or foregrounding the application reconnects and completes Desktop-authoritative synchronization before enabling a mutation. Deep links may remain only when they carry no stale interaction authority and do not depend on push delivery.
