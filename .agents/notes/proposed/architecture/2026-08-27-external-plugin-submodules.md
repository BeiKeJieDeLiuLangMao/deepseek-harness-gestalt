...
The implemented [built-in CLIProxyAPI account pool](../../implemented/architecture/2026-09-09-built-in-cliproxyapi-account-pool.md) supersedes the Sub2API-specific child and Offer-card plan above, but not this catalog pattern for independently released plugins. Its core fork remains foreign source pinned by submodule, while Gestalt-owned lifecycle and UI code live in the harness repository.
