...
已实现的[内置 CLIProxyAPI 账号池](../../implemented/architecture/2026-09-09-built-in-cliproxyapi-account-pool.zh.md)取代上文针对 Sub2API 子项与 Offer 卡的计划，但不会取代该目录对独立发布插件的通用模式。其核心 fork 仍是由 submodule 钉住的外部源码，而 Gestalt 自有生命周期与 UI 代码位于 Harness 仓库中。
