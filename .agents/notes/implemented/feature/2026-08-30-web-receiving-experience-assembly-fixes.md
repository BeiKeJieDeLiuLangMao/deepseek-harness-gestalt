...
`packages/api/session-controller/tests/receiving.client.spec.ts` drives `ReceivingQuestionBook` through the generated `memberQuestion` Remote, selects Host-id rows, and observes its projected `SessionFace`. It pins Host revision ordering, answer and decline RPCs, disconnect retention, terminal projection, two-Installation answered-elsewhere derivation, and disposal. `packages/client/ui-workspace/tests/workspace-browser.client.spec.tsx` pins arrival-edge expansion and post-arrival human collapse. `apps/web/tests/member-question-receiving.e2e.ts` drives authenticated ingress through the shipped Host receiver, Session Controller, generated Remote, WebSocket stream, dynamic module table, additive Decision Brief dock, product composer, shared question presentation, exceptional terminal bands, restart, and reload; it asserts identical recovered pending and terminal records, one Host Session in the bound Workspace, and no arrival model request. The [shipped Client assembly decision](../bug-fix/2026-09-08-member-question-client-assembly.md) owns the deployment-row guards.
