...
默认值是组合配置（`cordis.yml`）——由运维人员拥有、作用于整个进程。运行时切换是会话范围的覆盖，以一条仅日志事件记录在该会话的日志中；旋钮的 projection 单元折叠该事件，并且每个单元的状态都会形成检查点，因此重启后仍然有效和多会话隔离由持久 projection 缓存与日志回放共同保证，无需外部配置存储。进程内 subagent 驱动器在委派时对父级的显式覆盖项获取快照，并在子 agent 可选的 fork 前缀之后预置一条带来源标记的事件，因此委派无法回退到更宽的默认值（[决策](2026-07-25-subagent-policy-inheritance.zh.md)）。
...
每个拥有者贡献相同的组成部分：事件声明、在必需的 `ctx.sessionProjections` 注册表上注册的一个 projection 单元（`key` 为 `sandboxMode` / `approvalPolicy`、`stateVersion` 为 1、用于验证持久状态的 zod `stateSchema`、初始化为 `null` 的 `init`，以及作为 fold 的 `apply`——它是对旋钮事件的 find-last 形式转移，并以领域的封闭联合定型），以及唯一的写入路径（`setSandboxMode(session, mode)` / `setApprovalPolicy(session, policy)`——切换即其事件；没有任何东西在带外修改状态）。注册表会在已提交的会话事件上立即驱动每个单元，并统一为所有状态建立检查点——也包括仅供主机使用的单元，不设 `persist` opt-in——因此重启后仍然有效和多会话隔离依赖持久缓存与日志回放，而非各包自己的读取辅助函数。这是共享的框架基础设施，不是带归属服务的 facts map：每个领域保留自己的事件与单元；第三个旋钮在自己的包中写入自己的事件、单元与 setter，并注册该单元——驱动、检查点和读取接口都由注册表提供，无需复制 fold 导出。主机读取方通过 `stateOf(session, key)` 读取折叠后的覆盖值（`SandboxPolicyService.overrideOf` 和 `ApprovalService.overrideOf` 就是这些读取），贡献方与读取方都要求 `sessionProjections`（[必需的 projection seam](../architecture/2026-08-19-session-projection-mandatory-seam.zh.md)）。执行在两侧都遵循 projection 状态——bash 工具的按调用盖章将其作为 § 升级机制优先级链的中间层读取，approval seam 的 `'never'` 门控是[批准 Agent Note](2026-07-06-approval-seam.zh.md) 同一模式的另一侧。
...
- [会话 projection 是必需的 seam](../architecture/2026-08-19-session-projection-mandatory-seam.zh.md)——旋钮单元注册到必需的 `ctx.sessionProjections` 注册表；主机读取方通过 `stateOf(session, key)` 读取折叠后的覆盖值。
- [事件溯源会话](../architecture/2026-06-11-event-sourced-sessions.zh.md)与[独立纯日志事件](../simplification/2026-07-28-remove-synthetic-log-only-turns.zh.md)——projection 单元 fold 所依赖的日志即存储基础，以及锚定设计遵守的显式持久性边界。
