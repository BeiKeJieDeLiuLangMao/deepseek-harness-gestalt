...
`dsh-browser-runtime-deterministic` is the first Provider. It creates one temporary hierarchy with one tab, recognizes only configured pages, returns deterministic observations and PNG data, and publishes each committed state. Its package invariant checks the mutable publication stream for stable identities and exact revision succession. Disposal stops admission, drains the operation queue, and closes an open temporary Profile. Configuration owns identity prefixes, fixture pages, and screenshot data; invalid or ambiguous configuration fails at load.
...
The repository now has a complete Browser Runtime capability seam whose interface, deterministic Provider, and model Consumer can evolve independently. Session replay can reconstruct what the model learned, and concurrent mutations have an explicit conflict result. The deterministic Provider is a tracer rather than a production browser: persistent cookie/storage isolation, several Workspaces and tabs, native Electron control, Browser Dock UI, catalog policy, and migration/release work remain separate features.
