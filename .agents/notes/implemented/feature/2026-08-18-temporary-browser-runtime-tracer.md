...
`dsh-browser-runtime-deterministic` is the first Provider. One Provider instance admits one temporary hierarchy with one tab; close permanently exhausts that instance's capacity, so the stable identities and revision zero never identify a second lifecycle. It recognizes only configured pages, returns deterministic observations and PNG data, and publishes each committed state through contained post-commit fan-out. Its package invariant seeds from authoritative current state on initial load and hot reload, then checks stable identities, exact revision succession, and terminal closure. Disposal stops admission, drains the operation queue, and closes an open temporary Profile. Configuration owns identity prefixes, fixture pages, and screenshot data; screenshots must be non-empty canonical base64 with the PNG signature, and invalid or ambiguous configuration fails at load.
