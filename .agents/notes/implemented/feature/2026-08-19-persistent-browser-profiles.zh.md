...
`ctx.browserRuntime.create` 接收临时、命名持久或共享 Browser Profile。持久 Profile 复用稳定的 `persist:session-${idPrefix}-${name}` partition 与同一 `BrowserProfileId`。无密钥测试用带名称的存储 token 证明隔离。当本进程是 Electron 时，按 Electron 门控的 e2e 证明两个 partition 的 cookie 隔离。临时 Profile 获得唯一的 `tmp-N` session 名、空存储，且没有地址栏标签。
