...
Official page chrome lives in the snapshot sidebar tab type `browser`. Each official Workspace page is one sidebar tab. The sidebar tab bar is the page list. The address field is editable and navigates on Enter; the refresh control is `aria-busy` while observe or navigate is in flight. Desktop Host presents the same Runtime `webContents` as a `WebContentsView` on the Host `contentView` so the user can click, type, and scroll. Settings and the sidebar `+` menu are the same React components as `dsh web`; under Electron they mount in a second transparent `WebContentsView` stacked above official pages, and `dsh web` keeps the in-document panel and menu. The overlay document does not reconcile official pages or present/conceal live views. Its root frame mounts only the Settings seat and `shell.overlay`; it omits the Session, navigation, details, and Workbench trees, and its HTML, body, and root backgrounds stay transparent. The Desktop Workbench tab strip assigns `-webkit-app-region: drag` only to the unused tail after `+`, so tabs, close, `+`, and panel toggles remain pointer targets. An unpresented page remains attached to a transparent, off-screen, non-focusable `BaseWindow`, which keeps Chromium painting for screenshots; presentation reparents only the `WebContentsView` to the Host, and conceal returns it to that paint host. `loadURL` that rejects `ERR_ABORTED` after a redirect, or a Chromium net error after Chromium painted its error document, is a committed navigation, not a crash. A child `BrowserWindow` plus `setParentWindow` is not used because that path SIGSEGVs on macOS Electron 41. `dsh web` paints screenshot-plus-text; `about:blank` captures stay hidden so a new tab shows start copy. `+ → Browser` and `browser_create` each create one official page using the `ui-browser` default identity. Browser Workspace serializes creates and reuses an already-open Session instance only when its Profile matches that identity. An empty `+` tab calls `ensureOfficial`; `OfficialBrowserTab` has no direct-create fallback. The host patch also turns snapshot link takeover on (`browserInterceptLinks` and `browserInterceptHttps`): a GUI link click or a `sidebar_open` URL opens a sidebar `browser` tab whose recorded seed URL the bridge navigates to right after create. A rejected create is recorded on the tab meta, and the chrome shows a retry control instead of a perpetual creating placeholder; the next attempt clears the record. The Session projection stores each tab's last non-blank committed URL. When initial observe reports that the current Runtime does not own a projected target, the adapter unbinds that target, retains the tab's Profile identity, uses the existing Browser Workspace create path to replace the missing page in the same sidebar tab, and navigates the replacement to that URL before publishing it. Other observe failures remain empty and do not create a page. One page retains one Session-bound Remote verb set for its `ctx` and Session id, so sidebar rerenders do not restart observe and screenshot. Closing a sidebar tab closes that official page. A stale close revision is observed once and retried; a transient close failure retains the close intent so later reconciliation retries the Runtime close instead of reopening the sidebar tab. Disposing a live Session closes its process-local pages but retains the projection for a later restore. Archiving a Session emits an awaited cleanup event after the archive set is durable; Browser Workspace closes every live page owned by that Session and forgets its Runtime identities.
