...
The Provider admits one temporary Profile lifecycle: one Tandem session, one tab, DSH-owned opaque Profile/Workspace/browser identities from `idPrefix` around the Tandem tab id. Operations serialize through one queue; mutations check `expectedRevision` exactly. Disposal stops admission, drains the queue, destroys the session, and joins the process tree under `processGraceMs`.
