...
Schedule contributes an independent Session projection keyed `schedule` containing retained records in creation order plus the durable `paused` flag. Apply skips `schedule/change` events whose `seq` is below `Session.inheritedEventCount`; the Host definition has no `eventScope` field. Clients receive finished current values and never fold Schedule events or reconstruct state from tool calls or conversation output. The Client clock alone derives scheduled versus overdue presentation from `scheduledAt`.
