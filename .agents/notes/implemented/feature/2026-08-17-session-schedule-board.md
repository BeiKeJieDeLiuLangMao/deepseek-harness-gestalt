...
The `ctx.schedules` Service owns `schedules/pause`, `schedules/resume`, and `schedules/delete`. Their wire identity is a branded `SessionId`, so a human mutation does not invoke the generic Agent-resume lookup. One Session-scoped FIFO serializes human mutations with tool management and due delivery. An existing live root uses its ordinary preflight flush, append, post-append flush, and runtime recomputation. A cold Session is reserved through `sessionPersistence.prepare`, temporarily attached as the exact prepared Session for an append and flush, then detached without publishing an Agent or starting delivery. If Agent publication wins the preparation race, the mutation recomputes and uses that already-live root rather than waking another owner. The Session log remains the only durable authority, so pause and resume survive restart without another store.
...
Schedule domain and restart tests cover valid and invalid pause/resume transitions, paused deletion, listing paused records, runtime exclusion, Session FIFO ordering, persistence uncertainty, a preparation-versus-live-owner race, cold mutation without Agent publication, and a full JSONL Host remount before resumed delivery. Projection registry/cache tests cover owned-suffix behavior in live, cold, checkpoint, and restored paths; detached Host history proves `SessionHeader.seedLength` reaches cold restore; Schedule projection tests reject malformed changes and invalid transitions instead of publishing a partial value. Plugin lifecycle coverage proves the Schedule projection contribution leaves with its owning fiber. Typert and Client plugin tests cover Remote mounting, header ordering, active count, state rows, pause/resume, inline delete confirmation, failures, focus, and lifecycle cleanup.

A keyless assembled Desktop browser scenario boots the real Web bundle over HTTP from a durable-only Session fixture. It proves the board comes from the projection, pauses through the generated Remote, remains paused after reload, resumes, deletes through the inline second confirmation, and matches the committed accessibility snapshot. Browser screenshot evidence records the complete A-variant board without substituting transcript or mock projection state.
