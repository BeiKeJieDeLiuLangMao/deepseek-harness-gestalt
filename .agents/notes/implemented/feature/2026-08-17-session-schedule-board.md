...
Schedule contributes an independent `schedules` Session projection containing retained records in creation order plus the durable `paused` flag. The projection declares `eventScope: 'owned-suffix'`; the projection registry and cache start it at `Session.inheritedEventCount` in eager, lazy, restore, and replay paths. Clients receive finished current values and never fold Schedule events or reconstruct state from tool calls or conversation output. The Client clock alone derives scheduled versus overdue presentation from `scheduledAt`.
