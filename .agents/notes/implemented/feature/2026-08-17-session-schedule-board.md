...
The `ctx.schedules` Service owns `schedules/pause`, `schedules/resume`, and `schedules/delete`. The API resolver supplies the exact live root Agent from the wire Session id. Mutations validate that ownership and use the same Agent-scoped FIFO, preflight flush, append, post-append flush, and runtime recomputation as tool management and due delivery. The Session log remains the only durable authority, so pause and resume survive restart without another store.
...
Schedule domain and restart tests cover valid and invalid pause/resume transitions, paused deletion, listing paused records, runtime exclusion, FIFO ordering, persistence barriers, and restart survival. Projection registry/cache tests cover owned-suffix behavior in live, cold, checkpoint, and restored paths; Schedule projection tests cover create order, pause/resume/delete/dispatch, malformed changes, and fork prefixes. Typert and Client plugin tests cover Remote mounting, header ordering, active count, state rows, pause/resume, inline delete confirmation, failures, focus, and lifecycle cleanup.

A keyless assembled Desktop browser scenario boots the real Web bundle over HTTP from a durable-only Session fixture. It proves the board comes from the projection, pauses through the generated Remote, remains paused after reload, resumes, deletes through the inline second confirmation, and matches the committed accessibility snapshot. The recorded browser capture supplies visual evidence for the complete A-variant board.
