...
`dsh-client-ui-browser` presents the Session-owned Browser Workspace as official screenshot-plus-text chrome. Occupancy later moved into the workbench sidebar `browser` tab; see the [workbench official browser Agent Note](2026-08-21-workbench-official-browser.md). The collapsed preview still occupies Chat-declared `conversation.browser.preview`. Live facts arrive through `useProjection('browserWorkspace')`. Mutations go through the generated `remote.browserWorkspace` namespace, including `create`.

The chrome has no Profile switch or Agent-status row. The workbench sidebar tab bar is the page list; page chrome has no tab strip. Persistent Profile names appear next to the address field and as editable roster rows in settings section `id: 'browser'`. Renaming a roster entry replaces that partition key in `namedProfiles` and follows `defaultPersistentName` when it still named the old entry; Chromium partition data is not migrated. The active tab's label, address, and screenshot re-observe when that tab's listed revision advances, so a Binder-committed navigate replaces still-blank `about:blank` chrome. Refresh observes the Runtime's current URL, then navigates to it. The address field is editable: Enter navigates, and a host without a scheme receives `https://`. Back and forward walk a pane-local trail of committed URLs (address-bar navigations and, on Desktop, in-page clicks observed after a revision advance); a separate control opens the committed page in the default browser. A rejected page create records the error on the sidebar tab meta and the pane shows a retry control. The viewport shows the latest screenshot and page text and scrolls when the screenshot is larger than the pane; it does not embed a second process. Official chrome does not occupy `details`; the narrow overlay stays for other details occupants ([narrow overlay Agent Note](../bug-fix/2026-08-21-narrow-browser-dock-overlay.md)).
