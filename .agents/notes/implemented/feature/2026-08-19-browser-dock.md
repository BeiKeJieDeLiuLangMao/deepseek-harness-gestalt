...
The Host composition inserts `dsh-browser-runtime-deterministic`, `dsh-browser-workspace`, and `dsh-tool-browser` so the Dock has a Session-owned Runtime without a second Electron process. Web composition disables the host-plane `tool-browser` row so the standard, code, and cordis presets remount it, matching `tool-web`, and mounts the Dock plugin.
