...
`dsh-tool-subagent` exposes optional `provider`, `model`, and `reasoning_effort` fields only when its Agent-scoped instance records a non-empty Session policy and the bound subagent provider advertises `SubagentCapabilities.agentOptions`. A fresh top-level Session records the deterministic union of an enabled user authorization from [user-authorized subagent model routes](2026-08-24-user-authorized-subagent-model-routes.md) and the current snapshot from `agent.ctx.get('subagentRoutePreauthorization')?.snapshot() ?? []`; either source may be enabled independently. Sampling runs once at Agent or preset composition from that Agent's context, never through a reactive Provider inject. An absent Provider records durable `[]`, and a later Provider cannot authorize that Session. The deployment service and its static Provider are separate packages, the Settings owner has no deployment defaults or writes, and Provider contributions are effect-scoped. An empty recorded policy disables route fields and `list_subagent_models`. Disabled instances omit and reject model-facing selection. A route-selectable instance against a provider without the capability fails the plugin mount.
...
An enabled definition registers `list_subagent_models`. With no arguments the tool lists authorized registered providers; with `provider` it calls that adapter's advisory model catalog only after authorization; with `provider` and `model` it authorizes the exact route before resolving its reasoning efforts and default. Selectable tools on one Agent share one discovery registration and the same recorded route policy; inconsistent policies fail, and the last consumer removes discovery. Shipped product compositions put `modelSelectionSettings: true` on both Agent-scoped `subagent` and `subagent_fork` instances and register the default-off Host-owned `subagent-model-selection` setting with an explicit `enabled` switch and `allowedModels` list. The Plugins settings page stores both fields atomically. A deployment composition may independently set `deploymentRoutePreauthorization: true`; the Consumer samples the current registry snapshot only for a fresh top-level Session from `agent.ctx`. The resulting union, including an empty list, is recorded as `subagent/model-selection-policy` before any model request. A child Session copies the durable parent policy, and a resumed Session uses its recorded event instead of current Settings or Provider state. Therefore later Settings edits, Provider detach, and Provider replacement leave an existing authorized Agent stable and affect only subsequently composed top-level Sessions. The fixed discovery definition remains available without the optional LLM service, while discovery and selected-route calls fail until that service is present. Discovery lists the intersection of the live catalog and recorded policy, and the executor rejects explicit, configured, or provider-default routes outside it while allowing unchanged parent-route inheritance.

Shipped `subagent_fork` instances use the same recorded exact-route authorization as `subagent`. Omitting `provider` and `model` inherits the parent's route; an explicit pair selects only an authorized route. Every completed parent turn remains in the child seed regardless of route. Route changes can prevent provider-side KV Cache reuse and require prefilling that retained history. `reasoning_effort` follows the shared baseline and route-change rules; omitting it on a changed route selects the target model's default effort.
...
**Treat the live LLM registry as deployment authorization.** Rejected because provider registration and advisory catalogs do not grant a model permission to choose every route. Deployment authorization is an explicit exact-route contribution, separate from user preference and sampled only into new top-level Sessions.
...
**Export discovery as a separately loaded plugin entry.** Rejected because discovery belongs to the selectable delegation tools on one Agent. Sharing their registration keeps one name and removes it with the final consumer without a separate configuration entry.
...
**Keep fork routing fixed to the parent.** Rejected because an authorized child route can serve a different task while retaining completed context. The default still inherits, and an explicit change discloses the possible prefix cache cost.
...
- A Session can select only its recorded exact child LLM routes; disabled Sessions omit and reject model-facing route fields.
- User and deployment authorizations are independent inputs to a fresh top-level Session's sorted, deduplicated policy. Neither deployment startup nor route sampling writes Settings.
- Delegation-tool instances default selection off, expose a Plugins-page exact-route opt-in for new Sessions, and share `list_subagent_models` only in Sessions whose durable policy is non-empty; discovery and effective route selection are constrained to that policy.
- Shipped fork tools accept authorized route fields and preserve completed parent context. Omission inherits the parent route; an explicit route change may lose prefix cache reuse.
