...
`dsh-browser-runtime-deterministic` 是首个 Provider。它创建一个含单标签页的临时层级，只识别配置的页面，返回确定性观察与 PNG 数据，并发布每个已提交状态。其包级 invariant 检查可变发布流中的身份稳定性与精确修订顺序。释放阶段停止接收新操作、排空操作队列，并关闭仍打开的临时 Profile。配置负责身份前缀、fixture 页面与截图数据；无效或有歧义的配置会在加载时失败。
...
仓库现在具备完整 Browser Runtime 能力 seam，其接口、确定性 Provider 与模型 Consumer 可以独立演进。Session 重放能够重建模型获得的事实，并发写操作也有明确冲突结果。确定性 Provider 是 tracer 而非生产浏览器：持久 cookie/storage 隔离、多 Workspace 与标签页、原生 Electron 控制、Browser Dock UI、catalog policy 与迁移/发布工作仍属于独立功能。
