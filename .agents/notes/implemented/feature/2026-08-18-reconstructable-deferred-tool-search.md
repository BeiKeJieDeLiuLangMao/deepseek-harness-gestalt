...
The agent loop stores matched schemas on the durable `tool-result` block. Each prompt assembly treats those restored results as file input, validates every complete `ToolSchema`, reads them from `Session.deriveMessages()`, deduplicates by tool name, and retains a stored schema only while the current resolved view still contains an eligible deferred definition with that name. It then serializes the complete eligible set as one canonical reconstructed discovery block and applies the current byte budget before projection. Invalid or oversized restored schemas fail assembly rather than reaching the model; stale or ineligible names are filtered before budgeting. The next request therefore carries the exact schema returned by search, while removal or a narrower allow-only contribution prevents stale history from restoring or dispatching it. `request/header` continues to record the complete assembled request tools, so replay has one authoritative request snapshot.
