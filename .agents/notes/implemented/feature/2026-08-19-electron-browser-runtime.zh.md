...
Tandem 仍是 HTTP 与 MCP 操作词汇，不是 sidecar 二进制。`listenElectronBrowserHttp` 把 sessions、tabs、navigate、input、page-content、screenshot、focus 与 destroy 复制到 loopback origin。navigate、input 与 focus 会把客户端的 `expectedRevision` 与引擎修订号比较，并返回引擎已提交的修订号；不匹配时为 409 `BROWSER_REVISION_CONFLICT`。Desktop Host 启动该引擎，向 Node Web Host 导出 `DSH_ELECTRON_BROWSER_ORIGIN` 与 `DSH_ELECTRON_BROWSER_TOKEN_FILE`，Desktop 叠加层把 `dsh-browser-runtime-tandem` 挂载为 `sidecar: false` 的协议专用 HTTP 客户端。`command` 与 `cwd` 仍可选，供仓库内 HTTP fixture 使用，并在 `sidecar` 为 false 时于插件加载失败。生产环境从不启动 Tandem.app。客户端无论是否拥有子进程都会通过 HTTP 销毁剩余打开的 session。
