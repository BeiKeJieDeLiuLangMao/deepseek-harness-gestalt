...
已经存下的未知输入项在切换时予以保留。已存的 `reasoningEfforts: false` 在点选档位之前会保留，并显示单独提示，而不是看起来像未设置。DeepSeek 的模型能力仍由适配器持有；它的目录编辑器不提供 pi-ai 的输入或推理 profile 字段。
...
[输入测试](../../../../packages/client/ui-settings-models/tests/input-modality.client.spec.ts)和[档位测试](../../../../packages/client/ui-settings-models/tests/reasoning-effort.client.spec.ts)覆盖省略、顺序、未知项、线路拼写和无效声明。[提供方表单测试](../../../../packages/client/ui-settings-models/tests/provider-form.client.spec.tsx)经过创建和已有提供方两个入口，验证保存的路径操作、清空覆盖后恢复继承，以及无关编辑保留已存的不思考声明。[完整 Models 场景](../../../../apps/web/tests/models-settings.e2e.ts)通过真实 Host 保存声明，解析得到的模态和档位元数据，再重新打开编辑器核对无需密钥的 UI 快照。单独的标签组件测试通过，不能证明两个表单实际渲染了它。
