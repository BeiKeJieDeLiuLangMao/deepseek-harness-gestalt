...
**把每小时、附件和推送窗口留在提供方实例上。** 共享同一个 authority 存储的两个提供方会让每个窗口翻倍。共享事务映射与 50 个配对计数一致。
...
开放注册可以保持开放且没有允许名单，同时单个账号或 IP 不能无界保留安装、配对、附件或推送提示。运营仍需手工扩展已购买的两台实例；CloudMonitor 仪表盘、生产共享容量水位、可信代理客户端 IP、跨实例连接计数器，以及真实附件和推送产品路径，仍属部署或后续工作。尚未见过 `pollLogin` 的冷账号实例在 `getSession` 绑定会话后仍执行 20 条连接上限，并拒绝未知 id。每安装的在线、待确认和保留配对上限与账号范围配额同时生效；清理失败墓碑填满十六条记录上限时，安装可能先碰到 `PAIRING_RESOURCE_LIMIT` 再碰到 `QUOTA`。

## Testing

Account 单元测试钉住第 10、11 个 Desktop 与 Mobile 安装、同一安装替换、在 9 个在线槽位上并发两次新登录、第 20、21 次 `trackConnection`（含通过后端解析的未绑定会话）、第二个 GitHub 身份，以及登录卸载。Loader 加真实 TCP 的 Account HTTP 场景重复这些边界。Remote Access 单元测试钉住每小时账号与 IP 挑战、必需的客户端 IP、在重放保留和小时窗口推进下的 50 个配对、5 个并发附件、100 MiB 附件上限、精确 1 GiB 声明日字节、500 条推送提示、超大与并发已满附件的 60 秒硬上限 `retryAfter`、日字节上限的剩余窗口 `retryAfter`，以及共享同一个 authority 存储的第二个提供方拒绝第 11 次挑战、第 6 个附件和第 501 条推送。容量卸载后仍能列出已建立配对。真实 Personal Pairing HTTP 重复小时、配对、附件、推送和容量信封，证明伪造的转发头不会隔离第二个 IP 桶，并以 400 拒绝负的 `admit-blob` bytes。Relay 测试持有两个水位槽，接入 desktop 与 mobile，以 gate 重试延迟拒绝第三次 attach，投递一帧 desktop→mobile 密文，并在关闭或鉴权失败时释放。客户端保留 `QUOTA` / `PLATFORM_CAPACITY` 与整数 `retryAfter`。
