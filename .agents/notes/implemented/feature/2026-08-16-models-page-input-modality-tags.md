...
Unknown input entries already stored survive a toggle. A stored `reasoningEfforts: false` keeps that value until a tag is pressed and shows its own hint rather than looking unset. DeepSeek's model capabilities remain adapter-owned; its catalog editor does not expose pi-ai's input or reasoning profile fields.
...
The [input tests](../../../../packages/client/ui-settings-models/tests/input-modality.client.spec.ts) and [effort tests](../../../../packages/client/ui-settings-models/tests/reasoning-effort.client.spec.ts) cover omission, ordering, unknown entries, wire spellings, and invalid declarations. The [provider-form tests](../../../../packages/client/ui-settings-models/tests/provider-form.client.spec.tsx) exercise both create and existing-provider entry points, verify the saved path operations, clear overrides to inheritance, and preserve a stored non-reasoning declaration during an unrelated edit. The [assembled Models scenario](../../../../apps/web/tests/models-settings.e2e.ts) saves the declarations through the real Host, resolves the resulting modality and effort metadata, and reopens the editor against a keyless UI snapshot. A tag component passing in isolation does not establish that either form renders it.
