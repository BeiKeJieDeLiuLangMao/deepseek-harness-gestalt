...
在 Web 侧，正在运行的 continuable child 与普通会话共用同一个 primary Send/Stop 切换：客户端 `Session.cancel()` 将 Stop 路由到 `subagent.interrupt`（one-shot 地址保持不可取消，普通会话仍走 `session.cancel`）。child 运行期间 Enter 仍将后续消息加入队列。parent 离线但仍在运行的 continuable child 保留默认 composer，禁用输入区，但 primary Stop 仍然可达；停止后恢复为只读接管界面（周边目录与 composer 约定由 [Web subagent 对话](2026-07-27-web-subagent-conversations.md)拥有）。
