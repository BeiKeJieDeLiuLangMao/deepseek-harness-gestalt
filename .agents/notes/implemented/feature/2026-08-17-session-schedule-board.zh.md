...
`ctx.schedules` Service 拥有 `schedules/pause`、`schedules/resume` 与 `schedules/delete`。API resolver 根据 wire Session id 提供精确的 live 根 Agent。变更会验证该所有权，并与工具管理和到期投递共享同一条 Agent-scoped FIFO、preflight flush、append、post-append flush 和 runtime 重算。Session 日志仍是唯一持久权威，因此暂停与恢复无需另一个存储即可在重启后保留。
...
Schedule domain 与 restart 测试覆盖有效和无效 pause/resume 转换、删除 paused、列出 paused、runtime 排除、FIFO 顺序、持久化 barrier 与重启保留。Projection registry/cache 测试覆盖 live、cold、checkpoint 与 restore 路径中的 owned-suffix 行为；Schedule projection 测试覆盖创建顺序、pause/resume/delete/dispatch、畸形变更与 fork 前缀。Typert 与 Client 插件测试覆盖 Remote 挂载、标题栏顺序、活跃计数、状态行、pause/resume、行内删除确认、失败、焦点与生命周期清理。

一个无密钥 assembled Desktop 浏览器场景从只含持久 Session fixture 的状态经真实 HTTP 启动 Web bundle。它证明任务板来自 projection，通过生成的 Remote 暂停，reload 后仍保持暂停，随后恢复，并通过行内二次确认删除，且匹配已提交的无障碍快照。记录的浏览器截图为完整 A 版任务板提供视觉证据。
