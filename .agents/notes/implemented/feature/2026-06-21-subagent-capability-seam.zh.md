...
`dsh-tool-subagent` 绑定到恰好一个提供方名称（`Config.provider`）。若要暴露多种传输方式，请多次加载该工具插件，每次绑定不同的提供方和不同的 `toolName`（工具注册表拒绝重名）。*服务*持有多提供方注册表；*工具*选择其中一个——schema 中没有 spawn/fork/ACP 选择器。可选的按次 LLM `provider`/`model` 是另一条轴，由 `SubagentCapabilities.agentOptions` 门控（[按次 LLM 路由](2026-08-19-subagent-per-call-llm-route.zh.md)）。
