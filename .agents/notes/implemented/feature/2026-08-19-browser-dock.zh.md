...
第一个 Agent 标签页会打开 Dock。人工收起后，后续 Agent 活动不会再次打开它。收起预览是同一 Dock 的单行分层摘要。点击后层会用该标签页在列表中的修订号聚焦它；点击当前层会打开 Dock。Dock 可见时预览隐藏。普通 MCP 工具行仍留在对话历史中。选中 `browser_*` 工具行会聚焦列表中的标签页，且不写入 `dockOpen`；该路径由 [对话浏览器工具聚焦 Agent Note](2026-08-20-chat-browser-tool-focus-dock.md) 持有。
