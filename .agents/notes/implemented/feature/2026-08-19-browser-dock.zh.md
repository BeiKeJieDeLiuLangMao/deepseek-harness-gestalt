...
Host 组合插入 `dsh-browser-runtime-deterministic`、`dsh-browser-workspace` 与 `dsh-tool-browser`，使 Dock 拥有 Session 持有的 Runtime，而不需要第二个 Electron 进程。Web 组合会禁用宿主平面的 `tool-browser` 行，让 standard、code 与 cordis preset 重新挂载它，行为与 `tool-web` 相同，并挂载 Dock 插件。
