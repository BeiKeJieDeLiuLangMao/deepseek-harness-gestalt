...
Composer-staged PNG, JPEG, and static WebP images expose an annotation mode on the existing preview. History images reuse that mode on the ordinary preview; placing or selecting a pin opens the same note editor inside the preview portal. The annotate control sits beside the preview close control. Pins are numbered speech-bubble marks on the displayed raster. Pins use percentage coordinates against the displayed, EXIF-oriented raster. Animated GIFs stay sendable and refuse pins. Submission sends the original image with localized coordinate prose and no derived raster.
