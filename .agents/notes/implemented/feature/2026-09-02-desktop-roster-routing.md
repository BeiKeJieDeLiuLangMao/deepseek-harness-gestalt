...
Bound Desktop sessions never fall back to the local ask tool when `to_project_member` is present. The Web Host mounts `ctx.memberQuestionSender` without a production delivery port, so a constructed routed ask fail-closes as `DELIVERY_UNAVAILABLE` until the reviewed T4 registry transport is composed. Unbound sessions hide the routing parameter. Arbitrary non-roster addressees fail closed. The loopback projection is read-only; Project mutations remain renderer-to-Desktop IPC.

## Testing

`packages/interaction/tool-ask-user/tests/tool-ask-user.spec.ts` pins public-login routing, `INELIGIBLE_ADDRESSEE` before delivery, and cancellation through `routeResolver` and `boundProjectResolver`. `packages/platform/project-membership-desktop/tests/desktop-provider.spec.ts` pins case-insensitive login matching and abort of pending loopback reads. `packages/platform/project-membership-desktop/tests/loader-composition.spec.ts` boots the Web Host sender, Desktop membership bridge, and standard-preset `routeResolver` through the Loader and pins a bound routed ask that fail-closes as `DELIVERY_UNAVAILABLE` rather than `SENDER_UNAVAILABLE`, using the public GitHub login rather than an injected Account id. `apps/desktop/tests/project-membership-agent-runtime.spec.ts` pins Installation-sampled identity, actor mismatch, account-switch refusal, and quiescent disposal. `apps/desktop/tests/overlay-isolation.spec.ts` pins `ctx.memberQuestionSender` on the dumped Web Host and Desktop overlay. `snapshots/session/member-question-routed-ask` keyless snapshot replay pins `project_members` then `ask_user_question` with `to_project_member` set to the live public login, not an Account id.
