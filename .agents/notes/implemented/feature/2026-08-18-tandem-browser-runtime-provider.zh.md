...
Provider 在一个 loopback HTTP origin 上接收临时与命名持久 Profile。命名 Profile 映射到一个 HTTP session 与一个 `persist:session-*` partition；临时 Profile 使用临时 `session-*` partition。DSH 持有的不透明 Profile/Workspace/浏览器身份由 `idPrefix` 派生。所有操作通过同一个队列串行执行；写操作精确校验 `expectedRevision`。后续 create 失败时，已打开 Profile 的 origin 继续运行。释放阶段停止接收新操作、排空队列、销毁剩余 session，并在 `processGraceMs` 内 join 可选的 fixture 子进程。
