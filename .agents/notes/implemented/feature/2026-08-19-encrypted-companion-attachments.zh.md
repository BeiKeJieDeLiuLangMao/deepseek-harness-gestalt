...
Mobile 读取浏览器 `File`，使用该确切配对独立的随机 32 字节 attachment key 密封字节，擦除本地副本，以当前 Installation 证明只上传密文，并仅通过当前 generation 的 Snow channel 发送有界 operation。Desktop 在 durable confirmation transaction 中生成该 key，并在第一条 XKpsk3 transport payload 内与 Mobile grant 一起交付；transcript hash 仅用于认证词。Desktop 把已鉴别的 Relay selector 映射到已确认 pairing id，查找该 endpoint 所有的确切 key，校验并解密 blob，再调用 loopback Host 的 `session.admitAttachment`。`AttachmentStore.saveBytes` 把确切字节原子发布为 `ByteAttachmentRef`；Session Controller 的 [`session.admitAttachment`](2026-09-05-session-admit-attachment-remote.zh.md) 追加只写入日志的 `session/attachment-admitted` 引用并 flush，不会把字节或文件名文本加入模型历史。相同 operation 重试返回已记录引用，冲突 operation id 会失败。撤销会独立擦除 attachment key 与 96 字节 IK reconnect record。
