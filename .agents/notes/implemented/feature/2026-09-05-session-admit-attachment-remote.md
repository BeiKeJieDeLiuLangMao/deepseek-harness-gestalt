...
`SessionController` owns `session.admitAttachment` through `SessionAttachmentAdmission`. The method resumes the ordinary Session, then reuses `ApiSessionAgentController.serializeImageAdmission` so concurrent `operationId`s for one Session serialize at the live Agent/Session commit point. After the chain holds, it scans the Session log for that `operationId`: matching `sha256` / bytes / mediaType / name — compared after `displayName` from the attachment seam — returns the recorded `ByteAttachmentRef`; a mismatch fails as `ATTACHMENT_OPERATION_COLLISION` without appending. A first admission calls `saveBytes`, appends ignorable `session/attachment-admitted` (`source: 'companion'`), and `sessions.flush`. A recorded in-memory admission is not reported successful until flush completes, so a later retry of the same `operationId` finishes persistence. Wire bounds match the historical Companion schema (operation id ≤ 128, media type ≤ 127, name ≤ 255, canonical base64 for ≤ 100 MiB). Opaque files never become `ImageBlock`s or prompt text. `session.attachment` remains image read. Desktop still targets the name `session.admitAttachment`; swapping the Desktop caller off ApiProxy is a later wiring change.
