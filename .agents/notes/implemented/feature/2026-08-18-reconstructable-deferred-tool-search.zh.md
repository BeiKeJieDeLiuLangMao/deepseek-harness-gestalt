...
`ToolDefinition.deferLoading` 标记不会进入初始请求的已注册定义。随仓交付的 base bundle 启用 `dsh-tools.toolSearch`，由它贡献保留的 `tool_search` 基础设施 schema。搜索索引根据调用 Agent 当前解析后的视图重新构建，只包含合资格的 deferred 定义。其规范结果是精确匹配的 `ToolSchema[]`；它绝不会注册、启用或以其他方式激活工具。
...
`schemas()` 表示初始模型请求，会省略 deferred 定义。`catalogSchemas()` 表示 Host 与检查接口使用的当前完整合资格末端工具目录。MCP 实例通过 `deferLoading` 按服务器选择加入；发现期间，其完整实时世代始终保持注册。Code Mode 会把嵌套 `tool_search` 子分发得到的 schema 带到外层 `run_code` 结果，并在下一次组装时把这些定义加入生成的 SDK。
...
注册表测试证明初始省略、合资格目录保留、精确 schema 结果、持久的下一次请求与恢复重建，以及 allow-only 资格变化后的拒绝。MCP 测试证明按服务器延迟加载。pi-ai 测试证明 provider-neutral 元数据转换与原生 OpenAI Responses 请求载荷。无密钥 headless 示例通过真实 Agent、Session、system-prompt、工具注册表和 agent-loop 组合，运行搜索、续轮、deferred 执行、最终结果、请求 header 变化与恢复重建。
