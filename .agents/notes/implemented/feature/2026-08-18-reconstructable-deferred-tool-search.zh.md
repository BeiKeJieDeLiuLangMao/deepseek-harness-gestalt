...
agent loop 会把匹配 schema 存到持久 `tool-result` 块。每次提示词组装都会把这些恢复结果当作文件输入，并从 `Session.deriveMessages()` 读取。它只安全提取每个候选项自有、可枚举的字符串 `name`，丢弃不存在于当前合资格 deferred 视图的名称，并在不读取嵌套 schema 数据的情况下对其余原始候选项去重。随后，它把该原始合资格集合序列化为一个规范重建发现块，应用当前字节预算，再在投影前完整校验每个保留的 `ToolSchema`。无效或超限的当前 schema 会使组装失败，而不会抵达模型；格式错误、使用不支持 dialect 或体积巨大的过期项无法污染组装或消耗发现预算。因此下一次请求携带搜索实际返回的精确 schema；工具移除或更窄的 allow-only 贡献会阻止旧历史恢复或分发它。`request/header` 继续记录完整的已组装请求工具，使回放只有一份权威请求快照。
