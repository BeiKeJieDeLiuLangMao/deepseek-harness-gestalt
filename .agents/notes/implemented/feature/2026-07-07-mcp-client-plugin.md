...
1. On connect: drain `client.listTools()` pagination, derive every tool's `publicName`, then register each as a raw `ToolDefinition` via `ctx.tools.register()`. The MCP JSON Schema and description pass through unchanged (no `defineTool` DSL conversion); only the model-facing `name` is replaced. When `config.deferLoading` is true, `ToolBridgeOptions.deferLoading` copies onto each definition so `tool_search` publishes the schema and the initial request omits it; `apply` still rejects that configuration before connecting when `toolSearch` is disabled.
