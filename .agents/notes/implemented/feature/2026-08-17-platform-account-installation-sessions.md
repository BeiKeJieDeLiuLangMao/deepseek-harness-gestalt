...
Development and production use different HTTPS origins, fixed callbacks, GitHub OAuth Apps, credential references, database identities, and identity namespaces. Each Desktop and Mobile composition parses both complete identities and requires an explicit selection before rendering or traffic. The selected value binds the HTTP Consumer's one required CORS origin, client transport, OAuth adapter, backend database identity, local store, callback, and issued identity namespace; a missing or mismatched Consumer origin fails before route registration, and a single environment configuration cannot bypass the pair validation. HTTP responses, IndexedDB records, and Desktop encrypted files have explicit parsers. One lifecycle transition owner serializes load, login, polling, refresh, Account switching, and sign-out.
...
Platform deployment must supply atomic Account persistence, distributed invalidation, OAuth credentials, signing keys, audit retention, and HTTPS edge behavior. The operated listen process accepts only production; [production-only release CI](../process/2026-08-20-platform-production-release-ci.md) owns that restriction, while Desktop and Mobile compositions still parse a complete pair before selecting an origin. Spec-fixed open-registration installation and connection ceilings live in the Account provider ([open-registration quotas](2026-08-19-open-registration-quotas-capacity.md)). The in-memory backend and bus are acceptance/development adapters, not production durability. Native Mobile packaging must supply a stable WebView origin; the Mobile composition owns its Capacitor Browser adapter. Account deletion, session lists, remote sign-out, sign-out-all, recovery, identity linking, Personal Pairing, and Remote Access remain separate capabilities.
