...
无密钥组装 Member Question 路径覆盖 Markdown 读取、沙箱 HTML 渲染、receiver-owned path 与折叠/展开行为。Definition 测试覆盖全部六个注册与二进制重匹配；runtime 测试覆盖脏状态保留与关闭准入。图片、PDF、代码编辑、二进制下载、文件树变更、分栏位置、打开方式、保存快捷键与布局重挂载保留不在该组装场景内。已有 Better 组件测试会运行共享 viewer、编辑器和文件树实现，但不能据此确认这些路径在官方 occurrence 中验收通过。
