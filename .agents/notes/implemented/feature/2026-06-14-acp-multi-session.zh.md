...
ACP 桥接层将活跃会话存储在 `Map<SessionId, AcpSession>` 中。agent 作用域的回调使用 `ownedRecord`：在正向 map 中查找 `agent.session.id`，且仅当该模块拥有精确的 agent 对象时才接纳它，使外部的同 id 对象无法冒领会话。该模块拥有 Agent 句柄、MCP 挂载、模型选择、有序更新、记忆化关闭，以及可选的进行中提示词和最终结算它的持久轮次号。会话 header 拥有其 cwd；桥接层不保留平行的工作区或客户端能力状态。
...
单会话关闭、连接拆除与插件 dispose 共享各个 `AcpSession` 的记忆化关闭操作。事件与更新排空期间，桥接层保留活跃 map；它将每个待处理的提示词以取消状态结算，排空可继续的后代，在会话仍附着时刷写，并行 dispose Agent scope。只有精确记录的关闭操作结算后，才会把它从 map 移除。
...
标准 `session/close` 可独立让一个活跃会话进入静止状态，并保留其持久状态以供恢复。`session/list` 会省略活跃记录，而 `session/resume` 会拒绝仍处于活跃状态的 id，避免自动化客户端为同一持久会话创建两个 Agent 对象。
