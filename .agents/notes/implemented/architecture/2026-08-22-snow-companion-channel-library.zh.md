...
生产 Platform 挂载持久 PostgreSQL pairing authority、PostgreSQL Relay route store、Redis directory 与 coordination adapter、pairing HTTP 和 Relay WSS。Platform 不提供配对密码实现：旧的 Platform 中介操作 fail closed，产品端点只使用 mailbox 操作。Desktop 把 reconnect state 保存到 Electron `safeStorage` 保护且 owner-only 原子替换的文件。Active vault record 的 `PersonalPairingId` 是 retention 与 release identity；持久化的 `desktopGrant.pairingSelector` 是 authenticated reconnect-state 与 attachment-key lookup identity。两者不可互换，也无需相等。Protected-state load 会拒绝重复 active pairing id 与重复 Relay pairing selector，使重启后的 ownership 和 selector lookup 保持唯一。Mobile 把 reconnect state 与 Mobile-only Relay grant 保存到账号隔离的 IndexedDB 记录。每次物理重连都使用新的 attachment id 与新的 IK 临时密钥。
