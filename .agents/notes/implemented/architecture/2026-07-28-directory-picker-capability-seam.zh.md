...
- **API 适配器里直接调标准库（不建 seam）。** 否决：换装点仍是改适配器源码，失去 fixture（测试前置数据）／测试后端，与促成这项工作的插件教义相悖。
...
- 协议暴露生成的 `directoryPicker/list` 与 `directoryPicker/createDirectory` 方法及四个错误码；Connection fixture 提供确定性浏览树与确定性的 `directoryPicker/pick` 结果供无密钥组装测试使用。
