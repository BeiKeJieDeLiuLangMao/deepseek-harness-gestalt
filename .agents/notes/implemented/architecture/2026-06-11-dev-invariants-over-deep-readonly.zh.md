...
被接受的事件及其所有后代在发布前被深度冻结。`append()` 返回由 Session 拥有的冻结事件，`session/event` 观察者与 `eventAt(seq)` 接收同一记录。`snapshotEvents(fromSeq?, toSeqExclusive?)` 返回冻结的数组快照；先前返回的数组不会因后续 append 而增长。调用方只需要当前长度或单个事件时，`seq` 与 `eventAt()` 可避免物化数组。种子记录在构造成功前经过相同的验证、快照与冻结边界。
...
- `snapshotEvents()` 暴露稳定的不可变快照，而非持续增长的私有数组；`seq` 与 `eventAt()` 无需复制该数组即可完成标量读取。
