...
`modelRoute`, `commandCatalogSessionId`, and `skillCatalogSessionId` are lookup-only helpers. Omitting the catalog helpers hides those catalogs for a feature-owned Session. A catalog-addressed subagent without a feature route also hides commands and skills. No UI consumer reads those helpers in this slice. `historyScope` is declared and unread. This slice does not install a stock model catalog for ordinary Sessions and does not register the Side Chat product adapter.
