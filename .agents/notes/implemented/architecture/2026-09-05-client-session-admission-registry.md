...
Canonical `Session` prompt, cancel, queue, and command can be owned by a Client plugin without Host privilege. `ui-model-selection` reads live `sessions.modelRoute`: ordinary and catalog-addressed Sessions keep the Host catalog, and a feature route can hide or replace it. Side Chat registers its adapter on ClientSessions; skill/command catalog consumers and history suffix trimming remain later.

## Testing

`packages/api/session-controller/tests/session-admission.client.spec.ts` pins the same dispatch cases, prompt `requestId` forwarding, stock ordinary `modelRoute`, catalog-child hide unless a feature route opens it, omit-vs-explicit-undefined, and Host-aligned refusal to call `session.selectModel` for subagent identities. `packages/client/ui-better-sidebar/tests/sidechat-admission.client.spec.ts` drives `installSidechatAdmission` through live `ClientSessions` and `Session.prompt`. `packages/client/ui-model-selection/tests/model-directory.client.spec.ts` drives `ModelDirectory` load, select, and live hide through `subscribeAdmission`. `packages/client/ui-model-selection/tests/model-select.client.spec.tsx` hides the composer control when the directory store publishes `available: false`. `queue-store.client.spec.ts` pins queue mutation through an injected admission resolver.

The shared Client test runtime mirrors admission precedence, replacement, disposal, subscription, catalog lookup, and stock model-route availability. Its complete `inputState` and `inputActions` builders keep standard owner props aligned with the public input face; omitted actions fail at the call site instead of passing through a partial cast.
