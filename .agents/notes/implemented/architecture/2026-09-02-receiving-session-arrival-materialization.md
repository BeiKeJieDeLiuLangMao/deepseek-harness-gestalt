...
Authenticated arrival materializes exactly one Host Session for the `(originSessionId, receiving Account)` route key. The Host receiver still owns the opaque `ReceivingSessionId`. After committing the question row, `ingest` calls one injected Session materializer with the invitation-time Workspace binding and every retained question on that thread. `SessionController` registers that unique materializer and terminal Session sync as one Cordis effect: it reuses the receiver id as the Host `SessionId`, attaches the bound Workspace, titles the Session from the first brief origin line, appends ignorable `member-question/received` and any already-canonical `member-question/settled` records, injects each bounded brief with a stable plugin message id, and flushes persistence. A later durable terminal appends ignorable `member-question/settled` once; a failed flush retries after `receivingTerminalRetryMs`. Unloading the controller cancels timers, refuses stale writes, waits for in-flight syncs, and withdraws the registration. Injection stages model-visible context without waking the driver, so arrival request count remains 0. Human-turn admission remains on the remaining Host composition (currently ApiProxy).
