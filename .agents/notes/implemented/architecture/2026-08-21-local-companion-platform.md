...
[`examples/local-companion-platform`](../../../../examples/local-companion-platform/README.md) is the long-running development listen. It binds one `127.0.0.1` TLS endpoint, alternates `/v1/*` and Relay upgrades across two in-process instances, and shares memory Account, pairing-authority, and Relay route stores. The selected development identity is that TLS origin; production identities remain the operated `www.gestaltrun.com` pair so client pair validation still rejects a shared identity. GitHub authorization in this composition is `/v1/account/oauth/github/development-complete` on the same origin and always yields the `octocat` public identity. `LOCAL_COMPANION_PAGE_ORIGIN` reverse-proxies non-`/v1` paths to Mobile Vite so the browsing context can share the TLS origin, and the TLS front rewrites that Vite origin onto the selected HTTPS origin for Account and pairing CORS when a WebView cannot trust the bundled certificate. [`apps/platform/src/boot.ts`](../../../../apps/platform/src/boot.ts) does not import this example or `DevelopmentKeylessPairingHandshakeProvider`.
