...
`web_fetch` 的实现是一个匿名公开 HTTP(S) fetch 提供方 `http`。它从具体 URL 获取字节，解析并固定公开目的地，应用下述传输卫生措施，解码文本内容，并仅返回最小的模型可用结果：最终 URL、状态码、正文和截断标志。它不携带浏览器 cookie、编辑器凭证、git 凭证、内部认证令牌，也不隐式访问私有服务。
...
- 字面地址或一次主机名查询的完整结果必须只包含全球可达的单播 IPv4 或 IPv6 目的地。IPv6 解析还会发现当前 DNS64 前缀，并拒绝会转换为非公开 IPv4 的 NAT64 地址。回环、私有、链路本地、运营商级 NAT、多播、保留、过渡、转换及私有 IPv4 映射 IPv6 地址都会被拒绝。
- 请求在 Undici lookup 回调中保留已验证的地址集合，而不会在连接时再次解析主机名。原始主机名仍作为 HTTP Host 与 TLS SNI 值，DNS rebinding 无法在验证后替换连接目的地。
- 强制执行最大 URL 长度、响应字节上限、解码正文字符上限、超时和重定向跳数上限。
- Abort 信号传播到网络获取和高开销解码。
- 仅自动跟随同源重定向；每个跟随的 hop 都会重新查询公开地址，并固定自己的连接。跨源重定向以 `WEB_REDIRECT_BLOCKED` 失败，要求一次新的工具调用和新的公开地址验证。（Claude Code 的 WebFetch 使用同样的模型——它不自动跟随跨主机重定向，而是将重定向目标返回给模型以发起新调用。）
- 请求携带显式的产品 User-Agent，而非静默伪装浏览器。

只要 DNS 应答集合中存在任何非公开地址，提供方就会拒绝整组答案，而不会静默过滤不安全成员。该 fail-closed 规则防止连接族选择或 fallback 抵达未通过公开网络策略的地址。
...
提示词引导解释了语义分工——`web_search` 用于发现和获取当前信息，`web_fetch` 用于模型需要特定 URL 内容的场景——提示词和工具结果告诉模型用 Markdown 链接引用相关 URL。每个成功结果都会把提供方控制的文本标记为外部不可信数据。Fetch 转换会移除活动和隐藏的 HTML 内容；不安全的转换返回固定的省略标记，而非原始 HTML。
...
否决，因为普通 fetch 在打开连接时会再次解析主机名。攻击者可以在验证时返回公开地址，在第二次查询时返回私有地址。通过连接的 lookup 回调传入已验证答案集，可以在保留基于主机名的 HTTP 与 TLS 行为的同时关闭这段 rebinding 窗口。

### 只阻断看起来私有的主机名字符串，不固定解析地址

否决，因为主机名语法不能确定连接目的地：任意看起来公开的名称都可能解析到回环、私有网段或云元数据地址。地址分类应在解析后进行，而且可供连接 fallback 使用的每个地址都必须通过检查。

### 公开 fetch 每次调用都要求审批

在交付的 preset 中否决。公开地址验证会阻断 SSRF 目的地，而逐次确认会打断普通浏览，却无法可靠控制公开数据外发：模型仍可通过已挂载的 shell 工具访问同一公开网络。需要专门确认步骤的部署可以添加 `tools/pre-execute` policy 或禁用 `web_fetch`。
...
**Fetch 是网络边界，不仅仅是只读工具。** 公开地址验证与连接固定会阻止 `web_fetch` 抵达非公开目的地，但模型仍可能通过公开 URL 泄露数据，获取到的文本仍是不可信的模型输入。交付的 `cordis`、`code` 与 `standard` preset 会在每种 sandbox 和 approval mode 下暴露 `web_fetch`，且无需逐次确认。
