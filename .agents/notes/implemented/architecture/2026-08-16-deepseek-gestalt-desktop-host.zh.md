...
- Desktop 图标测试固定三个源文件摘要及其容器签名，要求 512x512 RGBA PNG，并检查 macOS、Windows、打包资源、Dock 与 BrowserWindow 的接线。
