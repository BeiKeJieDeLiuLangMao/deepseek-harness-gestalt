...
- 发布计划测试覆盖版本、分支和已有标签校验；release-note 测试覆盖双语渲染、manifest 完整性、版本与标签一致性、Git ancestry、提交数计算和工作流顺序；CI 工作流测试要求每个执行该校验器的已启用或手动作业都具有完整历史，并保持禁用参考作业不可执行。发布资产测试要求两个更新 feed、全部版本化 macOS 与 Windows 安装包及其 blockmap，并排除未打包应用内部文件。
