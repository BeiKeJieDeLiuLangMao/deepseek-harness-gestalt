...
The Encrypted Companion Protocol owns `operation-failed` as the lossless Host failure result. Its closed categories are HTTP status, invalid wire response, typed business error, and timeout. HTTP failures retain the numeric status, including 400; business failures retain a bounded code and message; every failure carries the originating operation id. The Desktop loopback client parses HTTP status before JSON, validates the RPC envelope and echoed id, and enforces `timeoutMs` as one absolute wall-clock deadline even while response chunks continue arriving. Its configured response accumulator cannot exceed the 60 KiB Companion application-message ceiling; cumulative overflow destroys the response and becomes an invalid-wire failure. It returns values rather than throwing these expected failures. The Mobile surface accepts results only through a receiver bound to the decoder's physical connection generation, correlates a current result with the current search operation, and exposes the failure as an alert; disconnection, replacement, or backgrounding makes the old receiver inert.
