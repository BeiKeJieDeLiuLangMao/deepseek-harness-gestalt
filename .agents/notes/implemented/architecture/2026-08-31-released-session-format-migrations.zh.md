...
静态历史清单包含 Gestalt v0 浏览器工作区快照、成员提问 asked/outcome/received/settled 记录、附件接纳引用和空模型允许列表。冻结校验器无需加载功能插件即可保留载荷与 ignorable 标记。浏览器修订号、时间戳、附件摘要和缓存文档路径属于各所有者的数据，不是 Session 序号引用。未知标签和畸形的已知载荷仍拒绝迁移。完整目录测试覆盖 v0 到 v3，并验证源输入保持不变。
