...
`packages/phone/phone-runtime/src/runtime-pool.ts` is an internal external-occupancy core. Each `acquireExternal` mints a new `PhoneRuntimeOccupancyId` and handle. Two external acquires share one Host-lifetime slot and one generation. `acquireIsolatedIos` stays on the type and rejects `PHONE_UNAVAILABLE` until a private HOME/simulator-set/listen-bind provider exists; it never falls back to the external generation. `PhoneRuntimeGeneration` has no dispose. Caller drop is `handle.release()` for that occupancy only. The pool stores the Adapter-returned `stop` bound to that child. Late start after abort/replace/dispose calls **that** `stop` and does not install. Replace bumps epoch; old handles reject `PHONE_ABORTED` and must not stop the new child. `stopExternal` aborts pending occupancies, leaves live occupancies unresolved, and joins a pending Adapter start plus its exact late-child `stop`. New acquire is refused until that cleanup settles. A cleanup failure stays on `lifecycle().cleanupFailures` and keeps the cell unavailable because child termination is unproven. Adapter `start` receives per-slot `config.provenance` (`host-external` | `host-isolated-ios`) plus optional executable/environment — never a Session id. Dispose/last-release race start/stop against configured `cleanupTimeoutMs`. The dispose Promise settles after that budget. `closed` means admission is closed, not that work finished. `cleanupPending` covers unresolved Adapter start as well as late instance `stop`. Late `stop`/`start` failures after the settled Promise are retained on `lifecycle().cleanupFailures` and must not become unhandled rejections.

`PhoneDevices` retains one external occupancy and delegates to the non-Service `MobilecliPhoneRuntime`. Each start creates a separate generation object. Private validation and listing/readiness callbacks are installed before startup; the facade keeps invariant identity and subscribers across replacement. Publication is revoked before cancellation. Ordinary disabled operations never acquire. Replacement checks cancellation after old-tree cleanup and admitted readiness before acquiring the replacement occupancy. Generation stop joins command operations, probes, capture readers, and exact Android trees, preserving cleanup failures and late cleanup ownership.

The sole `serverPort` configuration remains unchanged. Bounded port allocation, owned-listener validation, isolated HOME/private sets, DeviceKit arbitration, and native Host-SIGKILL containment remain unavailable. #574 remains open.
...
The Host-alive external occupancy core supports cancel-with-siblings, failed-start cleanup, and bounded dispose. Fake adapters exercise these obligations; two separate external pools using `stageFake` exercise independent generations, not isolated generations within one pool. Isolation remains unavailable until a private provider exists. Native descendant membership after Host death and bounded loopback port allocation remain unmet.

## Testing

`runtime-pool.spec.ts` covers distinct external handles, cancelled sibling `PHONE_ABORTED` without awaiting start, failed start then retry, dispose admission `closed` with `cleanupPending` through unresolved start and late `stop`, late cleanup failures on `lifecycle().cleanupFailures`, replace invalidation, pending-start containment and failed-cleanup restart refusal for `stopExternal` and replacement, memoized dispose, last-release/dispose cleanup budgets, isolated `PHONE_UNAVAILABLE`, and two fake external generations without a silent fallback.
