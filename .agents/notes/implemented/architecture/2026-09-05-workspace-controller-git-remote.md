...
Clients can read origin through `ctx.remote.workspace.gitRemote` without trusting a client-supplied filesystem path. Git-less Workspaces remain unbound until membership uses `local://workspace/<id>`. A failed clone leaves the exclusive child directory for the operator to delete or replace with a new name.
