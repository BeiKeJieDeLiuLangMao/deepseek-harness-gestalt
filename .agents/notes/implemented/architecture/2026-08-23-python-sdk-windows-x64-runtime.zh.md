...
GitLab 标签流水线会在 `windows-2025` 上构建 `node24-win-x64`，与三个 POSIX 目标并列。可复用 GitHub 构建器的 plan job 当前只接纳 `node24-linux-x64`、`node24-linux-arm64` 与 `node24-macos-arm64`，因此必需拉取请求 `python-runtime` job 不传入 `node24-win-x64`。公开 GitHub 发布 workflow 仍列出四个运行时目标，但在构建器补上 Windows runner 映射之前，该 Windows 名称会在同一 plan job 失败。目标解析、manifest、矩阵、发布内容与文档均不包含 Windows arm64。
...
Python 安装现在会选择无需 Node 的 Windows x64 运行时，并与 Linux、macOS 使用同一套显式 home 与 profile 自定义模型。GitLab 标签验证仍要承担该第四个可执行文件、运行时 wheel 与完整 keyless 黑盒测试；可信 head 还要承担真实提供方任务。GitHub 拉取请求 CI 当前只证明三个 POSIX 载体。Windows arm64 用户会收到明确的不支持平台错误，直到另一项原生产品决策提供并证明该载体。
