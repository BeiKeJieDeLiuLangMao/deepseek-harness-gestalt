...
The shared tsdown preset emits `lib/client.js.map` for every plugin and rewrites first-party source paths into the browser-resolvable repository shape `/packages/<group>/<package>/src/...`. Packages that restated the rewrite locally, including `ui-better-sidebar`, must still call that shared `browserSourcePath` so inlined workspace sources land on their `packages/` owner rather than a package-relative `../../../src/` or `../../../../../core/` path. Dependency paths remain unchanged; `sourcesContent` carries the source. A complete root build rejects first-party `.ts`/`.tsx` map sources that do not start with `../../../packages/<group>/<package>/src/` or `../../../../vendor/<package>/src/`. The Host serves the public `client.js.map` URL from the physical map and exposes no source route. The Vite shell also emits source maps, letting both shell code and out-of-graph plugins map stacks and performance profiles back to TypeScript/TSX.
