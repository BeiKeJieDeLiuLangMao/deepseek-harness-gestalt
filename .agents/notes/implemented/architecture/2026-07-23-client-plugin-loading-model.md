...
The [client shell layering note](2026-08-15-client-shells-and-dynamic-packages.md) defines the current static and dynamic package sets and the import rules between them. The loading machinery treats every `dsh.client` package as a host-graph row with one CommonJS `lib/client.cjs` factory bundle. Its declaration carries Cordis `inject` edges, synchronous module-table `external` requests, and the optional `immediately` prefetch mark; the composing app owns only the mounted roster.
...
The shared tsdown preset emits `lib/client.js.map` for every `dsh.client` plugin and rewrites first-party source paths into the browser-resolvable repository shape `/packages/<group>/<package>/src/...`. Packages that restated the rewrite locally, including `ui-better-sidebar`, must still call that shared `browserSourcePath` so inlined workspace sources land on their `packages/` owner rather than a package-relative `../../../src/` or `../../../../../core/` path. Dependency paths remain unchanged; `sourcesContent` carries the source. A complete root build requires those maps only for packages that declare `dsh.client` — the HTTP module host `/client.js` graph rows — and rejects first-party `.ts`/`.tsx` map sources that do not start with `../../../packages/<group>/<package>/src/` or `../../../../vendor/<package>/src/`. A workspace `lib/client.js` without `dsh.client` is not that route and may omit a sibling map. The Host serves the public `client.js.map` URL from the physical map and exposes no source route. The Vite shell also emits source maps, letting both shell code and out-of-graph plugins map stacks and performance profiles back to TypeScript/TSX.
