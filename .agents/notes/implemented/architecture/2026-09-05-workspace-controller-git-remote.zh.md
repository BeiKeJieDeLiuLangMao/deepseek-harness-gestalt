...
Client 可通过 `ctx.remote.workspace.gitRemote` 读取 origin，而不信任客户端提供的文件系统路径。无 Git 的 Workspace 在 membership 使用 `local://workspace/<id>` 之前保持未绑定。失败的 clone 会留下独占子目录，由操作者删除或另选名字。
