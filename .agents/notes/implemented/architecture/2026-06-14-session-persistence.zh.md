...
1. **接口**（`dsh-session-persistence`，`ctx.sessionPersistence`）：一个抽象的 `SessionPersistence` 服务，提供 `create`/`open`/`stat`/`list`/`export`；`create`/`open` 返回按会话划分的 `SessionHandle`，其上提供 `read`/`append`/`flush`/`close`（见[基于句柄的 seam](2026-08-27-handle-based-session-persistence.zh.md)）。其持久化单元就是现有的 `SessionEvent`（`{ type, seq, time, data }`），原样复用，无转换类型。
...
- **仅追加；崩溃的轮次被关闭，而非截断。** 已刷写的事件永不被重写。[语义检查点策略](../bug-fix/2026-07-21-semantic-session-checkpoints.zh.md)会在调用模型前排空请求、在调用工具前排空已记录的顶层调用，并在步骤结束后排空完整的响应/结果批次；循环则排空最终轮次边界。由于一个被中断的轮次可能包含大量有效工作，持久化会原样返回其连续、可解析的事件；平衡日志由读取方负责——恢复路径计算针对未应答 assistant 调用的风险分类错误结果、缺失的 `step/end` 与带 `{ kind: 'interrupted' }` 的 `turn/end`（`interruptedTurnClosers`），并通过写句柄追加它们；只读观察方则在内存中加入相同收尾事件。合成结果保证恢复后的提供方 transcript 仍然有效。只有撕裂的最后一次追加中的不完整片段会被丢弃——写入路径会在首次新追加前把其中恢复出的完整记录持久重写；已提交前缀中的解析错误或序号间隙属于数据损坏，会使该会话不可加载。
- **文件后端是规范实现，而服务保持可扩展。** `dsh-session-persistence-jsonl` 是唯一第一方提供方并通过 `runPersistenceContract`；抽象服务仍可供树外提供方实现。[仅 JSONL 持久化决策](../simplification/2026-08-30-jsonl-only-session-persistence.zh.md)负责移除第一方数据库提供方以及明确的兼容性切断。
...
- **`ctx.agents.create()` 和 `ctx.agents.resume()` 是异步工厂；恢复还跨越持久化边界。** `ctx.agents.resume({ resumeSessionId })` 打开会话写句柄、读取已存日志，并以持久化 id 发布准备好的 Session，继续其投影。[Session 准备阶段决策](2026-08-05-session-preparation.zh.md)负责未发布 Session 的所有权窗口。agent loop 不会硬注入 `sessionPersistence`（那样会让非持久化的演示永远挂起）；当它不存在时，`resume` 会以明确的错误拒绝。
...
格式版本控制：header 携带一个 `version`；冷读取拒绝任何非当前版本。预发布阶段的会话格式仍固定为 `SESSION_FORMAT_VERSION = 0`，不提供兼容性承诺：读取只接受当前 v0 记录，已退休的同版本格式会失败关闭（见[导出与预发布精简](../simplification/2026-08-27-persistence-export-and-pre-release-trims.zh.md)）。仅追加 + 刷写对尾部的不完整写入具有健壮性（冷准备时可容忍），但无法抵御未使用 fsync 时在行写入中途断电；数据库/WAL 后端是该场景下更强的选项。

## 后果

Service Definition、JSONL 提供方以及 `dsh-session` 中的元数据约定（`session.header`，`create(id?, options?)` 签名）带来持久恢复/fork、读取/回放路径、崩溃容忍，以及基于现有事件溯源日志的宿主侧会话访问。可复用的 `runPersistenceContract` 测试套件以相同的仅追加、连续 seq、惰性物化、逻辑恢复、整数元数据与可序列化语义约束该提供方及未来实现。持久化完整的逻辑日志还确定了事件保真度：即使 JSONL 将多个 `assistant/chunk` 打包到一条存储行中，每个事件也会精确保留。
