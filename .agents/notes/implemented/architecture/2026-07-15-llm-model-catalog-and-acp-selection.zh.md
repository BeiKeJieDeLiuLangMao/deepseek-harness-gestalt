...
> 目录与作用域选择决策仍然有效。ACP 选择的临时移除已由[标准 ACP v1 自动化控制](../feature/2026-08-22-standard-acp-automation-controls.zh.md)取代；后者通过标准会话配置暴露目录，而不会恢复 UI 投影。
...
ACP 自动化传输层通过标准会话配置选项消费建议性目录。其部署配置仍提供初始提供方／模型目标；每个会话拥有一个不透明的提供方／模型选择，以及依赖该模型的精确 reasoning-effort 选择。适配器拓扑变化会发布完整的选项状态。目录缺失绝不会使配置的路由失效：当前未列出的路由会被合成进选项。

### 提示词／请求一致性与持久化

`installModelSelection`（位于 `dsh-agent`）为前端拥有的选择安装 agent 作用域的 `system-prompt/assemble` 与 `agent/request` 监听器。普通消费方在每个步骤对选择做一次快照。ACP 会把准入快照与按会话模块中已识别的消息关联起来，直至 inbox claim，再把该选择固定到完整的已准入 turn，使异步图片准入、提示词变量与每个请求步骤保持对齐，同时不改变持久化 user source。并发选择会从下一个 ACP turn 生效。其他调用配置字段保持不变。
...
- 每个目录消费方拥有自己的选择交互。ACP 使用标准会话配置选项，不发出 DSH 特有的选择器或 UI 元数据。
- 请求头与基于提供方路由的会话形态保持兼容；不需要新的 JSONL 事件或格式版本。
- 目录读取可以是异步的，且每个调用方都会收到值的独立副本。

## 测试

单元测试覆盖目录值副本与格式错误的元数据、pi-ai 和 DeepSeek 目录投影、提供方／模型请求路由，以及提示词变量对齐；监听器安装在 agent 作用域的上下文中，因此能够实现 agent 间隔离。ACP 测试覆盖分组发现、无效和并发变更、拓扑更新、基于请求头的恢复、每 turn 路由固定，以及图片路由一致性；人类客户端各自测试其选择器展示。
