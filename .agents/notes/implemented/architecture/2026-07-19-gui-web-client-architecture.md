...
Client packages live under `packages/client/*`, with `apps/web` as the thin Vite application over the shell's boot export. Plugin packages keep their browser half under `src/client/`; **every build artifact lands in `lib/`** — the node half as ESM `lib/index.js`/`lib/invariant.js`, and the browser factory as CommonJS `lib/client.cjs` (the shared tsdown client preset emits both; there is no `dist/` directory). The runtime `exports["./client"]` keeps `lib/client.cjs` as its browser default and selects the dedicated `lib/client-node.js` ESM artifact for plain Node consumers. `ui-slots`, runtime, and ui-renderer form the infrastructure direction; feature plugins cooperate through services and slots rather than importing presentation implementations.
