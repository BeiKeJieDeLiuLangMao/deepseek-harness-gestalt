...
Web presentation owners keep implementation under `./src/*`. `ui-workspace` owns `SessionListState` grouping and `SessionNodeItem` rows; `ui-conversation` owns the keyed router for every finalized `ConversationNode`, Approval, and the narrow `InputBarPresentation` interface; `ui-tool` owns recursive Tool presentation, the built-in keyed roster, and unknown-Tool fallback; `ui-user-questions` owns Ask User; and `ui-attachment` owns message images. `ui-theme` exposes stable stylesheet subpaths. UI plugins do not publish extra `./presentation` values; Mobile imports internals through `./src/*`.
...
One presentation fix reaches Desktop and Mobile components, and Mobile tests exercise the same implementation files as Desktop. UI plugins do not publish extra `./presentation` values; Mobile imports internals through `./src/*`. The Mobile bundle also includes the shared Markdown and syntax-highlighting assets, increasing its initial artifact size. The JSON adapter is a transport representation, not a second presentation model: it exists only to reconstruct public Session/Workspace types and interaction `answer()` carriers without sending runtime classes or closures. The encrypted Companion v3 channel supplies the authoritative projection and generation-bound content and mutation adapters; product acceptance still requires the assembled operated Platform, Desktop, and Mobile flow rather than a component snapshot.
