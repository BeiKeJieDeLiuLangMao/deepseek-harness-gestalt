...
Mobile detail composition accepts the Client Runtime's `ConversationSnapshot`, `ConversationNode`, `ToolCallBlock`, and `PendingWait` values. `mountMobileEntry`, used by `main.tsx`, accepts the production `MobileCompanionPresentation` interface: Desktop-confirmed Session rows, a required Session-addressed image loader, current mutation authority, and optional create/submit/cancel/load callbacks. Mobile adds phone navigation and environment-derived locale/theme selection. It does not define a transcript content union, infer interaction authority from Relay lifecycle state, or mount Desktop columns, Settings, model selection, plugin configuration, or terminal input.
