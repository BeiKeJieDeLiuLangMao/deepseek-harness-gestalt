...
The Better Client entry now registers Files and viewers, Changes, Tasks, Side Chat, Terminal, Browser fallback, settings, produced-file and system-path routing, and the Host `sidebar_open` feed through the official Sidebar services and keyed Slots. A catalog or list selection whose durable title starts with `Side: ` opens or focuses that Side Chat occurrence on the ordinary owner instead of selecting the child as the shell Session. Browser Workspace and Phone register higher-priority or independent official definitions from their owning packages. The entry no longer creates a React root, provides `ctx.betterSidebar`, constructs the Better layout store or registry, imports the Better layout stylesheet, or installs the older open handlers.
