...
Feature Client packages do not construct `RemoteError`. Gateway's client face rebuilds wire failures in `invoke()` and folds carrier throws at stream boundaries. Session Controller is the other reconstruction owner: feature admission callbacks return the code-distributed `code`, `message`, and `details` fields, and its private admission runner rebuilds the public `RemoteFailure` for Session prompt, cancel, queue, command, and feature model methods. It preserves a structurally marked `RemoteFailure`; any other callback throw becomes `gateway/internal`, so a marker-free object with a plausible `code` never gains Remote authority. No feature Client imports the constructor, forges `isDSHRemoteError`, or declares another details map. A test double that needs a failure value takes `RemoteError` from `@deepseek-ai/dsh-client-test-runtime` instead of making a client package import the protocol as a value. Assertions match the code (plus details fields where they matter) with `toMatchObject`: `RemoteError` is an `Error`, its own-key set differs from the former literal, and `toEqual` fails on it.
