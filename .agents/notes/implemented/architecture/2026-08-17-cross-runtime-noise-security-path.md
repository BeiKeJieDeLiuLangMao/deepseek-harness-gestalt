...
Keep the bounded proof under `scripts/noise-security-path` rather than creating a product package. Product code must not depend on it. The proof owns two complete six-message Cacophony vectors, XKpsk3 and IK flows, bidirectional transport, fresh-ephemeral comparison, tamper/stale-pairing-transcript/transport-replay/order/cross-pairing/downgrade cases, a 65,535-byte maximum message round trip, and repeated 65,536-byte rejection. The stale pairing case accepts the old first message in a fresh responder state, emits a fresh second message, and rejects the old authenticated third message. A keyless snapshot pins the stable result while native disposable hosts prove the actual WebView executions.
