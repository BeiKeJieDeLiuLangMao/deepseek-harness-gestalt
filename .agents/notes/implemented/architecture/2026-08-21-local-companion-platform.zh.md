...
无密钥产品 Desktop 与 Mobile 共用 Relay 附着 id `desktop-development-keyless` 和 `mobile-development-keyless`。一字节入站帧是开发同步宣告；更长的帧是密封的 Encrypted Companion 消息。Web Host 打印 loopback URL 后，Desktop 用 Node `http.request` 绑定该 origin，并通过 Host `session.*` / `workspace.list` / `workspace.create` / `/api/respond` 确认 `create-session`、`open-session`、`submit-prompt`、`cancel-prompt`、`search-sessions`、`offer-attachment`、`settle-approval`、`answer-ask-user` 和 `query-operation-status`。同步与 Host 流帧投影按最近活动排序的 `session-catalog`；`search-sessions` 投影含标题、Workspace、摘要和 `session.search` snippet 的 `session-search` 页；打开的 Session 投影 Host history 以及现场审批与 Ask User 卡片，包括 inbox 拼接的用户原文和进行中的 assistant chunk。Host 无法接受操作时返回 `rejected`（`host-unavailable` 或 `host-rejected`），不再保持沉默。`offer-attachment` 通过 Host `session.prompt` 排队 `Attached: <fileName>`，让 agent 看到文件名。进程内回声权威只保留为单测替身。Mobile 用该 catalog 替换列表，向 Desktop 发起搜索而不是只过滤已缓存页，打开会话时发送 `open-session`，只在 Desktop 投影之后更新 transcript，在 Remote Offline 时拒绝作曲器 mutation，并在下一份 catalog 到达前恢复缓存元数据。已确认的 Mobile Relay grant 按 Account 保存，并在 `activate()` 时恢复，因此刷新后不必再粘贴一次性链接；账号页可清除该 Desktop 的 Companion Cache。Mobile 的 Workspace 名称会在 Web Host cwd 下采纳或创建 Host Workspace 目录。`DSH_DESKTOP_CWD` 或 `DSH_DESKTOP_USE_LAUNCH_DIRECTORY=1` 选择 Host Session 存储，使未打包 Desktop 能绑定与已打包 Gestalt 窗口相同的 cwd。Mobile 浏览连接标签跟随 `companionMayMutate`。
