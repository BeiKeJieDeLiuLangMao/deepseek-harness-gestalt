...
The first pairing uses an endpoint-owned opaque mailbox. Desktop creates and retains its XKpsk3 static key, ephemeral, and invitation PSK; Mobile decodes the invitation and creates messages 1 and 3 locally. Platform enforces Account ownership, expiry, ordering, single use, and idempotency while storing only opaque invitation and handshake bytes. After Desktop authenticates message 3, it creates the Mobile Relay credential locally. Platform registers only its SHA-256 digest with the route and pairing selector. Desktop seals the grant as the first XKpsk3 transport payload, then delivers only that ciphertext through the mailbox.
