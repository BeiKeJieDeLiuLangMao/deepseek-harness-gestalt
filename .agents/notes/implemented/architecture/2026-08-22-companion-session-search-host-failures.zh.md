...
Desktop 应用、Companion Host fixture 与 Companion browser driver 是独立 TypeScript Program。Fixture 拥有 Host 服务与 SQLite；driver 使用实际 Mobile 组件，Host 与 Client 的 `Context.sessions` 声明不会进入同一个 Program。即使某个 face 失败，三个 compiler face 仍都会报告诊断；普通类型与跨项目输出路径诊断仍会阻断。Fixture IPC 只传递准备命令和断言数据，不替代产品响应。
