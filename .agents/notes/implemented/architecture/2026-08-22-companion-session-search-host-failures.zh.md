...
Desktop 专用 composition 以 `openAt: first-search` 激活 `session-query-sqlite`，并把派生索引放在 `DSH_HOME/session-search.sqlite`；浏览器 `dsh web` 仍使用仓库默认的 `openAt: never`。`DesktopCompanionProductOwner.handle` 只允许 `search-sessions` 使用单元素参数 tuple；这个本地进程内 API caller 直接调用已安装的 loopback Host `session.search`，不需要 pairing dependencies、已连接的 Relay channel、Platform authorization request 或 mutation ledger。它不是可由远程未鉴别调用方访问的 endpoint。其他 Companion operation 都使用包含完整 authenticated pairing dependencies 的双元素 tuple。Mobile 会直接渲染每个关联的 `session-search` Session id/snippet 对，包括有界 Session baseline 与 Companion Cache 中不存在的命中，并且绝不会加入缓存中的标题、Workspace、摘要、transcript 或子串匹配。选择这类命中会立即打开以其 id 寻址的详情并请求 history；Desktop 会从完整 Host `session.list` response 的确切行解析其 running state，而不是只查询有界 baseline projection。

Encrypted Companion Protocol 以 `operation-failed` 作为无损 Host 失败 result。它的闭合类别包括 HTTP 状态、无效 wire response、类型化业务错误和超时。HTTP 失败保留包括 400 在内的数值状态；业务失败保留有界 code 与 message；每个失败都携带发起 operation id。Desktop loopback client 会在非 2xx response header 到达时立即结算，使 response body 累计与绝对墙钟 deadline 都不能替换已知状态。它会校验成功 RPC 的 envelope 与回显 id。普通 unary call 使用配置的 accumulator，且不超过 60 KiB Companion 应用 message 上限。`session.history`、`session.list` 与 `workspace.list` 可以改用由 transcript page 字节和条目上限推导出的更大固定投影源上限，因为 Desktop 会在 Encrypted Companion 编码前收窄这些完整 Host 值；attachment 使用独立推导的 admission 上限。Host event stream 使用同一投影源上限，因为 Desktop 在生成有界 live projection 前，只会从每个 frame 保留 interaction field 以及 Session 或 surface invalidation identity。累计数据超过对应上限时会销毁 response 或 stream，并产生无效 wire failure。这些预期失败会作为值返回，而不是抛出。Mobile surface 只通过绑定到 decoder 物理连接 generation 的 receiver 接受 result。它会把搜索 result 与当前搜索 operation 关联，并且只拥有 mutation channel 返回的一个尚未结算的 attachment operation id。sending 或 uncertain attachment 会阻止再次选择文件，直到确认、拒绝、失败或对账 status 释放该 id；其他 attachment id 的所有 result 都不会生效。attachment send completion 可以发布保留该 id 以便对账的 uncertain 状态。搜索与 attachment 失败会成为可见 alert；断开、替换或进入后台会让旧 decoder receiver 失效。

Desktop Host 会在发布的 Web Host 报告 loopback origin 后安装 `DesktopCompanionProductOwner`，在 Web Host 重启时替换该 RPC，并在关闭前移除。Desktop 入口 smoke 会创建真实 Session 并提交 prompt，再通过这个已安装 owner 要求一次索引命中与一次无命中的 `session.search`。这些结果只证明本地 Host 搜索安装与结果投影，不证明 Relay authorization 或 encrypted delivery。

Desktop 应用、Companion Host fixture 与 Companion browser driver 是独立 TypeScript Program。Fixture 拥有 Host 服务与 SQLite；driver 使用实际 Mobile 组件，Host 与 Client 的 `Context.sessions` 声明不会进入同一个 Program。每个辅助 Program 都把 `tsconfig.json` 放在其拥有的 test 源码目录内，使基于 project 的 lint 应用相同的 Host 与 Client 隔离。即使某个 face 失败，三个 compiler face 仍都会报告诊断；普通类型与跨项目输出路径诊断仍会阻断。Fixture IPC 只传递准备命令和断言数据，不替代产品响应。

Suite-local fixture 由 `driver.ts` 通过 `tsx/esm` import hook 从源码启动 `host.ts`，assembled browser test 通过 source alias 解析 `#testing/desktop/*` 与 `#testing/mobile/*`。这条拥有源码的路径不消费生成的 JavaScript 或声明。根 Client build 与 Client library、声明一起生成 Mobile browser bundle。built-bin 门禁随后在 `DSH_EXAMPLE_MODE=lib` 下运行 `apps/mobile/tests/mobile-browse-artifact.e2e.ts`，并禁用 source alias。该 smoke 经 Desktop 私有 import map 把 `#testing/mobile/MobileBrowse` 解析到持久化的 `apps/mobile/lib/MobileBrowse.js`，并在 Chromium 中渲染其生成 CSS；它不证明源码 fixture 或 assembled browser suite 消费产物。源码平面的 Host composition 会组合真实 Session event、SQLite provider、Host API carrier、loopback HTTP 与 Desktop owner，且不要求 Web client build artifact；它覆盖索引命中、无命中、已关闭和故障 provider。有界跨进程组装会把真实 HTTP 400 经 Desktop owner 与 Companion codec 送到发布 Mobile 入口的 alert。这些证据只证明发布 endpoint composition 与应用投影；经过评审的加密 channel 仍拥有 endpoint operation 交付与配对范围 attachment 依赖。
