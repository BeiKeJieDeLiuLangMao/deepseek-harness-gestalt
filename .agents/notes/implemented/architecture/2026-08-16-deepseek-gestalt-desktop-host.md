...
Electron supervises the Web Host through shutdown. Window exit, termination signals, and smoke completion stop the child and wait for process exit before the Desktop Host terminates; an intentional shutdown cannot trigger the one-time crash respawn.

The first Desktop Bundle is `0.1.0`, independent of the npm `dsh` line. The app id is `com.gestalt.deepseek`. Display name is DeepSeek Gestalt. Feed is GitHub Releases on `BeiKeJieDeLiuLangMao/deepseek-harness-gestalt` (`gestalt-v*` tags, non-prerelease). Mac notarizes with the 千机 team identity; Windows ships unsigned NSIS and still updates.
...
The macOS drag row leaves a fixed gap after the traffic lights and returns the sidebar toggle to its normal inset in fullscreen. Windows uses a full-window drag row with three non-drag caption buttons. Unsupported development platforms keep their system frame.
...
- macOS expanded, collapsed, and fullscreen layouts keep the sidebar toggle clear of native controls; Windows keeps its caption buttons at the right edge of the full-window drag row.
- Dock-style spawn uses the Launch Directory as cwd and does not register that path as a Workspace.
- Desktop shutdown waits for the Web Host process to exit, and the smoke test rejects an orphaned child.
