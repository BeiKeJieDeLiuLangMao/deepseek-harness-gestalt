...
The static historical inventory includes Gestalt v0 browser-workspace snapshots, member-question asked/outcome/received/settled records, attachment-admission references, and empty model allowlists. Their frozen validators preserve payloads and ignorable flags without loading feature plugins. Browser revisions, timestamps, attachment digests, and cached document paths are owner data rather than Session-sequence references. Unknown tags and malformed known payloads still refuse migration. Full-catalog tests exercise v0 through v3 and retain the source input unchanged.
