...
The reservation remains on the [`overflow-y: auto` `.scrollBody`](../../../../packages/client/ui-conversation/src/client/skeleton/ConversationRoot.module.css). Testing on the composer layers used when this decision was implemented, recorded in the [archived single-scrollport note](../../archived/bug-fix/2026-07-31-composer-text-layers-share-one-scrollport.md), showed that WebKit applied `scrollbar-gutter` to that box and ignored it on a hidden one. A hidden box would therefore have held in Chromium and silently failed in Safari. The overlay branch also retains `overflow-y: auto` as a clipping box nothing scrolls out of: a single-axis scroller computes the other axis to `auto`, so the horizontal axis is declared `hidden` rather than left to compute.
