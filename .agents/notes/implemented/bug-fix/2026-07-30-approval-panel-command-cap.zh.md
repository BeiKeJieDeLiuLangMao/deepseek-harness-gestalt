...
- InputBar 的 14 行上限通过自 `.composerSeat` 继承的自定义属性解析，并应用在 [`.scroll` 这个 composer 唯一的滚动盒](../../../../packages/client/ui-conversation/src/client/skeleton/InputBar.module.css)上。把输入栏渲染到该 seat 之外会丢掉这条声明（一个没有兜底值的未解析 `var()`），因此未来的 composer host 必须带上该属性——这也正是它声明在共享 seat 而非应用根节点上的原因。
