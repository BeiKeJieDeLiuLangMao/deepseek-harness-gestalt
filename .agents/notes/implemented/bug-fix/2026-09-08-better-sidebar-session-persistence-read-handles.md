...
`readPersistedSession()` opens a non-owning read handle, reads the validated complete log, and closes the handle before returning its `SessionInspection` metadata, exact `inheritedEventCount`, and events. A close failure after a successful read rejects the operation; simultaneous read and close failures produce an `AggregateError` carrying both failures. Route-specific parsing and preset composition start after the handle closes. Cold working-directory lookup propagates read failures; the Changes lens maps an unavailable cold read to an empty window. Side Chat uses the same helper for persisted model inspection and cold-resume setup, folds model state only from the exact child-owned suffix, uses `stat()` for a cold model-selection existence check, and uses `list()` for durable publication during close. The unused thread-log reader is removed.
...
Cold Side Chat resume reaches Agent write ownership after its read handle is released, so an active writer produces the ownership error instead of a missing-method failure. A nested Side Chat without an owned request uses its own descriptor model rather than an inherited parent descriptor or request. Persisted model inspection, cold working-directory fallback, Changes event replay, and durable close publication consume the current persistence service results. Every body read reports handle-close failure and releases its handle on success and failure.

## Testing

Persistence-read tests cover successful metadata and event reads, read failure, close failure, combined read and close failure, cold working-directory propagation, Changes empty fallback, and handle closure on every path. Side Chat lifecycle tests cover formal `stat` and `list` results, exact inherited-cut model recovery, successful preset recovery, read and parse failures, downstream preset-composition failure, and closure before those downstream operations. The keyless Web Side Chat journey holds an active write handle and verifies that cold resume reaches the expected ownership refusal, retains the draft, and leaves the durable log unchanged.
