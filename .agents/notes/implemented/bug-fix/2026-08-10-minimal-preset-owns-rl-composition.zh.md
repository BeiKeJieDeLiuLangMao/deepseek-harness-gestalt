...
独立的 [`sdk-minimal` bundle](../../../../packages/bundle/sdk-minimal/README.zh.md)是 `dsh --profile sdk-minimal` 的完整双工具组合。[裸双工具运行时决策](../feature/2026-08-11-minimal-profiles-bare-two-tool-runtime.zh.md)负责其启动方式专属的环境配置、裸文件系统与无压缩选择；[独立 profile 决策](../architecture/2026-08-24-standalone-sdk-minimal-profile.zh.md)负责其 launcher 与 bundle 位置。它的无密钥 SDK 进程测试断言组装后的系统提示词与双工具目录，installed-wheel 场景跨调用执行持久 Bash 并使用编辑器；Python SDK 教程提供可运行的入口。
...
Web RL 提示词固定不变，不能通过环境覆盖；独立 JSON-RPC 提示词由部署选择。Web preset 与 `sdk-minimal` profile 在各自的启动路径共享持久 shell 加编辑器行为；`sdk-minimal` 在 Windows 上选择 PowerShell。shell 状态按 agent 隔离，并随该 agent 一并消失。Web preset 为自身的 PTY 与裸文件系统服务实例承担开销，其他 preset 无需承担。Web preset 的 Bash 后端需要受支持的 POSIX 终端基础环境，因此该 preset 不支持 Windows agent。
