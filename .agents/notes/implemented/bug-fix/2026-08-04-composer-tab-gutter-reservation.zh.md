...
这条预留仍位于 [`overflow-y: auto` 的 `.scrollBody`](../../../../packages/client/ui-conversation/src/client/skeleton/ConversationRoot.module.css) 上。实现本决策时，针对当时 composer 图层的测试显示 WebKit 会对该盒子应用 `scrollbar-gutter`，却忽略 hidden 盒子；这份历史证据记录在[已归档的单滚动容器笔记](../../archived/bug-fix/2026-07-31-composer-text-layers-share-one-scrollport.md)中。因此，改用 hidden 盒子会在 Chromium 上成立，却在 Safari 上静默失效。覆盖分支也保留 `overflow-y: auto`，作为没有内容会滚出的裁剪盒；单轴滚动盒会把另一轴计算为 `auto`，所以横轴显式声明为 `hidden`，而不是交给推导。
