...
Web 入口拥有初始化，因为客户端插件不能依赖另一个插件先发现文档角色。[Official Browser](../feature/2026-08-21-workbench-official-browser.zh.md) 的原生视图设计仍是独立决策。此修复不实现[全屏 Settings 决策](../architecture/2026-08-27-settings-fullscreen-shell.zh.md)：当前 Settings root 没有 Desktop 请求订阅，因此 Settings bridge 请求没有对应的 UI 状态变化。
...
preload fixture 不能证明 Electron 视图层叠。原生 Electron 验收与缺失的 Settings 请求订阅仍是各自独立的义务；此修复不声称拥有 Settings golden。
