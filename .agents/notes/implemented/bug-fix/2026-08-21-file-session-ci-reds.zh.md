...
**pwsh spawn 在 `stdin_read` 或末行提示符时就绪；工具再等自己的 prompt。** `terminal-bash` 把 `stdin_read` 当作 spawn 就绪，因为该原因表示 shell 已在接输入（OSC+tail 或 `acceptsStdinWait`）。`inferred_idle` 仍要求最新非空白行在 send 视口或会话 scrollback 等于 `CONTROLLED_PROMPT`，横幅静默不算。setup 只提交一次：后来的空 follow-up 视口曾被当成「第一次发送」，再次倒入 prompt 函数源码。`tool-pwsh-persistent` 的 `get()` 随后循环到视口以 `__DSH_PERSISTENT_PWSH_PROMPT__ ` 结尾，才跑第一条模型命令，避免后端 setup 残片截断 `cwd=` / `keep=` 输出。剥掉 setup 源码后再 `includes()` 仍会接受 `setup-echo dsh> `；真实提示符还没打出时，每次 follow-up `startSend` 会重置自己的期限，于是 `inferred_idle` 饿死 `timeout`。现在由 spawn 的 `timeoutMs` 约束整个循环。ACP 与 loader 组合把 `idleSilenceMs`（以及会话交接处的 `handoffGraceMs`）加长，避免横幅到提示符的静默抢先结束。[persistent pwsh 笔记](../architecture/2026-08-11-pwsh-persistent-pty.md) 仍拥有双层 prompt 安装。
