...
**pwsh spawn 要等 ready-probe 标记；命令 idle 要等输出之后的提示符。** Linux pwsh 会在默认提示符、命令还没跑时就报告 `inputWaiting`，setup 回显的前几个字节也算输出。把这次等待当就绪，视口就停在残留 setup；每条路径都死等 `dsh> ` 会饿死 spawn，因为 Linux 常常只在下一次写入时才重打 `PS …>`。因此 pwsh 的 `acceptsStdinWait` 和 OSC `stdin_read` 只在当前 send 的最新非空白行是 `CONTROLLED_PROMPT`（`dsh> `）或工具层提示符后 settle。默认 `PS …> ` 重打不是已安装提示符。setup 与 ready-probe 发送设置 `bootstrap: true`，让 `inferred_idle` 可以在任意输出上 settle。spawn 随后写入 `Write-Output "__DSH_PWSH_READY__"`，只有看到该标记（或末行恰好是 `dsh> `）才返回。setup 回显加静默不是 spawn 就绪。普通命令发送不设 `bootstrap`，只在提示符行加上至少一行非提示符之后才 idle settle，因此单独的 `PS …>` 重打不能在命令跑完前结束。spawn 的 `timeoutMs` 约束整个循环。[persistent pwsh 笔记](../architecture/2026-08-11-pwsh-persistent-pty.md) 仍拥有双层 prompt 安装。
