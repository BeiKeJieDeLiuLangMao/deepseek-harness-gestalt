...
**`remote-access-client` 每个 entry 只发出一个文件。** 两个 tsdown face 都设置 `outputOptions.codeSplitting: false`，与 compaction 和 JSON-RPC demo 一致。包的 `files` 白名单与 `packageFileExtras` 不变。
