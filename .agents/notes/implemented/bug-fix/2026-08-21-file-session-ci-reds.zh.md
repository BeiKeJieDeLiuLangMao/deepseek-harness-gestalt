...
**pwsh 就绪是末行精确匹配，外加 spawn 墙。** `terminal-bash` 只把最新非空白行等于 `CONTROLLED_PROMPT` 当作已安装，send 视口或会话 scrollback 任一处即可。setup 只提交一次：后来的空 follow-up 视口曾被当成「第一次发送」，再次倒入 prompt 函数源码，末行永远停不住 `dsh> `，spawn 打到 `timeoutMs`。`acceptsStdinWait` 结束的 send 视口末行仍可能是 setup 回显，而 scrollback 已经打出真正的提示符。PSReadLine 只含空格的光标行不是提示符。`stdin_read` 不是启动捷径：只凭该原因当就绪会把 setup 残片留在 scrollback，后续 `pwsh` 调用于是返回被截断的第一条命令。剥掉 setup 源码后再 `includes()` 仍会接受 `setup-echo dsh> `；真实提示符还没打出时，每次 follow-up `startSend` 会重置自己的期限，于是 `inferred_idle` 饿死 `timeout`。现在由 spawn 的 `timeoutMs` 约束整个循环。ACP 与 loader 组合把 `idleSilenceMs`（以及会话交接处的 `handoffGraceMs`）加长，避免横幅到提示符的静默抢先结束。[persistent pwsh 笔记](../architecture/2026-08-11-pwsh-persistent-pty.md) 仍拥有双层 prompt 安装。
