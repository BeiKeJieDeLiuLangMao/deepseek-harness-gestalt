...
**把 prompt 函数放进 argv 的 `-NoExit -Command`。** 否决：MOTD 变成 UTF-16/乱码，且没有 `keep=ok`。`-File` 是 UTF-8 脚本，不是 argv 命令文本。
