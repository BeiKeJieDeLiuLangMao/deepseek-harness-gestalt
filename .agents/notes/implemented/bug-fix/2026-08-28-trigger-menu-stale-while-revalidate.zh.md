...
在已打开的 `@`/`/` 触发菜单里，每个按键都会发起一次新的候选请求。菜单 reducer 的 `hit` 分支过去会把各组重置为 pending-空，于是列表在 100–460 ms 的请求往返期间塌缩成骨架屏，每输入一个字符就重绘一次——细化查询时肉眼可见的闪烁（#3234）。
