...
Side Chat 会复用完整 Conversation 内容树，不嵌套应用 chrome，也不显示 child workbench 控件；空白草稿的 composer 保持在底部。主 conversation 不提供可选显示宿主值，继续沿用原有路由。Side Chat 文件链接、行导航、produced-file chip、文件夹显露、编辑器写入与“添加到对话”操作都会处理 child 资源，同时把标签显示在 parent 的可见 workbench 中。每个官方文件地址都会将资源 Session 与工作目录之外的 path 及相对点路径段分开保留。Child 日志保留精确的继承前缀，随后记录 child-owned descriptor。
