...
managed-tree fixture（测试前置数据）将 PID 文档写入同目录的独占暂存文件，完成刷写并关闭后，通过原子重命名发布。可控路径在两段暂存写入之间暂停，并可对较大的写入请求返回仅写入一个字节的结果，用于验证 `tree.json` 在完整文档可读前始终不存在，且发布前会消费每次短写结果。发布标记前的零进展写入用于验证 fixture 会先强制终止并等待后代进程退出，再传播发布失败。
