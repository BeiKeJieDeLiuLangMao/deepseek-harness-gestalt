...
The managed-tree fixture writes its PID document to an exclusive same-directory staging file, flushes and closes it, and publishes it with an atomic rename. The controlled path pauses between two staged segments and can return one written byte for a larger request, verifying that `tree.json` remains absent until a complete document can be read and that every partial-write result is consumed before publication. A zero-progress result before the publication marker verifies that the fixture force-kills and awaits its descendant before propagating the publication failure.
