...
The managed-tree fixture writes its PID document to an exclusive same-directory staging file, flushes and closes it, and publishes it with an atomic rename. The controlled path pauses between two staged segments and can cap each low-level write to one byte, verifying that `tree.json` remains absent until a complete document can be read and that every partial-write result is consumed before publication.
