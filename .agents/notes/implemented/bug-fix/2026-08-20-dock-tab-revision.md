...
`packages/browser/browser-workspace/tests/workspace.spec.ts` pins listed revisions on a two-tab focus and close. `packages/client/ui-browser/tests/browser-dock.client.spec.tsx` and `browser-preview.client.spec.tsx` assert the addressed listing revision. `apps/web/tests/browser-dock.snapshot.ts` remains a single-tab restore. The headless `browser-runtime` and `browser-runtime-tandem` stream-json snapshots now carry per-tab revision on every `browser/workspace` event.
