...
`pnpm run duplication` 报告零克隆。聚焦的归属测试覆盖 `@deepseek-ai/dsh-host-webserver/http`、Platform Account HTTP 信封、Remote Access HTTP 组装路由，以及 Desktop Account／pairing bind。去掉已开票的 `jscpd:ignore` 注释后再跑门禁，仍报告零克隆。
