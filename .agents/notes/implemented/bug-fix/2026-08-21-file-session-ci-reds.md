...
**Put the prompt function on `-NoExit -Command` in argv.** Rejected: UTF-16/binary MOTD and no `keep=ok`. `-File` is a UTF-8 script, not argv command text.
