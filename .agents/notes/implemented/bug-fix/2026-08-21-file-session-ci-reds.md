...
**pwsh `stdin_read` from a stdin wait requires output on that send.** Linux pwsh reports `inputWaiting` at the default prompt before the line runs; treating that empty wait as ready returns a blank viewport and later `keep=` / `cwd=` extracts fail. Disabling the wait entirely starves spawn: OSC+tail never arrives and `inferred_idle` never wins against banner output, so the shell hits `timeoutMs` or exits during startup. A pwsh `acceptsStdinWait`, OSC `stdin_read`, or `inferred_idle` therefore settles only after the active send has output. `inferred_idle` still needs the latest non-whitespace line to equal `CONTROLLED_PROMPT` on the send viewport **or** session scrollback. Setup is submitted once. Substring `includes()` after stripping the setup source still accepted `setup-echo dsh> ` and, when the real prompt had not printed, each follow-up `startSend` reset its own deadline so `inferred_idle` starved `timeout`. The spawn `timeoutMs` now bounds the whole loop. ACP and loader composition keep a longer `idleSilenceMs` (and `handoffGraceMs` where the session hands off) so banner-to-prompt idle does not win the race. The [persistent pwsh note](../architecture/2026-08-11-pwsh-persistent-pty.md) still owns the two-layer prompt install.
