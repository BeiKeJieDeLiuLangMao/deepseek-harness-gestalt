...
**pwsh spawn keeps the official `includes` loop, but the setup source must not contain the installed prompt.** Restoring the official Host string `'dsh> '` inside `PWSH_PROMPT_SETUP` made Linux coverage and the ACP `persistent-pwsh-tool-turn` snapshot red again: the PTY echoes that source before the function runs, spawn returns, and the next write concatenates. Last-line / ready-probe / `-NoExit -Command` checks also failed: Linux often does not reprint a last-line `dsh> `, a probe in the source is the same false-ready, and `-Command` dumped binary into the PTY. The setup therefore concatenates the prompt at runtime (`'dsh' + '> '`, and the tool prompt the same way), then loops empty follow-ups until viewport or scrollback `includes` the installed text. A follow-up that settles on silence without that text is not readiness; `timeoutMs` bounds the wait. Official inferred-idle still has no extra last-line gate. The [persistent pwsh note](../architecture/2026-08-11-pwsh-persistent-pty.md) still owns the two-layer prompt install.
