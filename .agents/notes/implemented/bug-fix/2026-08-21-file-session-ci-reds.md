...
**pwsh spawn is ready on OSC `stdin_read` or a last-line prompt; the tool waits for its own prompt.** `terminal-bash` does not treat a pwsh `inputWaiting` probe as ready: Linux pwsh reports that wait at the default prompt before the command runs, so the send viewport is empty and later `keep=` / `cwd=` extracts fail. `stdin_read` is OSC+tail only. `inferred_idle` still needs the latest non-whitespace line to equal `CONTROLLED_PROMPT` on the send viewport **or** session scrollback, so banner silence does not count. Setup is submitted once: a later empty follow-up viewport used to look like “first send” and dump the prompt-function source again. `tool-pwsh-persistent` `get()` then loops until the viewport ends with `__DSH_PERSISTENT_PWSH_PROMPT__ ` before the first model command. Substring `includes()` after stripping the setup source still accepted `setup-echo dsh> ` and, when the real prompt had not printed, each follow-up `startSend` reset its own deadline so `inferred_idle` starved `timeout`. The spawn `timeoutMs` now bounds the whole loop. ACP and loader composition keep a longer `idleSilenceMs` (and `handoffGraceMs` where the session hands off) so banner-to-prompt idle does not win the race. The [persistent pwsh note](../architecture/2026-08-11-pwsh-persistent-pty.md) still owns the two-layer prompt install.
