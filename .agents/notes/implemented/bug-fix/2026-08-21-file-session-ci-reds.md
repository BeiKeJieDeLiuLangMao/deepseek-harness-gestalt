...
**pwsh readiness is a last-line exact match plus a spawn wall.** `terminal-bash` treats the latest non-whitespace line as installed only when it equals `CONTROLLED_PROMPT`, on the send viewport **or** session scrollback. Setup is submitted once: a later empty follow-up viewport used to look like “first send” and dump the prompt-function source again, so the last line never stayed `dsh> ` and spawn hit `timeoutMs`. A send that settled through `acceptsStdinWait` can still end on setup echo while scrollback already holds the real prompt. A PSReadLine cursor row of spaces only is not the prompt. `stdin_read` is not a startup shortcut: treating that reason alone as ready left leftover setup in scrollback so later `pwsh` calls returned a clipped first command. Substring `includes()` after stripping the setup source still accepted `setup-echo dsh> ` and, when the real prompt had not printed, each follow-up `startSend` reset its own deadline so `inferred_idle` starved `timeout`. The spawn `timeoutMs` now bounds the whole loop. ACP and loader composition keep a longer `idleSilenceMs` (and `handoffGraceMs` where the session hands off) so banner-to-prompt idle does not win the race. The [persistent pwsh note](../architecture/2026-08-11-pwsh-persistent-pty.md) still owns the two-layer prompt install.
