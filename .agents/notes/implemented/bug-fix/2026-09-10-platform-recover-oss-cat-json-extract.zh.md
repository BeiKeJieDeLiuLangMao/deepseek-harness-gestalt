...
`apps/platform/scripts/platform-oss-json.sh` 中的 `platform_extract_json_object` 从第一个 `{` 起取出一份对象，并拒绝随后的 JSON 值。剩余的非 JSON CLI 附言不是权威。该 helper 优先使用 `python3`，其次 `python`，再次 `node`，因为 Windows Git Bash 的 recovery 测试没有 `python3`。`platform-recover.sh` 始终 source 该 helper，并在读取持久字段前把成功的 `oss cat` 标准输出经它过滤。该 helper 不能只放在 `platform-cloud-assistant.sh`：测试会先对 `platform_cloud_run` 打桩，因此 recover 会跳过 source assistant。复制 recover 脚本时必须同时复制它所 source 的同级文件，因为 recover 通过 `BASH_SOURCE` 定位它们。未完成部署探测仍用合并后的标准输出与标准错误检测 `StatusCode=404`；退出码为 0 的探测必须仍能提取一份 JSON 对象，才算存在未完成锁。
...
- chatter 用例会从 PATH 中隐藏 `python3` 和 `jq`，仍通过 `python` 或 `node` 提取。PATH 包装在这些名字不存在时会把 Git Bash 的 `usr/bin` 作为目录保留，并在无法创建 symlink 时复制 keep-list 工具。
...
`apps/platform/tests/production-env.spec.ts` 覆盖前缀 chatter（`0.006891(s) elapsed`）、后缀 chatter（`average: 419(byte/s)`）、在 PATH 中没有 `python3` 或 `jq` 时拒绝额外 JSON，以及不变的 rollbackable / commit-pending / committed / bootstrap 阶段顺序。
