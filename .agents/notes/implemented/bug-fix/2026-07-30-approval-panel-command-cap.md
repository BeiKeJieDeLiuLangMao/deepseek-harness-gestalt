...
- The InputBar's 14-line cap resolves through a custom property inherited from `.composerSeat` and applied to [`.scroll`, the composer's only scrolling box](../../../../packages/client/ui-conversation/src/client/skeleton/InputBar.module.css). Rendering the bar outside that seat would drop the declaration (an unresolved `var()` with no fallback), so a future composer host has to carry the property — which is why it is declared on the shared seat rather than the app root.
