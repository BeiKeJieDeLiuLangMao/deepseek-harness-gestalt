...
实现本决策时，composer 把光标与选区留在透明的原生 textarea 中，由 backdrop 绘制可见字形，并由隐藏的镜像层决定完整草稿高度。当时的[单滚动容器决策现已归档](../../archived/bug-fix/2026-07-31-composer-text-layers-share-one-scrollport.md)；它要求 textarea 不持有可滚动溢出：每次草稿提交后，`scrollHeight` 与 `clientHeight` 相等，`scrollTop` 为零。
