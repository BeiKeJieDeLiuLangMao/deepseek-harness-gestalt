...
`packages/client/ui-browser/tests/browser-page-chrome.client.spec.tsx` and `use-browser-page.client.spec.tsx` fail when chrome stays `about:blank` after a `status: open` navigate whose listing revision advanced. `packages/browser/browser-runtime-deterministic/tests/runtime.spec.ts` verifies that navigate commits the returned URL and title to the next observe. `apps/web/tests/browser-dock.snapshot.ts` pins expanded Dock chrome after open and Refresh against `about:blank`.
