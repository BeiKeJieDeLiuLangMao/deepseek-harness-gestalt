...
包内测试针对四种联接状态钉住 `providerUsable`，并针对新门槛与每一个存留的诊断钉住 `onboardingReadiness`；分区测试覆盖首次运行姿态（含从未写入的官方用户层）、删除后隐藏残留官方行、另一提供方已可用时隐藏空闲官方行、普通行姿态，以及在新增卡保住草稿的同时折叠设置卡片的那次取消。`onboarding-usable-provider` web e2e 泳道通过真实协议重放整个场景：两张卡片都开着时取消、改配 `minimax-cn`、重载，然后不再出现接管和官方设置卡片——并附一份关闭后状态的 aria golden。
