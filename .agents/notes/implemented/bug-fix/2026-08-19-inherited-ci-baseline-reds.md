...
**Clearing the shallow-checkout red surfaces the companion baseline's remaining gates.** The Linux coverage lane now finishes its tests and fails the per-file 100% inventory on Gestalt client, schedule, remote-access, and search files that never had a covering lane; those files join the existing `TODO(gui)` coverage exclude list, and `revokeCredential` plus the Desktop Account source gain owning tests instead of an exclude. Consumers-lane oxlint and jscpd failures are the same companion surface: deprecated `escape`/`unescape`, unbound input-action methods, untyped `ctx.get('web')`, and cloned snapshot/HTTP/tab/field blocks. The `pwsh-tool-turn` header sidecar is refreshed to the current sandbox sentence, `job_*` "job" wording, and job-then-pwsh tool order the Linux composition emits. Schedule-board aria goldens consume AM/PM inside `{{clock}}` so UTC runners do not diverge from a UTC+8 recording; workspace-management re-hovers and force-clicks the hover-only row action.
