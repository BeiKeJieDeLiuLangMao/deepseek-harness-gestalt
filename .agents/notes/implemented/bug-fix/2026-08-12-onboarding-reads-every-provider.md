...
`needsSetup(row, anyUsable)` takes the same fact, so the setup card is the first-run posture alone. The Models list includes the official empty-path row while that posture holds, or while `dismissedSetup` keeps it as a row for the rest of the session, even when the join marks the user section vacant. With another provider reachable, a vacant official route stays off the list; an occupied official route is an ordinary row carrying the missing-key dot, one Edit click from the same card.
...
Package tests pin `providerUsable` over the four join states and `onboardingReadiness` over both the new gate and every surviving diagnostic; the section tests cover the first-run posture, including a vacant official user section, the hidden vacant official row once another provider is usable, the plain-row posture, and the cancel that collapses the setup card while the add card keeps its draft. The `onboarding-usable-provider` web e2e lane replays the whole scenario through the real wire: cancel with both cards open, configure `minimax-cn` instead, reload, and find no takeover and no official setup card — with one aria golden of the dismissed state.
