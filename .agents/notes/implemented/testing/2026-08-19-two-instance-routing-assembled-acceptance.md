...
`pnpm exec vitest run packages/platform/remote-access-http/tests/two-instance-assembled.spec.ts` — one assembled case against two in-process Loader compositions over loopback TLS, with in-memory pairing-authority and route-store adapters plus a test Redis coordinator. The existing keyless example snapshot and package unit suites remain the lower-level coverage.
