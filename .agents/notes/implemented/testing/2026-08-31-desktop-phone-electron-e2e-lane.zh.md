...
运行器在启动前强制重建所有被消费的 Host、client、web 与 Electron main 产物。Electron e2e TypeScript 源码使用专用 Desktop compiler face，所属 package 与仓库 typecheck 命令都会执行该编译面。运行器对 Electron Service 的 release metadata 请求设置上限，使 metadata host 不可用时能进入该 service 捆绑的版本映射回退。它移除环境中的凭证与 Platform Relay 变量，提供 keyless 回环模型端点，并且只把验收产物写入 gitignored 的 `.artifacts/e2e-electron/`。手机 Electron 运行器会写入一份带 `windowPresentation: 'hidden'` 的私有源码专用 profile，设置 `DSH_DESKTOP_E2E=1`，并传入 `--dsh-e2e-profile=`，使产品 `BrowserWindow` 以 `show: false` 构造，且 activate/second-instance 不会 restore 或 focus；`CI=true` 不选择呈现方式，打包或未启用的 profile 会被拒绝，省略 profile 时仍保持可见。从隐藏 renderer 捕获的截图仍是有效证据。预构建隐藏验收还会让 Electron 与 ChromeDriver detach，在收集证据后删除仍存活的 WebDriver session、清除 runner 持有的 session id，再通过 Electron Service 独立的主进程 CDP bridge 请求 `app.quit()`。产品干净退出要求已就绪 Host 记录 `code=0 signal=null requestedStop=abort`，其后出现 `shutdown complete`；删除与退出请求任一失败仍属致命错误。该呈现 seam 不是原生进程树所有权或 Host 退出出处；[有界 Host 世代 fixture 所有权策略](2026-09-05-bounded-host-generation-fixture-ownership.zh.md)仍只是 fake 基础，不授权真实 fixture 执行。源码专用 hidden-window smoke 只持有一个精确的 Electron `ChildProcess`。spawn 与 exit/stdio 观察器在返回 owner 对象之前安装；`readResult`、deadline 等待与 kill 在之后的 microtask 中运行。fake-adapter 测试不执行 Electron。每条直接子进程退出路径都只证明进程本身；renderer/GPU 树清理始终 unverified。该 owner 从不删除隔离的 userData 根目录。直接子进程 exit 0 不是验收通过：owner 把进程事实与 `acceptance` 分开报告，`assertHiddenWindowSmokeAcceptance` 拒绝非零/带信号/`forced`/`unverified` 进程事实或未接受的 payload，`runHiddenWindowSmoke` 是注入 owner 再做 verdict 的调用方（Electron spawn 仍需显式启用且未执行）。调用方必须对结果文件调用 `parseHiddenWindowSmokeResult`，仅在 `visible: false`、`shownOnCreate: false`、`activate: 'handled'`、`captureEmpty: false` 且捕获宽高为正有限整数时接受，或记录显式 `{ failed: true, message }`。deadline 放弃后，分离的 child-error 观察器保留到 `exit`/`close`，最多写入 8 条 late diagnostics；原始 result Promise 在调用外部 result deadline 之前就被观察，expiry 或 throw 之后的拒绝只记一次 late，不重复为 prompt。smoke 脚本在捕获为空、尺寸非正、或捕获后窗口可见时，不会写入成功结果。Electron 若加载，须先打包为 `out/hidden-window-smoke.mjs`；该 smoke 需显式启用，不启动 Host 或 mobilecli，也不是 WDIO `isVisible` 验收。POSIX 命令以 detached 方式启动，但当前 detached group 处理只是 best-effort fixture 清理，不是严格所有权或强隔离。暂存的 mobilecli launcher 是 POSIX fixture，因此该通道在 Windows 上快速失败；Windows 资产选择与可执行文件命名由独立 package 测试负责。命令只有在 stdio 关闭且串行日志 writer 刷新完成后，才允许检查构建结果或审计日志。运行器会记录适用的 Electron、Host 与 fake 标识，把清理结果写入 `cleanup.json`，并聚合报告清理错误。Electron main/renderer 错误行与 Desktop smoke 错误会让通道失败，并写入 `log-audit.json`。
...
该通道无需密钥，并且会启用源码专用的隐藏 BrowserWindow，但该呈现 seam 不是 Issue #572 所要求的真实设备、同一 Host 恢复或原生隔离证据，也不替代单元测试。SIGKILL 之后等待直接子进程退出不能证明进程树已清理。smoke owner 不会删除 userData。证据分别标明 Host 启动、传输字节、解码画面可见性、输入转发与 best-effort 清理。仓库 Electron runtime 冒烟通过不能替代本通道；本通道通过也不授权产品发布或合入 `master`。
