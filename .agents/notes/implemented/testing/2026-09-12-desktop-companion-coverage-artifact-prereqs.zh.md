...
3. 在 `apps/desktop/tests/packaged-main-bundle.spec.ts` 中，通过 `beforeAll` 显式声明 `generateDesktopHostTypertArtifacts()` 局部前置生成，使测试能够在干净树上自足打包 `main.mjs`，不再依赖其他测试的执行顺序。
4. 在 `apps/desktop/tests/host-rpc-assembled.spec.ts` 中，将 `companion-product.ts` 的加载推迟到 `beforeAll` 中 `generateDesktopHostTypertArtifacts()` 执行之后动态导入，匹配 `companion-host-business-error.assembled.spec.ts` 中的安全模式。
...
- 在无预建 `lib/` 产物的干净检出树上，`packaged-main-bundle.spec.ts` 自主生成所需 Typert remote 产物并将 `@deepseek-ai/dsh-client-ui-desktop/protocol` 解析到源码，实现独立测试通过。
