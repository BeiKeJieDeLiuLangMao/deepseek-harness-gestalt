# Agent Note: Keyless Host expected output for Android capture-source IO

Status: implemented

English | [中文](2026-09-05-android-capture-wire-expected.zh.md)
...
**Broad snapshot normalizers for ports, tokens, and protocol messages.** Rejected: only mint identity fields are projected; JSON-RPC error text and mapped coordinates stay literal so a missing-bounds failure cannot match the aspect-mismatch expected output.
