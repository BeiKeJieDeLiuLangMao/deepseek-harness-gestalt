...
The consumer job owns the [single Linux build](../process/2026-07-30-independent-ci-consumer-build.md), so `apps/web/dist` and the package `lib/` directories remain in its workspace for the browser suite. On hosted runners, CI installs Chromium and its system dependencies at the Playwright version in the lockfile. On the persistent failover VM, the image owns the Linux system packages and CI installs only Chromium, avoiding per-run `apt` mutation. Pull requests opportunistically restore an operating-system-and-lockfile-keyed browser cache without paying compression and upload on the required path, with an operating-system prefix fallback across lockfile changes; a miss proceeds through the normal installation. The hosted default-branch Linux serial definition that could refresh this cache is disabled. The self-hosted standby runs the same comparison without hosted cache actions only when its optional runner is available.
...
For pull requests, the gate runs only in the Linux consumer job: these scenarios target POSIX, and the other PR jobs do not provision Chromium. The hosted default-branch Linux serial aggregate is disabled. The self-hosted default-branch aggregate includes the comparison only when its optional runner is available, while the macOS and Windows serial definitions remain browser-free. A PR's `all checks passed` verdict already depends on the consumer job, so a browser compare failure blocks the merge without requiring a new branch-protection check name.
...
Before merge, every PR proves that the current web assembly matches all committed browser expected outputs, turning a missed refresh from an “unrelated change in a later PR” into a failure in the PR that introduced it. The cost is Chromium provisioning and one serial pass through the browser scenarios in the consumer job; the consumer-owned build avoids a duplicate build, and an opportunistic browser-cache hit can avoid a download. The gate does not depend on that hit, still makes no claim of cross-platform browser consistency, and requires a Playwright/Chromium upgrade that changes the ARIA format to refresh and review the expected outputs explicitly.
