...
The root build completes Host tsc and Host tsdown first, with Host tsdown running Typert and generating the Remote Client contract. It then completes Client tsc, the Desktop application check, Client tsdown, and the Web build:
...
`build:lib:host` owns the first three steps, `build:lib:client` owns the next four, and `build:web` runs last. `typecheck` must also run the complete Host lib phase first because Client tsc, the Desktop application check, and cross-face integration tests require declarations generated by Host tsdown; it does not need Client tsdown or the Web build.

Each aggregate tsc phase emits JavaScript, declarations, and incremental state to `lib/types`. Tsdown reads only that JavaScript and produces published bundles; it neither reads source nor emits declarations. The Host contracts-ready pass is a no-emit TypeScript program over selected tests.

Host tests that import generated `/remote` declarations use the `.generated.host.spec.ts` suffix. The first Host tsc excludes that exact pattern. After Host tsdown emits the declarations, the contracts-ready pass selects the same pattern and reuses `tsconfig.host.json` compiler options and Project References without copying the graph. An empty selection, a generated Remote consumer without the suffix, a missing declaration, or a TypeScript diagnostic fails the Host lib build.

`apps/desktop/tsconfig.json` checks the complete Desktop `src`, `tests`, and `scripts` application after Client tsc. Desktop consumes Host and Client projects, so it belongs to neither root aggregate and emits no artifacts. A separate no-emit program checks the exact `POST_GENERATION_CROSS_FACE_TESTS` integration-test inventory. Its five tests remain with their Platform package owners, use only those tests as roots, and reuse the Desktop, Host, and Client configs' direct Project References without flattening aggregate file lists. Workspace constraints require every test to be an exact file and exact Host exclusion, reject duplicate or stale entries, require the complete Desktop config, and fix the public script order.
...
A clean build is the authoritative check of ordering correctness: with no existing `/remote` artifacts, the first Host tsc must succeed without generated-contract consumers, Host tsdown must generate the contract, the Host contracts-ready pass must accept those consumers, and then Client tsc, the Desktop application check, cross-face integration check, Client tsdown, and Web build must succeed. No phase may write artifacts into `src`.
...
An independent Client build is no longer a self-contained entry on a clean worktree; repository commands, CI, and release flows must run the Host lib phase first. After that phase, `build:lib:client` checks the Client aggregate, complete Desktop application, and exact cross-face integration tests before bundling. Developers of ordinary packages do not need to understand or copy this exception and continue to choose one aggregate according to the package's runtime environment.
