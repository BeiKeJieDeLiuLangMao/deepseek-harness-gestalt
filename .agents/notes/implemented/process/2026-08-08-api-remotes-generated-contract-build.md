...
`build:lib:host` owns the first three steps, `build:lib:client` owns the next three, and `build:web` runs last. `typecheck` must also run the complete Host lib phase first because Client tsc and the Desktop application check require declarations generated by Host tsdown; it does not need Client tsdown or the Web build.
...
`apps/desktop/tsconfig.json` checks the complete Desktop `src`, `tests`, and `scripts` application after Client tsc. Desktop consumes Host and Client projects, so it belongs to neither root aggregate and emits no artifacts. The workspace constraints gate owns the post-generation application inventory, requires the no-emit config and complete inputs, rejects a root aggregate reference, and fixes the public script order.
