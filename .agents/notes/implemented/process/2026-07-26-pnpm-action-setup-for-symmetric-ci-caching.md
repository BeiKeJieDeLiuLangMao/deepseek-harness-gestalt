...
- **Restore-only cache consumption** (hand-rolled `actions/cache` steps): the three required Linux pull-request jobs and the Wine-based required Windows job restore matching archives without saving, keeping cache compression/upload off their latency-sensitive paths — an asymmetry `setup-node`'s cache cannot express. Each configures a store outside the action's replaceable install directory and resolves that path; the Linux jobs skip restore on their optional self-hosted route because a provisioned persistent runner owns its local store.
...
- **Convert the required jobs' caching to `cache: pnpm`.** Rejected: the restore-only asymmetry is a documented latency decision in `ci.yml` comments; erasing it to unify tooling inverts the priority.
- **Save pull-request caches.** Rejected because compression and upload would enter the required path. These jobs consume a compatible archive when one exists and otherwise perform the immutable install normally.
