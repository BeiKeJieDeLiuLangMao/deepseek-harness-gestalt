...
每个就绪票仍然只有一个 writer、一条 `codex/<issue>-<slug>` 分支和一个隔离 worktree。隔离 ticket writer 从指定的 test/script 对开始，并在任何 skill 或文档巡读之前先写 RED。Writer 遵循 [`implement`](../../../skills/implement/SKILL.md) 和[推送前检查](../../../skills/dsh-pre-push-checks/SKILL.md)。他们不开 pull request。
...
[先前的默认编排注记](2026-08-16-default-ticket-delivery-orchestration.zh.md)仍然拥有请求权限、隔离 writer、作为持久状态的 GitHub、GUI 证据、清理证明和发布停点。交付完成还要求对该已 MERGED 的 `Closes` pull request 执行该退役步骤。本注记拥有 pull request 数量、谁负责合并、探索笔记存放位置，以及 retro 闸门。
