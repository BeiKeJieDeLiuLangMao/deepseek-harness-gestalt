...
Fidelity comparison and the acceptance walk each add a dedicated Codex validation responsibility, but stable owner sessions may carry repeated fixes without multiplying sessions. Both validations use the verified macOS background path. A missing draft, route, computer-use path, or completed step stops delivery instead of asking the user to debug the environment or changing the evidence scope automatically.
