...
实测的大规格 runner 尺寸仍作为手动基准测试清单。[公开仓库 runner 决策](2026-08-18-public-repository-ci-runner-defaults.md)已经取代了将其作为普通拉取请求默认值的做法：必需的 Linux 作业使用 `ubuntu-latest`，原生 Windows 使用 `windows-2025`。只有操作方另行配置对应标签后，`suite=larger-runner-benchmark` 才会比较相互独立的关键通道；`suite=consolidated-runner-benchmark` 在同一限制下比较完整聚合流程。每项基准测试都会先报告实测的处理器和内存容量，再运行仓库工作。

拉取请求的必需正确性检查不再依赖组织 runner 分配。[可移植必需 CI 决策](2026-07-23-portable-required-pull-request-ci.md)拥有聚合流程成员关系，[串行参考流程](2026-07-21-serial-cross-platform-ci-reference.md)拥有完整且未分片的定义。以下基准测量用于指导调度器与容量选择，但不会让其标签进入分支保护。

原有的门禁级和粗粒度主流程分片作业已从工作流中移除。相应的静态、lint、覆盖率、快照和场景分片选择器也已从仓库中移除，因此未使用的诊断路径无法继续维系第二套 CI 架构。

Linux 主流程使用 3 个相互独立的标准托管作业。覆盖率单独运行，并设有自己的工作线程上限；静态调度器负责不消费生成输出的源码和文档检查。第三个作业负责唯一一次 Linux 构建，随后让 lint、Node 24 运行时兼容性、依赖构建产物的快照、文档类型检查和所有产物消费方基于该目录树启动。这种[消费方独立构建](2026-07-30-independent-ci-consumer-build.md)使 3 个作业都能立即请求 runner，而无需重复编译或传输仅供本次运行使用的产物。生成的 NodeNext 消费方目录不会纳入 Oxlint 的文件发现范围，因为这些进程重叠执行时，产物检查会删除这些目录。pnpm store 会得到恢复，但缓存上传不会进入拉取请求关键路径；Oxlint 没有由仓库管理的结果缓存。性能报告采用每个作业从 `startedAt` 到 `completedAt` 的区间；runner 排队延迟是容量证据，而非仓库执行时间。
...
[拉取请求双 Windows 拓扑](2026-08-08-native-windows-pull-request-ci.md)将必需的 Wine 信号与独立且不阻断的标准托管原生作业分开；后者会独立强制执行受支持源码覆盖率。重复的 lint、覆盖率和快照清单由 Linux 负责。
...
**再次让大规格 runner 标签成为必需项。**其测量结果可以缩短活动耗时，但不可用标签会使正确性检查持续排队。因此，个人公开仓库接受较慢的标准托管执行，并让大规格 runner 比较保持手动触发。
...
必需拓扑保留 3 个相互独立的 Linux 作业且不保留分片选择器，但把标准托管路线的 fan-out 上限设为 2。只有在重新测量有价值且标签已经配置时，手动基准测试才会加入更大规格。

完整作业测量能同时呈现计费时长与工作流复杂度。拆分 Linux 会重复两轮设置，但消费方通道拥有唯一一份已构建目录树，且覆盖率、静态检查与构建后消费方分别进入 runner 分配。

性能目标是观测结果，而非取消截止时间或正确性要求。当映像、依赖、调度器或定价发生变化而需要重新测量时，仍可使用手动全规格和串行套件。

大规格 runner 标签缺失或改名时，只会影响手动基准测试。可选自托管标签缺失时，其热备或故障切换路线可能排队，但不会影响标准托管的拉取请求默认路线。
