...
The measured larger-runner sizes remain manual benchmark inventory. The [public-repository runner decision](2026-08-18-public-repository-ci-runner-defaults.md) supersedes their use as ordinary pull-request defaults: the required Linux jobs use `ubuntu-latest`, and native Windows uses `windows-2025`. `suite=larger-runner-benchmark` compares isolated critical lanes only when an operator separately provisions its labels, while `suite=consolidated-runner-benchmark` compares whole aggregates under the same limitation. Each benchmark reports observed processor and memory capacity before running repository work.

Required pull-request correctness no longer depends on organization runner allocation. The [portable required-CI decision](2026-07-23-portable-required-pull-request-ci.md) owns the aggregate membership, and the [serial reference](2026-07-21-serial-cross-platform-ci-reference.md) owns complete unsharded definitions. The benchmark measurements below guide scheduler and capacity choices without making their labels part of branch protection.
...
The [dual Windows pull-request topology](2026-08-08-native-windows-pull-request-ci.md) keeps the required Wine signal separate from a non-blocking standard-hosted native job that independently enforces supported-source coverage. Linux owns the duplicate lint, coverage, and snapshot inventories.
...
**Make larger-runner labels required again.** Their measurements can reduce active time, but unavailable labels leave correctness jobs queued. The personal public repository therefore accepts slower standard-hosted execution and keeps larger-runner comparisons manual.
...
The required topology retains three independent Linux jobs and no shard selectors, but bounds their standard-hosted fan-out at two. Manual benchmarks add larger sizes only when remeasurement is useful and the labels have been provisioned.

Complete-job measurement exposes both billed time and workflow complexity. Splitting Linux repeats setup twice, but the consumer lane owns the only built tree and coverage, static checks, and post-build consumers enter runner allocation independently.
...
Missing or renamed larger-runner labels affect only manual benchmarks. Missing optional self-hosted labels can queue their standby or failover routes, but they do not affect the standard-hosted pull-request default.
