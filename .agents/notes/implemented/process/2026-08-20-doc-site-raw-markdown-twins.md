...
Each documentation build owns its complete resolved `outDir` as disposable output. `buildDocumentationSite` fixes the build entry to `website/`, and `docSiteBuildOptions` accepts only an output below the site root supplied by that entry. The callback removes that output after configuration loads and before the current bundle or `public/` files are written; a link-shaped output path is unlinked without traversing its target, and an output whose existing parent resolves outside the site root is rejected. Raw projection does not remove files at `buildEnd`, so its collision check sees every file produced by the current build and still rejects a public file, page, or image at the same path.
...
Agents fetch any page as plain Markdown by dropping the URL's trailing slash and appending `.md`, and discover the whole set at `/llms.txt`; the rendered site is unchanged. Consecutive builds replace the complete prior output without treating their own twins as another producer. The build output carries one extra Markdown file per route, an alias per index route, and image copies beside them — kilobytes against the bundled assets. Twin content keeps GitHub-style heading text while the rendered site slugs punctuation-heavy headings differently; agents resolve headings themselves, so no gate covers raw-tree fragments. Deferred as unneeded for the twins' audience: `llms-full.txt`, and a per-page "view as Markdown" control, which would require a theme directory the stock-theme site deliberately lacks.
