...
每个被选中的穷尽 plan 还会启动 3 个相互独立的原生 Windows 分区，分别负责构建与 runtime、完整受支持源码覆盖率，以及静态可移植性。`windows node 24 / native verdict` 要求 3 个分区全部成功。分区默认使用 `windows-latest`；`DSH_CI_FAILOVER_WINDOWS=selfhosted` 会把可信 PR 重定向到 `dsh-win-ci`。[标准托管主路径决策](2026-08-18-standard-hosted-primary-ci.zh.md)负责该选择器，[并行原生证据决策](2026-08-24-parallel-native-windows-evidence.zh.md)负责分区清单与并发。

原生 verdict 被刻意排除在 `all-checks-passed.needs` 之外，且不使用 `continue-on-error`：必需 aggregate 既不等待它，也不会因它改变结论；每个原生分区及其 verdict 都保留未被掩盖的结果。任一分区失败、取消或跳过都会使原生 verdict 失败。Linux 与 Wine 仍负责 branch protection 结果。

覆盖率分区保留 8 个隔离的单 worker shard，并在标准托管容量上并发运行 4 个 shard 进程，在故障切换池上并发运行 8 个。每个 Vitest project 都使用 fork worker，因为 Node 24 的 CJS lexer 致命故障可在 Windows 与 POSIX 的共享 worker 中复现。两项覆盖率门禁都将 Vitest 默认的单测试和轮询预算设为 30 秒，因为在完整 Windows 插桩下，进程、Git、SQLite、watcher、语法和静态门禁 fixture 可能超过 15 秒。只导入 `scripts/` 源码和子进程的 translation-pairing 合并套件仍留在豁免重型门禁。LSP 源码与 ACL sandbox 源码仍计入 Windows 分母，只有此前已排除的 spawned runner 入口保持排除。
...
尽管如此，每个穷尽 PR 都会获得真实 NT 内核、NTFS、PowerShell、Windows 进程、原生 addon 和受支持源码覆盖率信号。3 个分区会重复不可变 setup，使各自的证据时钟能够重叠；测试、阈值、构建产物检查和可移植性门禁均未删除。
