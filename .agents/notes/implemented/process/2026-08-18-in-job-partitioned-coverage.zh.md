...
`DSH_COVERAGE_MAX_WORKERS` 继续控制无插桩豁免门禁和普通非分区路径的规模，不会调整分区子进程。标准托管 Windows 会串行执行顶层门禁，并把分区并发设为 1，因此 8 个相互隔离的 shard 会依次执行。16 核 Windows 故障切换池保留 8 路分区扇出。标准托管 Linux 会把 4 个固定分区限制为最多 2 个活跃进程，64 核故障切换池则保留 4 路扇出；每个插桩进程仍只有 1 个 worker。
...
Linux 与 Windows 使用相同的协调器，并各自设置分区数量、并发上限与外围 worker 预算。本地覆盖率默认保持简单；只有调用方显式选择分区包脚本并提供大于 1 的合法数量时，才启用分区。
