...
The root coordinator selects writers from the active runtime and does not implement. [Root-session orchestration](2026-09-03-root-session-orchestrates-only.md) owns that split. Codex uses isolated Codex worktree tasks when they are available. When the Codex task API is unavailable but worktree isolation remains available, the root dispatches writers sequentially into one dedicated worktree at a time through the Agent tool. DSH uses `subagent_fork` when the writer benefits from the current conversation and plain `subagent` when inheritance adds nothing. A UI prototype is not drawn in the coordinating session: Codex gets an independent worktree task with a short brief; DSH gets a subagent after that brief exists. [Fused UI prototype variants](2026-09-02-fused-ui-prototype-variants.md) own the UI draft rules. A structural scheme follows [the human review pack](2026-09-02-human-scheme-review-pack.md). Model selection remains a per-ticket root decision.

On the normal path, every writer receives one ticket, branch, and isolated worktree. The writer does not open a pull request; a merger subagent integrates the ticket branch into the specification branch as [the spec-PR decision](2026-09-02-spec-pr-delivery-and-retro.md) requires. The loss of worktree isolation or of an Agent tool is a reported blocker; the root does not write in the coordinating session. A product-shaping prototype lives on its own pushed branch and supplies planning input that implementation tickets adapt rather than merge verbatim. Cleanup verifies terminal state through the executor that actually ran; it archives a Codex task only when one was created and never removes a shared checkout. Exact dedicated-worktree and branch validation remains common cleanup behavior. A missing dedicated worktree after the MERGED `Closes` pull request is already retired.
...
- Writer isolation and durable branch evidence remain invariant across runtimes on the normal path; lost isolation or a missing Agent tool stops delivery instead of writing in the root session.
