...
The [pre-push workflow](../../../skills/dsh-pre-push-checks/SKILL.md) selects outgoing evidence. GUI smoke and browser work follow the [Desktop test-instance and runtime-memo decision](2026-09-02-desktop-test-instance-and-runtime-memo.md). Writers do not open pull requests. After assembled evidence and the [retro gate](2026-09-02-spec-pr-delivery-and-retro.md) pass, the specification pull request enters `master` and closes the tickets. Delivery completes only after the root then retires that work: it deletes the merged ticket and specification branches, removes remaining exact dedicated worktrees, and runs `git worktree prune`. A missing dedicated worktree after that MERGED `Closes` pull request is already retired. Preserve a remaining worktree and report it when unique unmerged work is still there. Tag creation, GitHub Releases, publication, signing, notarization, and deployment always require explicit per-release approval.
