...
The [`record-browser-gif`](../../../skills/record-browser-gif/SKILL.md) workflow treats one storyboard as one evidence chain pinned to an exact pull-request commit. Recording begins only after both a complete non-recording acceptance flow passes and Standards and Spec review have no code findings; the workflow does not impose an order between those prerequisites. A code change before recording requires affected checks and review to run again before recording the new head. After publication, the delivery ledger may retain a GIF only when the later change cannot affect its behavior, route steps, or environment; the GIF keeps its original commit provenance and never represents the newer head. An affected storyboard is re-recorded as one run, and uncertain impact broadens revalidation. The DSH Web acceptance supervisor requires a clean committed worktree, builds or verifies that exact commit, starts isolated state, and exposes the expected visible revision; the browser verifies that revision before the first real-model call or captured frame. Each run uses fresh `DSH_HOME`, `DSH_AGENTS_HOME`, workspace, session, and isolated browser state, and every published frame comes from the same server and model-backed scenario run. When a fresh browser context is unavailable, the exact origin's cookies and site storage are cleared before navigation. Existing user browser state is used only when requested or required, is stated next to the GIF, and does not substantiate fresh client state. A failed capture run is discarded and repeated from fresh roots rather than combined with another run.
...
The available browser-control workflow remains preferred. When it is unavailable, the recorder uses the repository-declared Playwright dependency in an isolated headless browser rather than installing another driver or opening the user's browser. A native production surface may be replaced only through normal application configuration with an official browser-operable production backend, and that override is stated next to the GIF. This records narrower browser evidence and does not replace required Codex computer-use acceptance of the native Electron route. Fixtures, mock transports, synthetic events, and test-only hooks do not substantiate a real-production claim.

Publication verifies the boundary again. The [unified `gif-assets` branch](2026-09-02-unified-gif-assets-branch.md) contains media only, the staged and published bytes match the verified artifact, and a private-repository asset is checked through authenticated API or raw requests for its path, byte size, checksum, response status, and media type. This proves the repository-member review path only; the [documentation-site image decision](2026-08-06-doc-site-carries-its-images.md) owns why a public site cannot depend on a private raw URL. Immediately before the first pull-request embed, the live head must still equal the recorded commit. After that edit, the live head is checked again and must remain at that recorded value. A later retained embed states the original commit and its unchanged evidence scope; GitHub's Markdown renderer separately must produce the expected image.
...
**Trust a successful `gif-assets` push or an anonymous fetch.** A push proves only that git accepted bytes, while private repositories intentionally reject unauthenticated raw requests. Authenticated byte verification plus GitHub Markdown rendering tests the two publication boundaries that reviewers use.
