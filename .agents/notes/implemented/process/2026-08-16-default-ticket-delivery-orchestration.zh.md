...
根任务是协调者和监控者。它分析、拆解、派发和验收；它不实现。[根会话只做编排](2026-09-03-root-session-orchestrates-only.zh.md)拥有该拆分。[一张规格 pull request](2026-09-02-spec-pr-delivery-and-retro.zh.md) 才是合入 `master` 的路径；merger 子代理集成 ticket 分支。GitHub Issue、该 pull request、检查和远端 head 是持久状态。每个就绪 ticket 只有一个可写负责人和一个 worktree；相互独立的 ticket 可以并行。读密集型探索和评审可以使用 subagent，而后续指令和跨 ticket 发现通过根任务作为 brief 交给 writer，并记录到 GitHub 或所属仓库文档中。根任务监控所有独立任务，并在协调任务中呈现结构化人工阻塞，因此该流程既不依赖同级 agent 通信，也不要求用户自行发现阻塞任务。
