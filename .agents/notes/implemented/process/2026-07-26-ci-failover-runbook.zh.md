...
3. 切换到此完成。Linux 故障切换状态下，工作流还会把 `DSH_COVERAGE_MAX_WORKERS` 从 2 提高到 8、把 `DSH_SNAPSHOT_MAX_CONCURRENCY` 从 8 提高到 12、提高其他有界工作进程设置，并跳过托管路径的缓存恢复，因为虚拟机的持久 store 会直接提供热安装。Windows 故障切换状态下，原生作业的覆盖率工作进程会从 1 提高到 2，publint 工作进程会从 1 提高到 8。
