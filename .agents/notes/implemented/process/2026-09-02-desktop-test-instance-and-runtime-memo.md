...
[`dsh-desktop-test-instance`](../../../skills/dsh-desktop-test-instance/SKILL.md) owns agent-started Desktop test Electron lifecycle. One user goal has one instance and one active desktop-input owner. The agent stops the recorded instance, then any other live test processes for that goal, before a replacement starts. Native product GUI checks run in a Codex computer-use session against the actual isolated Electron route. They prefer [macOS background computer use](https://learn.chatgpt.com/docs/computer-use#when-to-use-computer-use), then an existing headless or virtual-display route, then headed foreground control; [Windows computer use remains foreground](https://learn.chatgpt.com/docs/computer-use#windows-foreground-use). Each selected mode must prove exact-application screenshot, user-level input, and the resulting UI before acceptance, and background mode must also preserve the user's foreground application. Headless rendering, process existence, direct DOM or Electron IPC scripting, and a standalone Web surface do not prove the native route. A missing bridge marks that mode unavailable for the scenario; building one requires accepted infrastructure scope. Human review starts only after the agent completes the route. The agent chooses the operated Platform config from the scenario: `apps/desktop/tests/fixtures/operated-platform.json` unless the run must talk to live Platform or the diff changes Platform identity, callback, Relay, or companion-attachment fields. A keyless probe copies no model configuration; an explicitly authorized model-calling instance blind-copies only `settings.yaml` and `.credentials.yaml` from the normal DSH Home through `copyModelConfiguration` in `scripts/web-acceptance.ts` and does not invent provider models.

`.agents/local/runtime-memo.json` is gitignored checkout-local inventory. Desktop records the exact application path, revision, mode, input owner, PIDs, ports, and scratch paths there; [`ego-browser`](../../../skills/ego-browser/SKILL.md) records the live DSH task-space id in the same file and reuses that id until the user asks for a fresh space, the goal changes, or the recorded space is gone. Creating a second DSH space while the first still exists fails the round.
...
**Require headless mode without proving computer-use access.** Electron can render offscreen or under a virtual display without exposing a screenshot-input-result loop to the active Codex integration. Mode selection follows demonstrated control rather than the launch flag.
...
**Start every agent check in the foreground.** Foreground control covers native UI but unnecessarily interrupts the user when background or proven headless control can exercise the same route.
...
Agents can replace a test Electron without leaving a second window, Host, or database on the machine, and browser work stays in one DSH space for the goal. Native acceptance prefers background control and admits headless or headed execution only with recorded capability evidence and fallback reasons. Model calls from those instances use the installed provider catalog instead of a fixture default.

The memo is local and can go stale; a missing process deletes its record rather than blocking the next start. Human review waits for the agent's complete route in the selected computer-use mode. Automated Electron lanes remain responsible for their own survivors and remain necessary CI evidence. A live-Platform run still needs Environment fields for `write-operated-platform-config.mjs`; the fixture cannot stand in for that identity.
