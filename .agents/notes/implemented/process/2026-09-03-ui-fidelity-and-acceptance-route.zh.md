...
然后派发**带 computer use 的专用 Codex 验收环境会话**。该会话不是根会话，也不是票 writer。它停掉该目标的残留实例，通过同一后台路径启动一个全新的隔离 Desktop，按场景选择 fixture 或 live Platform，种好路线需要的数据，并通过用户级输入走完实际 Electron 路线。它记录已验证的应用，并在重建、诊断、重新走查和清理期间保持为唯一桌面输入 owner。卡住的步骤是 writer 修复或报告的人工阻塞；该会话不切换驱动或执行模式。只有智能体完整走通才开始用户交接。该会话随后把路线、应用路径以及起始状态交给用户。
...
还原度对照和验收走法各自增加一项专用 Codex 验证职责，但稳定的 owner 会话可以承接多轮修复，不必持续增加会话。两项验证都使用已验证的 macOS 后台路径。缺少稿、路线、computer-use 路径或已完成步骤会停止交付，而不是让用户去调试环境或自动更改证据范围。
