...
- **只恢复缓存**（手写的 `actions/cache` 步骤）：三个必需的 Linux 拉取请求作业和基于 Wine 的必需 Windows 作业只恢复匹配归档而不保存，把缓存压缩／上传挡在它们的延迟敏感路径之外——这种不对称是 `setup-node` 的缓存无法表达的。每个作业都在 action 可替换的安装目录之外配置 store，并解析该路径；Linux 作业在可选自托管路线中跳过恢复，因为已配置的持久化 runner 拥有本地 store。
...
- **把必需作业的缓存也转换成 `cache: pnpm`。**否决：只恢复不上传的不对称是 `ci.yml` 注释中有记录的延迟决策；为统一工具而抹掉它，属于颠倒优先级。
- **保存拉取请求缓存。**否决，因为压缩和上传会进入必需路径。这些作业在兼容归档存在时消费它，否则照常执行不可变安装。
