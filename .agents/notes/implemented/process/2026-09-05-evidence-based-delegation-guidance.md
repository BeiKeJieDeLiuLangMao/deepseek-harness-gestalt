...
[`docs/agents/delegation-routing.md`](../../../../docs/agents/delegation-routing.md) is the provider-neutral owner for delegation routing and child-context reuse. Dispatchers choose an available provider and model under current user restrictions, separate configured capability and official product claims from repository task evidence, and accept work only through observable checks. Installation priorities live in conditional owners rather than a generic tier table.

Dispatchers inspect direct continuable children before starting a new child. Related follow-up continues an owner whose evidence, scope, permissions, and model remain suitable. Independent review, unrelated work, an unavailable or stale owner, or a capability that cannot be gained safely starts fresh. A safe host-supported turn-boundary model change may retain the owner session; `send_message` itself cannot switch provider/model or cwd. Forking is reserved for tasks that genuinely require completed parent history. A delta brief carries only changed facts and completion evidence. Continuation does not redirect a running turn, expose all workspace history, or guarantee provider-side KV cache.
...
[`docs/agents/delegation-routing-cliproxyapi.md`](../../../../docs/agents/delegation-routing-cliproxyapi.md) is the optional installation pointer. The delivery skill's model-routing reference owns the current CLIProxyAPI role priorities, excludes Terra and Luna, keeps the root route user-selected without fallback, chooses independent review from a non-author model, and distinguishes exhausted quota, rate limiting, authentication, network transport, and code failure. It relies on the live catalog and verified capabilities rather than static account counts or claimed Astra-exclusive quota.
...
**Always start fresh or always continue.** Rejected because independent review requires separate judgment, while related fixes and a safe host-supported route switch can preserve valuable owner evidence. The decision depends on scope, evidence currency, model fit, permissions, independence, and actual host capability rather than an arbitrary token threshold.
