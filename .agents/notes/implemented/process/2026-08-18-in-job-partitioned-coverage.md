...
`DSH_COVERAGE_MAX_WORKERS` continues to size the uninstrumented exempt gate and the ordinary non-partitioned path; it does not resize partition children. Standard hosted Windows serializes top-level gates and sets partition concurrency to one, so its eight isolated shards execute sequentially. The 16-core Windows failover pool retains eight-way partition fan-out. Standard hosted Linux limits the four fixed partitions to two active processes, while the 64-core failover pool retains four-way fan-out; every instrumented process remains single-worker.
...
A normal failed test still emits a blob through `--coverage.reportOnFailure`, allowing the merge to report the complete coverage state before the coordinator returns failure. A partition that exits with Vitest's exact isolated `Worker forks emitted error` and `Worker exited unexpectedly` diagnostics, without a `Failed Tests` section, is deferred until every partition's first attempt settles; the coordinator then deletes its blob and retries it once without another partition running beside it. An ordinary test failure is never retried, and a failed retry remains a gate failure. Spawn failure, signal termination, any other non-zero exit, a missing or extra blob, or a failed merge all make the gate fail. The coordinator removes only its owned coverage tree and unlinks a link-shaped path instead of recursively following it.

## Verification

`scripts/coverage-partitions.spec.ts` pins argument construction, package-script separator removal, partition-count and concurrency parsing, bounded scheduling, one-worker partitions, the single merged threshold command, failed-test merging, one bounded serial retry for an isolated worker exit after sibling first attempts, repeated-exit failure, failure diagnostics before complete-blob validation, waiting for sibling partitions after a spawn failure, and link-safe cleanup. `scripts/run-gates.spec.ts` pins opt-in selection, invalid-count rejection, the complete Windows inventory with its blocking split, and unbuffered streamed output. `scripts/ci-workflow.spec.ts` pins the hosted and failover Windows limits.
