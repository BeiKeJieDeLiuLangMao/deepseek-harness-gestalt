...
The protected `mobile-release` Environment builds one release-signed Android APK through the existing packaging script. The workflow uploads the APK and a manifest as the repository-accessible `mobile-acceptance-candidate-<candidate_sha>` Actions artifact. The manifest binds the candidate commit, plan, version, build, repository workflow run, APK digest, signing-certificate digest, and digests of the packaged runtime-identity record and packaging script. One signer reader accepts legacy and signature-scheme labels from Android SDK `apksigner` while requiring successful verification and exactly one 64-hex signing-certificate SHA-256 digest; zero, multiple-signer, and rotated-certificate outputs fail because the manifest records one certificate identity. The runtime record is generated from validated `VITE_PLATFORM_*` inputs, copied into the APK assets, and loaded by the application entry as its sole Platform identity authority. The workflow extracts that exact record from the signed APK, checks its origin against the protected Environment, and recomputes every manifest field before upload. The manifest contains no credential or origin value. Candidate-build-only produces no iOS artifact, tag, Release, TestFlight upload, release acceptance, or publication evidence.
...
**Match one signature-scheme label.** Rejected because Android SDK `apksigner` labels certificate output according to the APK's active V1, V2, V3, or V3.1 scheme, while the certificate digest is the stable field.
