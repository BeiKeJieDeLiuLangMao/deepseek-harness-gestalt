...
彻底删除可变的会话摘要。`SessionSummary` 与 `SessionMeta` 这个名称不再存在；后端存储和返回的元数据仅为 `SessionHeader`。抽象服务不再包含 `SessionPersistence.update()`。交付的 JSONL 提供方没有摘要伴随文件机制（`writeSidecar`/`readSidecar`/`touchSummary`/`removeSidecars`/`sidecarPath` 或 load/list 覆盖逻辑），树外提供方也实现同一份不含摘要的服务约定。
...
这次移除同时收窄公开服务约定和 JSONL 磁盘格式；摘要是有意为未来设计的结果，而非意外；如今原 Agent Note 描述 `SessionMeta` 之处已是 `SessionHeader`，这就是摘要消失的原因。它还简化了当时的[共享持久化写入协调器](../../archived/architecture/2026-06-18-shared-persistence-write-coordinator.md)：不再有可变摘要后，该协调机制也不再需要 `updateSummary` 钩子。

## 无需迁移

交付的 JSONL 提供方没有可变摘要格式或迁移路径：它只读写 `SessionHeader` 和仅追加日志。仓库没有第一方 SQLite Session 提供方。[仅 JSONL 持久化决策](2026-08-30-jsonl-only-session-persistence.zh.md)负责已移除提供方所写数据库的兼容性切断，并要求操作者在升级前用旧版本导出这些数据库。
