...
- **`json-rpc-2.0` for `dsh-sdk-jsonrpc-server`**: deletable correlation/dispatch is real (~100–130 lines), but the [SDK protocol](../../../../packages/sdk/protocol/README.md) requires newline-delimited JSON-RPC wire compatibility with the hand-written Python SDK client, and the server package is single-maintainer. `vscode-jsonrpc` is a worse fit still (Content-Length framing and cancellation vocabulary the protocol lacks).
