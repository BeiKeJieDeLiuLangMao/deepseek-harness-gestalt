...
- **以 `json-rpc-2.0` 替换 `dsh-sdk-jsonrpc-server`**：可删除的关联／分发代码确实存在（约 100–130 行），但 [SDK 协议](../../../../packages/sdk/protocol/README.zh.md)要求 newline-delimited JSON-RPC wire 与手写 Python SDK 客户端保持兼容，且 server 包只有一位维护者。`vscode-jsonrpc` 更不合适，因为它使用 Content-Length framing，还带入本协议没有的取消词汇。
