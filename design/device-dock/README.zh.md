...
| 轴 | 议题 | 状态 |
|---|---|---|
| 轴 1 | 多设备并存：单例 vs 每设备一 tab | **单例就地切换（用户验收后改定 2026-08-29）**——一个 「手机」 tab；`打开` 与设备下拉 `updateTab` 把 meta 翻到所选设备，标题为 `手机·<name>`；不再铸造 `phone:<serial>`，无 serial `dedupeKey` |
...
- 三轴已锁定：单例就地切换、恒可达 + badge、已连接画面 1:2 定比居中。
