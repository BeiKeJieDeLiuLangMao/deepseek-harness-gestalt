...
- **Web browser snapshot** (`pnpm run test:web`; required Linux PR gate): first install the pinned headless Chromium with `test:web:setup`; `test:web` [builds](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md) and compares replayed output with the Web, Mobile, and Platform browser goldens under their owning `apps/*/tests/snapshots/` directories. Run one inventory file with `test:web:focus -- <file>`. CI is read-only; record/refresh stay local ([lane](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.md), [gate](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.md)).

Session fixtures omit body sequence/time envelopes; replay synthesizes them without changing persistence. Core preflight checks canonical packed rows with `verify-session-fixture-layout`; [the migrator](../scripts/migrate-packed-session-fixtures.ts) writes them. Expensive scenarios can precheck replay call counts and visible `text-delta` output with `assertReplayFixture()`.

## The with-key policy: inference is cheap here

We are DeepSeek — do not ration real-API tests. A no-key test proves plumbing; only a with-key run proves the agent works against a real model. Cover file-writing prompts, multi-turn conversations, tool use, and mid-stream cancellation. Highest-value are **smoke tests** that boot the real application, send one prompt, and check the world — they catch the "green unit tests, broken product" class that mocks cannot ([postmortem 0001](postmortem/0001-acp-default-export-drops-inject.md)). Self-skip keeps secretless CI and keyless contributors unblocked; it is not a cost signal. Keyless and with-key scenarios live beside the application, profile, or package they exercise.
