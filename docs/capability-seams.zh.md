...
flowchart LR
  ...
  svc_schedules["ctx.schedules<br/>Durable Session reminder owner"]
  pkg_ui_schedule["ui-schedule"]
  ...
  svc_platformAccount["ctx.platformAccount<br/>Platform Account identity seam"]
  ...
  svc_tools --> pkg_agent_tool_eligibility
  ...
  svc_tools --> pkg_tools_eligibility
...
| `ctx.schedules` | `core` | [`schedule`](../packages/schedule/schedule) | - | `ui-schedule` | - | 负责人工 Remote 变更与根 Agent timer 生命周期；Session 日志仍是持久权威，浏览器消费其独立 projection。 |
...
| `ctx.settings` | `seam` | [`settings`](../packages/settings/settings) | [`settings-file`](../packages/settings/settings-file) | [`llm-deepseek`](../packages/llm/llm-deepseek), [`llm-pi-ai`](../packages/llm/llm-pi-ai), [`settings-controller`](../packages/api/settings-controller) | - | 插件注册命名空间 schema 并解析分层值；提供方存储原始文档。LLM（大语言模型）适配器在用户分区下将其入口配置注册为组合基础；Web 网关提供经过脱敏的分层描述符，并写入用户层。 |
| `ctx.credentials` | `seam` | [`credentials`](../packages/credentials/credentials) | [`credentials-local`](../packages/credentials/credentials-local) | [`llm-deepseek`](../packages/llm/llm-deepseek), [`llm-pi-ai`](../packages/llm/llm-pi-ai), [`settings-controller`](../packages/api/settings-controller) | - | 配置携带对机密信息的引用；提供方拥有实际值。消费方按操作解析，因此轮换后的凭据会在紧接着的下一次请求中生效；Web 网关提供不含实际值的视图和只写存储。 |
...
| `ctx.desktopProjectMembership` | `core` | [`project-membership-desktop`](../packages/platform/project-membership-desktop) | - | [`tool-project-members`](../packages/interaction/tool-project-members), [`tool-ask-user`](../packages/interaction/tool-ask-user) | - | 校验受 token 保护的 Desktop loopback 投影，并向 agent preset 工具提供已鉴权 Account、Workspace Project、名册和合格成员提问路由读取，而不暴露 Platform 凭据。 |
...
| `ctx.workspaceRegistry` | `core` | [`workspace`](../packages/workspace/workspace) | - | [`workspace-controller`](../packages/api/workspace-controller), [`session-controller`](../packages/api/session-controller) | - | 通过领域设施拥有带 WorkspaceId 品牌类型的记录；稳定的 sessionIds 账户驱动 Host RPC 与 GUI 投影。 |
...
| `ctx.memberQuestionReceiver` | `seam` | [`member-question-receiver`](../packages/interaction/member-question-receiver) | [`member-question-receiver`](../packages/interaction/member-question-receiver) | [`member-question-receiver`](../packages/interaction/member-question-receiver) | - | 持久化认证 arrival、Host Session 物化、canonical terminal、expiry 与一次已 reservation 的高层 human admission；包内折叠的 ingress adapter 是当前 Consumer。 |
| `ctx.memberQuestionWorkspaceBinding` | `core` | [`member-question-receiver`](../packages/interaction/member-question-receiver) | - | [`member-question-receiver`](../packages/interaction/member-question-receiver) | - | 为 Host Session 物化把已认证的 receiver Account 与 cloud Project 解析为唯一且既存的本地 Workspace。 |
...
| `ctx.sessionProjections` | `core` | [`session-projection`](../packages/session/session-projection) | - | [`tool-todo`](../packages/todo/tool-todo), [`session-title`](../packages/session/session-title), [`session-controller`](../packages/api/session-controller) | - | 各领域注册由状态驱动的折叠单元；Session Controller 通过生成式 Remote 发布快照与实时投影帧。 |
| `ctx.sessionProjectionCache` | `core` | [`session-projection-cache`](../packages/session/session-projection-cache) | - | [`session-controller`](../packages/api/session-controller) | - | 持久化投影单元状态检查点，并为会话列表提供有界冷快照，无需加载完整日志。 |
...
| `ctx.agentDefaultModel` | `core` | [`agent-default-model`](../packages/core/agent-default-model) | - | [`headless`](../packages/bundle/headless), [`session-controller`](../packages/api/session-controller) | - | 通过 settings 分层默认 `ModelSelection`，让直接入口与 Host 支撑的 Agent 入口共享同一个状态所有者。 |
...
| `ctx.directoryPicker` | `seam` | `directory-picker` | `directory-picker-native`, `directory-picker-browse` | [`workspace-controller`](../packages/api/workspace-controller) | - | 带判别标记的交互能力：原生后端在 Host 显示设备上打开一个操作系统选择器，浏览后端为应用内浏览器提供列表与创建原语；双端后端通过其浏览器侧填充 ui-workspace 目录流程的 slot（不通过协议发布）。 |
