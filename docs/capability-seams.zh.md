...
| `ctx.browserRuntime` | `seam` | `browser` | [`browser-runtime-deterministic`](../packages/browser/browser-runtime-deterministic), [`browser-runtime-tandem`](../packages/browser/browser-runtime-tandem) | [`tool-browser`](../packages/browser/tool-browser), [`browser-workspace`](../packages/browser/browser-workspace) | - | 不透明的 Profile、Workspace、浏览器与标签页身份保留在 ctx.browserRuntime 后；deferred Consumer 使用普通发现、结果与展示路径。 |
| `ctx.browserWorkspace` | `core` | `browser` | - | [`tool-browser`](../packages/browser/tool-browser) | - | 每个 Session 独立持有 Dock 事实、实例与标签页，并建立在 ctx.browserRuntime 身份之上；当调用 Agent Session 存在时，deferred Consumer 会绑定所创建的标签页。 |
