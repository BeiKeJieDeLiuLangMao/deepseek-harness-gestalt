...
| `ctx.browserRuntime` | `seam` | `browser` | [`browser-runtime-deterministic`](../packages/browser/browser-runtime-deterministic), [`browser-runtime-tandem`](../packages/browser/browser-runtime-tandem) | [`tool-browser`](../packages/browser/tool-browser) | - | 不透明的 Profile、Workspace、浏览器与标签页身份保留在 ctx.browserRuntime 后；deferred Consumer 使用普通发现、结果与展示路径。 |
