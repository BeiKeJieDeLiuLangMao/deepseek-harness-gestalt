...
Desktop is an application composition rather than a package compiler face. The workspace constraints gate keeps it out of both root aggregates and requires its complete no-emit check after Client tsc.
...
Each master push also runs bounded Linux and Windows standby smokes after rebuilding a clean current-platform dependency tree and executing its Codex and Claude Code payloads. Complete unsharded standby inventories run daily or through the `standby-exhaustive` dispatch. The persistent Windows service host omits the `knip` and duplication gates already owned by the Linux inventory while retaining the Windows-specific and portable repository gates. Reports use the `failover-readiness` classification, so readiness failures remain visible without lowering product-regression success rates; the [failover runbook](../.agents/notes/implemented/process/2026-07-26-ci-failover-runbook.md) owns the two independent takeover switches.

The default branch and daily schedule produce hosted Linux and Windows pnpm-store and Playwright caches. Keys include OS, architecture, Node, pnpm, and the lockfile; pull-request consumers try the exact key and then only the matching environment prefix. Gate reports record each primary key, matched key, and exact-hit result. A miss still performs the same immutable install and browser provisioning from a clean workspace; workspace `node_modules` and build outputs are never cached.

A Draft pull request with fully known low-risk paths runs tests for changed packages and every reverse consumer, with 100% thresholds applied to existing changed source files. GUI and model-visible paths also run assembled snapshots. Ready pull requests and changes to workflows, lockfiles, vendor source, protocols, session lifecycle, agent loop, build configuration, multiple product areas, or unknown paths run the exhaustive plan. A `merge_group` event runs that plan against GitHub's candidate merge tree and exposes `candidate verdict` as its single fail-closed conclusion. Its attestation keys the Git tree, lockfile, workflow and Planner, toolchain, and gate inventory. A master push with the same tree and complete key reuses that proof and runs only the named master smoke; missing or mismatched proof runs the exhaustive fallback. `pnpm ci:impact --base <ref> --head <ref> --plan-only` prints the same Draft command locally; omit `--plan-only` to execute it.

Exhaustive pull-request plans keep Wine as the required Windows signal and run native Windows build/runtime, coverage, and static portability as parallel observational evidence. Coverage uses two Windows blob-producing workers and one parallel Windows coverage-exempt worker; a hosted Linux reducer applies thresholds to the self-contained blobs without executing product code. Build/runtime and static portability each retain an independent worker. Their single native verdict fails on any failed, cancelled, or skipped owner without delaying `all checks passed`. Merge Queue makes the native verdict and macOS Electron lane required alongside every normally blocking lane. Each owner publishes inspectable evidence; the complete local inventory remains `pnpm run check:ci:windows-complete`.
