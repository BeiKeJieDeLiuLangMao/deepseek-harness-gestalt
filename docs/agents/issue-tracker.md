...
Pull-request policy read authentication is an explicit deployment option in `.github/issue-management/config.json`. Set `pullRequestReadAuthentication` to `anonymous` only for a public repository: the `pr` command omits `Authorization` from all pull-request and referenced-Issue reads even when a workflow token exists, while every API error remains fatal. Set it to `token` for repositories that require authentication: the command sends `GH_TOKEN` or `GITHUB_TOKEN` as a Bearer token and fails before its first request when neither exists. Other values are rejected at startup. GitHub documents unauthenticated access to public resources for both [requested reviewers](https://docs.github.com/en/rest/pulls/review-requests#get-all-requested-reviewers-for-a-pull-request) and [pull-request reviews](https://docs.github.com/en/rest/pulls/reviews#list-reviews-for-a-pull-request). The personal-account tracker uses `anonymous`.
