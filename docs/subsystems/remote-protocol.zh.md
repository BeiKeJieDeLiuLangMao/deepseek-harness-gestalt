...
Relay Transport 版本协商独立于 Encrypted Companion 应用协商。Relay 只能解析 attachment、转发、心跳、撤销与 transport error 元数据；其转发 payload 始终是字节。只有双方从 major 2 和 1 中选出最高的安全共同 major，并同时保留已认证加密、配对密钥隔离与重放保护后，Companion 消息才可用。

协商结果是进程内不可伪造的 capability，`encodeCompanionMessage` 与 `decodeCompanionMessage` 都必须持有它。`COMPANION_UPDATE_REQUIRED` 和 `COMPANION_SECURITY_CAPABILITY_MISSING` 会指出必须更新的 endpoint。调用方不能在协商成功前生成应用消息，因此失败路径只携带版本和 capability 元数据。
