...
The session-controller inspection path exposes an immutable logical read as `SessionInspection`: validated storage metadata and the complete contiguous event log. It is a detached value, not an open handle or write-ownership token.
...
A backend refuses a log it cannot faithfully interpret with `SessionFormatUnsupportedError`, distinct from `SessionPersistenceCorruptionError` because nothing is damaged. A header version ahead of `SESSION_FORMAT_VERSION` names the direction (the log was written by a newer harness, so upgrade the harness); one behind it states that this build ships no upgrade path. An event type outside this build's generated vocabulary is refused unless its envelope carries `ignorable: true`, because silently skipping an unrecognized required event could change reconstruction. The message appends the raw log path when the backend keeps one artifact per session. The JSONL backend refuses a foreign version from the raw header before validating the current header fields or decoding event rows, so a structurally different future format reports the upgrade direction rather than corruption.
