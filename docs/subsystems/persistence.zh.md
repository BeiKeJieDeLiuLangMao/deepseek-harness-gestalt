...
每一次日志读写都经由句柄流动，绝不经由按 id 寻址的服务方法：句柄是未来跨进程写租约可以把守的唯一入口。一种句柄类型同时服务两种访问——在 `read` 句柄上执行修改是运行时的 `SessionReadOnlyError`，而非类型层面的拆分——而进程内单写者所有权使得在已有活跃持有者时第二次 `open(id, 'write')` 以 `SessionAlreadyOwnedError` 拒绝。
...
session-controller 检查路径以 `SessionInspection` 暴露一份不可变的逻辑读取：经过校验的存储元数据与完整连续事件日志。它是分离值，不是打开的 handle，也不是写所有权 token。
...
后端用 `SessionFormatUnsupportedError` 拒绝无法可靠解读的日志，它与 `SessionPersistenceCorruptionError` 区分，因为数据没有损坏。header 版本高于 `SESSION_FORMAT_VERSION` 时，错误会指出日志由较新的 harness 写入，应升级 harness；版本较低时则说明当前构建未提供升级路径。事件类型不在本构建生成的词汇中时也会被拒绝，除非其信封携带 `ignorable: true`；静默跳过无法识别的必需事件可能改变重建结果。后端为每个会话保留独立文件时，消息会附上原始日志路径。JSONL 后端会直接从原始 header 拒绝外来版本，再校验当前 header 字段或解码事件行，因此结构不同的未来格式会报告升级方向，而不是误报损坏。
...
随产品交付的 provider 实现抽象 `SessionPersistence` 约定（`create`/`open`/`stat`/`list`，逐会话 `SessionHandle` 承载 `read`/`append`/`flush`/`close`，全程可选支持取消），并通过共享的持久化契约套件。[仅保留 JSONL 持久化的决策](../../.agents/notes/implemented/simplification/2026-08-30-jsonl-only-session-persistence.zh.md)说明了第一方范围为何只保留一种物理格式：
