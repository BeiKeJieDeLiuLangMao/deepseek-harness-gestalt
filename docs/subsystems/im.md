...
Types shared by IM account takeover: durable accounts and route rules, delivery cursors and outbound receipts, local simulation instances, idle DingTalk DWS, and Wangwang merchant overlays. The [Host composition Agent Note](../../.agents/notes/implemented/feature/2026-09-10-im-host-base-composition.md) owns the `dsh-base` row set; this page records the exact fields from [`packages/im/im-core/src/types.ts`](../../packages/im/im-core/src/types.ts), [`packages/im/im-core/src/delivery/types.ts`](../../packages/im/im-core/src/delivery/types.ts), [`packages/im/im-core/src/simulation/types.ts`](../../packages/im/im-core/src/simulation/types.ts), [`packages/im/im-dingtalk/src/types.ts`](../../packages/im/im-dingtalk/src/types.ts), and [`packages/im/im-wangwang/src/types.ts`](../../packages/im/im-wangwang/src/types.ts). Package READMEs own adapter spawn, tool registration, and live-lane authorization.

## Accounts and routing

`ImAccountId` and `ImRouteRuleId` are [branded ids](core.md#branded-ids). Account metadata stores only a `CredentialRef`; secrets never appear on the record. A disabled specific rule keeps its workspace binding and does not fall back to an `all` rule. Unconfigured conversations must not route to a workspace.
...
Real and simulation scopes use colon-safe encoding so they cannot collide. Inbound records are deduplicated on `(scopeId, externalMessageId)`. Ambiguous outbound receipts settle as `result_unknown` and must not be retried automatically. Simulation tools register only after a workspace has a configured target; a stopped instance is terminal.
...
DingTalk mounts idle on `dsh-base` and does not spawn DWS until `startConsumer`. Wangwang stays an overlay because its adapter fails loud without an admitted merchant directory; inbound sender classification trusts only durable local outbox echoes.
