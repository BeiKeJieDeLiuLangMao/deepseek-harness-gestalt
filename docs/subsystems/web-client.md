...
- `ClientSessions` provides `ctx.sessions`, owns Session scopes and stable `SessionBinding` objects, and projects the selected list state. `stageProvisional()` and `openForRender()` list and open an explicit identity without changing `list.current`. `registerAdmission()` and `registerAdmissionAdapter()` bind feature-owned prompt, cancel, queue, and command routes to exact Session identities without granting Host authority. Its discriminated model route combines stock selection with catalog/projection state for ordinary Sessions and requires inspection plus selection for feature-owned state.
