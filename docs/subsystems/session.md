...
declare class Session {
  ...
  */
   ...
   * @param opts - Surface events require `surfaceOp` and may cite earlier
   *   `sourceEventSeqs`; Assistant messages embed their exact provider stream
   *   and cannot cite top-level source events. Non-surface events may pass
   *   `{ ignorable: true }` when readers without that event type can skip it
  ...
  append<T extends SessionEventType>(
    ...
    ...opts: T extends SurfaceEventType ? [opts: SurfaceIntent<T>] : [opts?: IgnorableEventIntent]
  ): SessionEvent<T>;
  /**
...
- `fork(source, boundary?, childSessionId?)` accepts a live `Session` object or live `SessionId`, selects source events through the inclusive `boundary` seq (default: current last event), requires the selected prefix to end outside an open turn, then creates a live child session with deep-cloned seed events, child metadata (`parentSession`, `isSeeded`, and inherited `cwd`), and the exact `inheritedEventCount` cut.
...
Types: [ModelCatalog](llm-streaming.md) · [SessionId](core.md) · [SessionInspection](persistence.md) · [SessionSearchRequest](session-query.md)
...
Types: [CreateSessionOptions](persistence.md) · [PrepareSessionOptions](persistence.md) · [SessionId](core.md) · [SessionSeq](core.md)
