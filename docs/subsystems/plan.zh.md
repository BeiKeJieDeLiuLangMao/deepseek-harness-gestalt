...
`plan/mode`（`{ active: boolean }`）是仅记日志、整值替换的[会话事件](session.zh.md)：持久且可回放，绝不进入模型 transcript（文本记录）。可选注册的 `plan` 投影单元会折叠已提交模式、命令结算状态，以及最近一条请求头记录的模式。`ctx.planMode` 通过 `stateOf()` 读取该状态；若缺少 registry、`plan` 键或 `turnBoundary` 键，首次依赖访问即失败。客户端仅接收 `{ active, pending }`；恢复、fork 与压缩（compaction）会从日志恢复这两个字段。完整事件声明见[持久化日志事件目录](../persistence-catalog.zh.md)。
