...
::: tip 表单与配置文件
模型页提供 API 密钥、显示名称、API 地址、API 协议、模型 ID、名称、容量，以及 pi-ai 的输入类型和思考档位声明。请求兼容性开关、请求头、超时、重试策略、自定义推理线路取值和 `modelOverrides` 仍在 `$DSH_HOME/settings.yaml` 中设置，也就是模型页写入的同一份文档。可以直接编辑它；浏览器与服务器在同一台机器时，也可以点击设置页顶部的**打开配置文件**打开它。适配器会在下一次请求时重新读取，无需重启。下面各小节介绍多数网关会用到的字段。
:::
...
因此自定义提供方下的视觉模型需要这份列表。展开该模型行，勾选**文本**和**图片**——它们写入的就是 `$DSH_HOME/settings.yaml` 里的 `input` 数组：
...
如果你手动录入的模型全都接受图片，可以在提供方上设置一次回退值，不必逐个模型写。提供方卡片上同一组标签对应**默认接受的输入**：
...
对于声明了推理等级的模型，模型选择器会提供**推理等级**菜单。内置提供方的模型从已安装目录继承其等级，手动录入的模型在配置前不声明等级。打开**设置 → 模型**，编辑提供方，展开**自定义设置**，再打开模型的**高级设置**选择**思考档位**。清空所有标签会恢复继承。同一声明也可写入 `$DSH_HOME/settings.yaml` 的 `reasoningEfforts`，包括自定义线路取值：
...
- **手动录入的模型没有推理等级菜单**：在模型页打开该模型的**高级设置**并选择**思考档位**，或在 `settings.yaml` 中声明 `reasoningEfforts`。
...
- **图片在发送前被拒绝**：该模型未声明图片模态。在自定义模型的**高级设置**中，为**接受的输入**选择**文本**和**图片**，等同于 `input: [text, image]`；在 DeepSeek 自身的路由上，请选择声明了图片能力的模型 `deepseek-v4-flash-vision-exp`。
