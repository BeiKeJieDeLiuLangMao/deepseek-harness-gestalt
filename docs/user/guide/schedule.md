...
DeepSeek Gestalt Desktop enables Schedule by default. This overlay opts one browser-only `dsh web` process into the same reminders and board without changing the shipped default Web composition:
...
With Schedule enabled, a successfully opened Session with retained reminders shows a task board in the conversation header. It lists the complete prompt, scheduled, overdue, or paused status, one-time or exact repeating cadence, browser-local target time, and relative time. The board can pause, resume, and delete a reminder; deletion requires confirmation, and each successful change survives reload through the Session log. It does not create or edit reminders and never acknowledges reminder delivery. The sidebar separately places a non-interactive alarm after the title of grouped, flat, and search rows when their currently available projection value is non-empty; a cold Session's cached alarm can be briefly missing or stale.
