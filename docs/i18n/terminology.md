...
| File Mention | 文件提及 | | 文件引用、@file | assistant 收尾正文里对产出文件的行内代码链接；不要用来称呼 File Reference |
| File Reference | 文件引用 | 文件引用（File Reference） | 文件提及、工作区引用、附件 | 用户在提示词里写下的 `@path`；不携带文件字节，内容只通过 `read` 取得 | |
| checkpoint | 检查点 | | | |
| chunk | 分片 | | | |
...
| session | 会话 | | | |
| session event | 会话事件 | | | |
| Session Reference | 会话引用 | 会话引用（Session Reference） | 文件引用 | 另一会话当前表层的有界只读快照 | |
