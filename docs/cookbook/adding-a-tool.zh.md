...
- `presentResult(args, { content, isError, meta? })` 返回完成后的卡片：
  ...
  - `read` 提供从持久化 `result.meta` 重建的完整文件窗口：文件 `path`、从 1 开始的 `offset`、返回的 `lines`（每行保留其文件行号）、`totalLines`，以及可选的 `lang` 高亮提示；不具备 `read` 能力的 UI 会回退到原始结果内容。不存在 `read` 调用视图——读取调用的 pending 状态保持为 generic 卡片，因为内容只会在 `execute` 后出现。（tool-fs 的 `read`。）
...
中性词汇定义在 `dsh-tools` 中；工具绝不导入 UI 或传输类型。该 API 的消费方将每个 `card` 映射到各自的视图。设计与原因见[渲染意图联合体 Agent Note](../../.agents/notes/implemented/architecture/2026-07-02-tool-render-intent-union.zh.md)；`dsh-tool-fs`（generic/diff）和 `dsh-tool-bash`（terminal）是参考实现。

## Web Client 呈现

内置 Web Client 不消费 `presentCall` 或 `presentResult`。Session `page` 与 `follow` 会传输原始 `tool/call` 和 `tool/result` 事件，其中包括持久化的 `result.meta`。Client 插件在 `tool.call.toolview` keyed slot 中注册其 wire tool 名称，并根据 `ToolCallBlock` 的参数、内容、错误、元数据、既有 Code Dispatch `parentCallId` 与 Session 路径事实派生组件 props。它在本地校验这些 wire 值，并在输入格式错误或不受支持时返回 generic 行。

当既有 Web 卡片需要模型可见内容无法无损保留的有界结构化结果事实时，使用 `output.presentationMeta(args, value)`。不要在 metadata 中保存 React props 或选中的卡片，不要把 Host 工具实现导入 browser bundle，也不要创建另一个 Client presenter registry。只定义 Host presentation 方法不会增加专用 Web 卡片。[Client 派生呈现 Agent Note](../../.agents/notes/implemented/architecture/2026-08-23-client-derived-tool-presentation.zh.md)定义所有权、回退与等价性要求。
