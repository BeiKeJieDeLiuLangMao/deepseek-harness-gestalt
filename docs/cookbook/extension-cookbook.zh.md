...
随附应用通过 `packages/bundle/*/cordis.patch.yml` 贡献 profile 层；产品 `dsh` launcher 通过具名 profile 负责 Web、ACP、SDK 与一次性 headless 执行。可选的用户 overlay 位于 `apps/cli/config/examples/`；profile 集成测试位于 `apps/cli/tests/profiles/`，各包专属 Loader 组合则与包测试放在一起。
