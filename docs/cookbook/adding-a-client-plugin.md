...
1. Create `packages/client/<name>` with `package.json`, `tsconfig.json`, `tsdown.config.ts`, `src/index.ts`, and `README.md`. Add `src/invariant.ts` and an `./invariant` export only when the package owns an independent runtime relationship. A browser plugin also provides `src/client/`, the declared `./client` export, and `src/css-modules.d.ts` when it uses CSS Modules. Use `@deepseek-ai/dsh-client-<name>` and the package README's Model Experience section; when no invariant is published, state why in the README.
...
4. Use shared tokens through CSS Modules, follow the standing localization rule for product copy, and write code comments in English.
