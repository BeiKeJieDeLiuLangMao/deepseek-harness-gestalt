...
- **Web 浏览器快照**（`pnpm run test:web`；必需的 Linux PR（Pull Request）门禁）：先用 `test:web:setup` 安装固定的 headless Chromium；`test:web` 会[构建](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.zh.md)并将回放输出与 Web、Mobile 和 Platform 各自 `apps/*/tests/snapshots/` 目录下的浏览器 golden 比较。用 `test:web:focus -- <file>` 运行清单中的单个文件。CI 只读；record/refresh 留在本地（[车道](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.zh.md)、[门禁](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.zh.md)）。

会话 fixture 省略正文序号／时间 envelope；Replay 会合成它们且不改变持久化。核心 preflight 使用 `verify-session-fixture-layout` 检查规范打包行；[迁移器](../scripts/migrate-packed-session-fixtures.ts)负责写入。昂贵场景可先用 `assertReplayFixture()` 检查 Replay 调用次数与可见 `text-delta` 输出。
