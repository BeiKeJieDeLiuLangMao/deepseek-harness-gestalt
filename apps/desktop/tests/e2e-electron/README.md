...
The launcher owner records the directly created WDIO child before readiness and observes its `exit` independently of inherited pipe `close`. The hidden capability keeps Electron detached while the WDIO `after` hook deletes the live WebDriver session, clears the runner-held session id, and then requests `app.quit()` through Electron Service's independent main-process CDP bridge. Deletion and quit failures both fail teardown. A zero exit passes only after the verifier finds one ready Host, that exact Host's `code=0 signal=null requestedStop=abort` normal-quit exit, exactly one `shutdown complete` receipt, zero model requests, unchanged input hashes, and closed Host, fake, and CDP listeners. The runner then removes its exact private scratch; evidence remains under `.artifacts`.
