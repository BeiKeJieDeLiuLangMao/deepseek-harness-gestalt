...
Closing the last window on every platform, Ctrl+C, and smoke-test completion cancel any pending start, stop the Personal Pairing and production-gated Relay owners, stop the Web Host, and wait for their work to drain before Electron terminates. System sleep stops Remote Access; resume reloads it only for the still-signed-in Account. Startup or a later Host crash gets one retry before the window shows the Host error. There is no windowless daemon, background Host, or remote wake path.
...
- **Production Personal Pairing crypto is not assembled** — the development keyless flag proves controller wiring only; production Settings and the Host bridge fail closed until independent Noise review admits a product provider.
