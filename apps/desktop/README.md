...
Personal Pairing is configured only in the real `手机配对` Settings section. The preload exposes Mobile Access, challenge, pending-confirmation, and paired-device verbs without adding status to the normal Session header, sidebar, approval, composer, or offline views. After Account sign-in, the Host-owned controller signs a fresh current-Installation proof for each operation, creates XKpsk3 invitation state locally, and forwards only opaque mailbox messages. On confirmation, Desktop creates the Mobile Relay credential, submits only its SHA-256 digest to Platform, seals the grant with the first Snow transport payload, and retains the reconnect record in a `safeStorage`-protected owner-only atomic file. The same owner runs the real WSS lifecycle only while Mobile Access is enabled. `SnowDesktopAttachmentOwner` admits only a currently projected route/selector/attachment/generation-bound IK request, replaces an active selector only after authentication, and immediately follows IK2 with a versioned encrypted `foreground-sync`. Development selects no keyless product controller.
