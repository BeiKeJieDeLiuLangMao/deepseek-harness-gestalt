...
function electronNativeStagePort(): NativeStagePort {
  return {
    addDownloadedListener(listener) {
      electronAutoUpdater.on('update-downloaded', listener)
    },
    removeDownloadedListener(listener) {
      electronAutoUpdater.removeListener('update-downloaded', listener)
    },
    addErrorListener(listener) {
      electronAutoUpdater.on('error', listener)
    },
    removeErrorListener(listener) {
      electronAutoUpdater.removeListener('error', listener)
    },
  }
}
...
async function boot(): Promise<void> {
  ...
  try {
    ...
    if (process.env.DSH_DESKTOP_SMOKE === '1') {
      await finishSmoke(window, running.url)
      return
    }
    void ensureChromeOverlay(window, running.url)
  } catch (error) {
