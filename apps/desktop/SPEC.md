...
Ship **DeepSeek Gestalt**, a Desktop Host: an Electron window that starts a bundled official Node plus a locked `dsh web` Web Host on loopback and loads that URL. The existing Session Surface stays in one window with many Workspaces in the sidebar. Desktop adds only a window frame, a GESTALT badge, a drag strip, and an Update Control next to Settings. Browser `dsh web` is unchanged. Updates come from GitHub Releases on the personal fork, after the user confirms download.
...
- Do not change Web Host capabilities. The native directory picker stays. Desktop only adds the Apple Events entitlement for notarized macOS.
...
Modules under test: Desktop Host spawn/URL discovery, Launch Directory, updater state machine, Desktop overlay chrome (badge, drag strip, Update Control), sidebar foot layout (Settings and footer actions on one row), and the guarantee that browser composition does not mount the overlay.
