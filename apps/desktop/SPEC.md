...
- Desktop Host starts Web Host with `--host 127.0.0.1 --port 0 --no-open` and a Desktop-only `--patch` overlay, then loads the authenticated loopback launch URL printed as `dsh web: http://127.0.0.1:<port>/?token=…` while treating the token-free origin as the public Host URL. `--no-open` is required because `dsh web` otherwise hands that URL to the OS default browser, duplicating the Desktop window. The overlay also sets `web-runtime.openBrowser` to false. Desktop Host exchanges the launch token once over loopback `GET /` for an in-memory Host cookie, uses that cookie on unary `/api` and `/api/remote.mux` upgrades to the same origin, and never writes that cookie to disk or to Companion. Companion archived filtering reads the `workspace/follow` snapshot cache, not a unary `workspace.list`. Companion conversation history reads `session/follow` then `session/page({address,throughSeq})`; packed `chunkrow/*` records expand through the Session history codec.
...
- Updater protocol (from the prototype state machine): phases `disabled | idle | checking | available | downloading | preparing | downloaded | installing | error`. `autoDownload` is false. User must confirm download. `available` still rechecks the GitHub feed on the 15-minute interval without hiding Download; a later `update-available` replaces the shown version. On macOS, `preparing` waits until Squirrel stages the bundle; Install and restart is offered only after that. Ordinary quit does not install.
