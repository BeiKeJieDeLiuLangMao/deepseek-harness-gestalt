...
`pnpm run typecheck:desktop:contracts-ready` 以不生成产物的方式检查三个独立 TypeScript Program：Desktop 应用、Host 400 响应 codec probe 与 Companion process driver。它只执行检查，不生产运行时产物；被引用包的声明产物必须已存在。即使某个 compiler face 失败，三个 face 也都会执行；普通类型错误和跨项目输出路径诊断仍会阻断。Browser driver 使用实际 Mobile 组件，Host 与 Client 的 `Context.sessions` 声明绝不进入同一个 Program。Fixture IPC 只传递进程生命周期命令和断言数据，不替代产品响应。

Process-driver 测试显式传入 child entry，在不加载第二套 Host API 的情况下验证进程所有权。完整 Client 构建包含 `apps/mobile/tsdown.config.ts`；其 build-after smoke 位于 `apps/mobile/tests/mobile-browse-artifact.e2e.ts`，禁用配置驱动的源码 alias，经 Desktop 私有 import map 把 `#testing/mobile/MobileBrowse` 解析到 `apps/mobile/lib/MobileBrowse.js`，并在 Chromium 中连同生成的 CSS 一起渲染。该 artifact smoke 只覆盖持久化 Mobile browser component，不会把源码级 browser suite 变成产物 consumer。
