...
- **账号池状态保持隔离** — Desktop 在自己的 `userData` 下生成私有运行时 root，其中包含相互分离的 management 与 inference key、配置、auth 文件和日志。发送 inference key 前，它会验证自身 spawn 的子进程 PID 拥有所选 loopback 监听。关闭时只停止并等待该子进程组退出，然后删除运行时 root；它从不读取 `~/.cli-proxy-api`、Sub2API 数据或外部服务配置。
