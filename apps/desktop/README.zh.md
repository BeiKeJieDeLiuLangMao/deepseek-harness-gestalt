...
DeepSeek Gestalt 的 Desktop Host。Electron 拥有窗口、菜单、GitHub 自动更新，以及进程内 Browser Runtime `webContents`。它启动捆绑的官方 Node 加上 `dsh web --patch ./cordis.patch.yml --no-open --host 127.0.0.1 --port 0`，并打开该环回 URL。`--no-open` 阻止再唤起系统默认浏览器，因为 Desktop Host 已经拥有窗口。窗口从第一帧起显示本地 GESTALT 启动标记，并在 Session Surface 绘制完成后再撤下该标记。叠加层加入 Schedule、GESTALT 次标、拖拽带、Update Control，以及指向 Host loopback Browser origin 的 Tandem 形态 HTTP 客户端；只有更新可操作或发现版本后发生错误时，控件才会出现。仍显示「下载」时，同一 15 分钟 GitHub feed 检查仍会运行，后续 latest 版本会替换已提供的目标。浏览器 `dsh web` 不加载这层，并继续使用确定性 Browser Runtime。叠加层始终加入 phone environment、稳定 runtime、stream 与 tool。runtime 会等待 environment owner 从显式 `DSH_PHONE_MOBILECLI` override、已校验的托管 current 或系统发现结果中选择可执行文件；准备完成后无需重启。「手机」tab 这一行本身来自 web-app roster，而非这层叠加——重复插入会撞出重复 loader entry id 并中止启动，组合 boot 规格测试正是对此的回归防护。

在所有平台关闭最后一个窗口时，会先以 `window-close` 原因排空 Relay，并 POST presence close，使花名册读者立即看到 Offline，然后退出 Desktop Host。重新打开会启动新进程，其启动时的 `setSignedIn(true)` 通过既有心跳推导恢复 Online。Ctrl+C、quit 与 smoke 测试结束都会关闭 Host 启动准入，等待已接纳的 Browser/Membership 初始化、精确持有的当前或退役 Web Host，以及 runtime 释放完成。Account、Pairing、presence 或 Host 的独立清理失败不会跳过其他清理；显式 `app.exit` 在这些操作全部落定后执行，失败时退出码为 1。首次关闭请求决定模式和退出码。诊断输出失败不能阻止显式失败退出。就绪前的子进程停止失败保留为 `WebHostStartupCleanupError`，因此即使启动未返回运行中的 Host，关闭仍报告失败；已观察到子进程退出的普通取消不会导致关闭失败。关闭请求 promise 表示策略处理完成，包括已处理的清理失败；其 resolve 不证明清理成功或子进程已退出。原生 `allow-quit` 与重复 quit 行为保持不变，不构成 Electron 退出屏障。Host 替换复用 Browser 和 Membership runtime；关闭后的迟到启动结果不能安装 Companion 就绪状态。系统 sleep 会停止 Remote Access；resume 只为仍处于登录状态的账号重新加载。源码 Electron smoke 会在 sleep、关闭手机访问、关闭窗口与 quit 后读取各次 Relay owner 状态，再检查 Web Host 子进程 PID 已消失。首次启动或后续 Host 崩溃共允许一次重试，之后窗口才显示 Host 错误。不存在无窗口 daemon、后台 Host 或 remote wake 路径。Chromium 持久 partition 位于 Electron `userData/Partitions/<name>`；loopback API token 放在 `userData/browser-runtime` 下，绝不写入 Tandem Browser Application Support。Dock 仍是截图、标题与文本的原生窗格。

主窗口只接受当前环回 Host 同源导航。包括 GitHub 授权在内的普通 HTTP 链接交给系统浏览器；其他来源和 scheme 不能替换 Session Surface，也不能创建另一个 Electron 窗口。Platform 账号签名密钥和令牌保存在 Electron userData 下的 `safeStorage` 加密文件中。针对每次实际运行的 Platform HTTPS 请求与 Relay WSS 建连，Electron 都会在 attachment deadline 内解析有序系统代理列表。一个捆绑的官方 Node helper 传输加密 Relay frame；另一个单请求 helper 传输 Account、Personal Pairing 与附件 HTTP，因为 Electron 的内嵌 TLS 运行时无法完成这两类实际运行的 TLS 连接。两个 helper 都接受不含凭据的 HTTP CONNECT 候选或 `DIRECT`，只继承证书与临时目录环境字段，在 IPC 处强制 wire 上限，跟随生命周期取消，在所属操作结束前等待退出，并且只对 allowlist connection failure 继续尝试。HTTP helper 接收一个 HTTPS URL、method、有界 body 与该请求的 header，从不跟随 redirect，返回有界 response 后退出；Platform credential 不会保留到该次请求之后。HTTP 204、205 与 304 以 null body 重建，因为 Fetch 禁止这些状态带 body；membership heartbeat 与未绑定的 `by-remote` 都回答 204。Relay helper 不接收 Platform Account、Snow、Session 或应用凭据状态。preload 只暴露当前状态与生命周期动词。Account 的 `beginLogin` 立即返回当前快照，因此 Settings 不会在 Host Account HTTP 期间被堵住。`accountCancelLogin` 会中止进行中的 `beginLogin` POST，取消后不再打开系统浏览器，等到 `shell.openExternal` 静止，丢弃 `pending` 与 `pendingPrivateKey` 并发布 idle；取消之后完成的 poll 不会创建会话。Account、Personal Pairing 与 updater 快照会同时投影到主 Session Surface 和当前 Settings overlay；已销毁或已替换的 overlay 不再接收后续事件。

个人配对只在真实的 `手机配对` 设置区中配置。preload 暴露手机访问、挑战、待确认决策与已配对设备操作，不会向普通 Session 标题栏、侧栏、审批、输入框或离线视图增加状态。每个已配对设备行会展示经过鉴别的 Mobile 名称与平台，以及由 lease 推导的在线状态、配对时间和最后一次经过鉴别的 Relay 访问时间。账号登录后，由 Host 拥有的控制器为每项操作签署新的当前安装证明，在本地创建 XKpsk3 邀请状态，并且只转发不透明 mailbox 消息。确认时，Desktop 为该配对分别创建 Desktop 与 Mobile P-256 credential 以及随机 32 字节 attachment key，只向 Platform 提交 credential 的 SHA-256 公钥 digest，并以第一条 Snow transport payload 密封 Mobile grant 与 attachment key。可恢复确认事务会在重试期间保留同一把 key；active record 将它与 IK reconnect record 分开保存在 `safeStorage` 保护且 owner-only 原子替换的文件。持久化 Relay selector 标识经过鉴别的重连状态和 attachment-key 查找，`PersonalPairingId` 标识记录 ownership 与 release；两者无需相等，加载时遇到重复 active 值会 fail closed。同一个 owner 只在手机访问开启时按配对运行独立的 Desktop credential 与 WSS lifecycle，并在关闭开关、退出账号、sleep、关闭窗口或 quit 时停止。`SnowDesktopAttachmentOwner` 只接纳当前投影且绑定 route/selector/attachment/generation 的 IK 请求。它的 IK2 envelope 会携带 Desktop 的加密 Companion offer，并让原始 Snow channel 保持 pending。Mobile 在下一条 Relay ciphertext 中返回自己的加密 offer；Desktop 此后才会创建协商后的 codec 并发送 `foreground-sync`。必需的正数 `DSH_REMOTE_RELAY_NEGOTIATION_TIMEOUT_MS` 会独立于 Relay attachment 限定第三帧等待时间。超时会取消原始 Snow finalizer、清除 pending negotiation、在不发布 foreground 或应用数据的情况下使 pairing projection 失效，并要求现有 Relay lifecycle 重建。版本不匹配、grant 轮换、撤销、attachment 替换、connection loss 或任一次发送失败也会在应用数据前释放 pending transport，并允许新的 IK 重试。开发环境不选择 keyless 产品控制器。
...
锁定的 Web Host 快照包含一些 Desktop 默认不激活的包。默认配置不设置任何 MCP server；Cordis 自修改与 Code Mode / PTC preset 仍可选择，但都不是默认 preset；standard preset 中的 `subagent_codex` 与 `subagent_claude_code` 模板保持关闭；Web 能力提供 `web_search`，但不提供 `web_fetch`。production HMR 保持关闭。浏览器 `dsh web` 仍让 `session-query-sqlite` 使用 `openAt: never`；Desktop 专用 overlay 则选择 `openAt: first-search`，并把派生索引放在 `DSH_HOME/session-search.sqlite`，因此 Companion 搜索会委托给权威 `session.search`，而不是扫描 Mobile 缓存。Desktop Host 会针对当前 Web Host loopback origin 安装 `DesktopCompanionProductOwner`，在 Web Host 重启时替换该 RPC，并在关闭前移除。该 owner 只暴露 Companion 批准的 Session 与 Workspace 发现、conversation history、prompt、取消、当前 Approval 与 Ask User settlement、attachment 准入、图片读取与搜索 operation；它不会透传任意 Host RPC。Conversation history 与实时替换只接受缺省或字面量 `true` 的 Host assistant `interrupted` 字段，并把 `true` 原样转发给共享 assistant 节点；消费方不从轮次完成状态推导该字段。Surface 发现会保持稳定的 Host 快照，同时让每页选择完整投影不超过 48 KiB wire 上限的最大 Session 前缀；下一游标按实际传输数量推进。超大的 conversation window 会保留最新的完整节点，并通过 `hasMore` 暴露更早历史；如果仅最新节点就超过上限，则只按 UTF-8 截断其中面向用户展示的文本字段。普通响应保留固定的 60 KiB Companion 上限；`session.attachment` 则使用 operated attachment deadline，以及按协议最大摘要校验图片结果配置的独立响应上限。Companion 文件准入经 `AttachmentStore.saveFile` 持久保存确切字节，追加只写入日志的 `session/attachment-admitted` 引用，不会把文件字节或文件名占位文本加入模型历史。同一 operation id 与元数据的重试返回已记录引用；冲突复用会失败。headless、ACP 与 JSON-RPC example 是其他应用组合，不是 Desktop 插件。
...
至少一个已鉴别 Mobile attachment 存活时，Desktop owner 才会消费权威 Host mux 与 Session event stream；第一个 attachment 启动两条 WebSocket，最后一个 attachment 关闭它们。Companion major 4 attachment 通过 `observe-session` 最多选择一个已打开 Session：该 Session 中已经记录的输出会产生有界 conversation 替换，隐藏 Session 的权威变化则只产生摘要、过滤归档项后的位置与 Workspace 归属替换。observation epoch 会从 Mobile 切换或关闭 Session 后才完成的工作中移除 conversation 字节。Workspace 创建、更新、移除、重排与归档 frame 会触发新的权威分页 baseline；归档 Session id 同时过滤该 baseline 与 live replacement。每个分页 cursor 拥有一份冻结的 Host snapshot，不受无关 Desktop 发送 revision 变化影响。同一物理 channel 上的后续 baseline 会保留 Mobile conversation、operation correlation 与已观察焦点；只有 attachment replacement 才会建立新的 receiver generation。pairing-scoped projection source 会在最多包含 32 个 Session 的有界队列内合并同一 Session 的重复变化，Snow owner 会按 channel 单调递增的 Desktop revision 串行发送所有 projection。live conversation 超过 48 KiB projection 上限时，会先丢弃最旧 node，再按 UTF-8 截断单个超大的最新文本 node，保留可用的最新尾部而不触发重连，并设置 `hasMore`，使 Mobile 能从保留的最早 sequence 请求旧 history。Host 替换、stream 失败、projection 失败、队列超限、peer 替换、sleep、关闭功能、退出账号、关闭窗口与 quit 都会在重连或 teardown 前清除 listener 和 pending work。重连会建立新的 generation 与权威 baseline，而不会重放 transport event。

## 开发

`pnpm run typecheck:desktop:contracts-ready` 以不生成产物的方式检查三个独立 TypeScript Program：Desktop 应用、Companion Host fixture 与 Companion browser driver。它只执行检查，不生产运行时产物；被引用包的声明产物必须已存在。即使某个 compiler face 失败，三个 face 也都会执行；普通类型错误和跨项目输出路径诊断仍会阻断。Host fixture 拥有 Host 服务与 SQLite，browser driver 使用实际 Mobile 组件，Host 与 Client 的 `Context.sessions` 声明绝不进入同一个 Program。Fixture IPC 只传递准备命令和断言数据，不替代产品响应。

Suite-local Host fixture 由 `driver.ts` 通过 `fork(..., { execArgv: ['--import', import.meta.resolve('tsx/esm')] })` 从源码启动 `apps/desktop/tests/companion-fixture/host.ts`；assembled browser test 通过 `tsconfig.base.json` 中的源码 alias 解析 `#testing/desktop/*` 与 `#testing/mobile/*`。这条源码路径不消费生成的 JavaScript、声明或包的私有 import map。完整 Client 构建包含 `apps/mobile/tsdown.config.ts`；其 build-after smoke 位于 `apps/mobile/tests/mobile-browse-artifact.e2e.ts`，禁用配置驱动的源码 alias，经 Desktop 私有 import map 把 `#testing/mobile/MobileBrowse` 解析到 `apps/mobile/lib/MobileBrowse.js`，并在 Chromium 中连同生成的 CSS 一起渲染。该 artifact smoke 只覆盖持久化 Mobile browser component，不会把拥有源码的 Host fixture 或 assembled browser suite 变成产物 consumer。
...
- **账号池状态保持隔离** — Desktop 在自己的 `userData` 下生成私有运行时 root，把该运行代用作 core cwd 与 HOME，并传入操作系统环境白名单，而不是继承 Desktop 启动环境中的 storage/provider/credential 变量。相互分离的 management 与 inference key、配置、auth 文件和日志均保留在运行代中。发送 inference key 前，Desktop 会验证自身 spawn 的子进程 PID 拥有所选 loopback 监听。恢复会先发布新端点与 key，再替换 Web Host。关闭仅对该子进程组执行有界优雅终止与强制终止，然后删除运行时 root；它从不读取外部 cwd 的 `.env`、`~/.cli-proxy-api`、Sub2API 数据或外部服务配置。账号池不可用不会阻止 Desktop 其余部分启动。
