// PROTOTYPE — restore the approved Mobile Companion phone shell at real density.
// Question: does the locked list/detail IA hold on a phone-sized viewport?
...
function viewFromUrl() {
  return new URLSearchParams(location.search).get('view') === 'lab' ? 'lab' : 'phone';
}

function scenarioFromUrl() {
  const value = new URLSearchParams(location.search).get('scenario');
  return scenarios.some((scenario) => scenario.key === value) ? value : 'live';
}

function prototypeSearch(overrides = {}) {
  const params = new URLSearchParams(location.search);
  params.set('variant', overrides.variant ?? state.variant);
  params.set('view', overrides.view ?? state.view);
  params.set('scenario', overrides.scenario ?? state.scenario);
...
function scenarioTabs() {
  return `<nav class="scenario-tabs">${scenarios.map((scenario, index) => `<button class="${state.scenario === scenario.key ? 'active' : ''}" data-scenario="${scenario.key}"><small>0${index + 1}</small>${scenario.label}</button>`).join('')}</nav>`;
}
...
function prototypeState() {
  return {
    ...
    mobileRoute: state.mobileRoute,
    mobileSession: state.mobileSession,
...
function render() {
  ...
  document.body.classList.toggle('phone-view', state.view === 'phone');
  if (state.view === 'phone') {
    document.querySelector('#app').innerHTML = `<div class="locked-prototype phone-only">
      <header class="phone-lab-bar">
        <div><span>PROTOTYPE</span><strong>Mobile Companion</strong></div>
        ${scenarioTabs()}
        <div class="phone-lab-actions">
          <a href="${prototypeSearch({ view: 'lab' })}">对照 Lab</a>
          <a href="/concept.html?variant=A">概念稿</a>
          <button class="inspect-button ${state.inspector ? 'active' : ''}" data-action="inspect">${icon('code', 16)}</button>
        </div>
      </header>
      <main class="phone-stage"><div class="phone">${mobileApp()}</div></main>
      ${state.inspector ? `<pre class="phone-state">${JSON.stringify(prototypeState(), null, 2)}</pre>` : ''}
    </div>`;
    bind();
    return;
  }
  document.querySelector('#app').innerHTML = `<div class="locked-prototype">
    <header class="lab-header"><div><span>SCOPE-LOCKED PROTOTYPE</span><strong>Mobile Companion</strong><p>${variant.key} — ${variant.name}</p></div><div><a class="inspect-button" href="${prototypeSearch({ view: 'phone' })}">手机 UI</a><span class="real-badge"><i></i>LIVE DSH · 127.0.0.1:3080</span><button class="inspect-button ${state.inspector ? 'active' : ''}" data-action="inspect">${icon('code', 16)} Scope map</button></div></header>
    ${scenarioTabs()}
...
function bind() {
  document.querySelectorAll('[data-scenario]').forEach((button) => button.addEventListener('click', () => {
    ...
    history.replaceState(null, '', prototypeSearch({ scenario: state.scenario }));
    render();
  }));
