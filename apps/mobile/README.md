...
The bundled Mobile entry keeps list/detail navigation in `apps/mobile` and renders an opened Session from the Client Runtime's Desktop-authoritative `ConversationSnapshot`; it has no Mobile content-block union. The detail route imports only explicit public `./presentation` entries from `ui-conversation`, `ui-tool`, `ui-user-questions`, and `ui-attachment`, plus public `ui-primitives`. Those entries execute the same Markdown, code, image, ordinary and unknown Tool, diff, terminal summary, Approval, Ask User, error, and InputBar implementations as the Desktop Web composition. Mobile supplies navigation, image loading, and prompt/cancel adapters, and never mounts Desktop columns, Settings, model selection, plugin configuration, or terminal input.

Product evidence builds and loads this entry. `apps/mobile/prototype-companion` and development ports 5173/5174 are not Mobile product acceptance origins.
