...
- Remote Companion traffic 与附件 flow 不在此 shell 范围内。`CompanionForegroundRuntime` 把可见性接到真实 Relay 生命周期，且 `settleCompanionInteraction` 要求前台重连与 Desktop 权威同步；原生 APNs/FCM 登记与真机投递仍不在此 shell 范围内。
