...
- Companion Cache 使用开发 AES-GCM 密钥，而不是配对派生的产品密钥。账号页提供「清除此 Desktop 缓存」；配对密钥记录仍留在配对缝。
- `CompanionForegroundRuntime` 是 Relay start/stop 的唯一所有者：配对与可见性共用一条转移队列，一字节 Desktop resync 会调用 `synchronize()`，`unpair()` 会丢掉 grant，因此之后的可见性变化不能再 `start()` socket。无密钥开发以 `mobile-development-keyless` 附着，在存活附着后向 `desktop-development-keyless` 宣告，并仅在 `companionMayMutate` 时发送 Encrypted Companion 的 `create-session`、`open-session`、`submit-prompt`、`cancel-prompt`、`search-sessions`、`offer-attachment`、`settle-approval` 和 `answer-ask-user`。浏览区用 Desktop `session-catalog` 替换列表，用 `search-sessions` / `session-search` 搜索，打开会话时发送 `open-session`，只在 Host 投影之后更新 transcript。命名 Workspace 会在 Web Host cwd 下采纳或创建 Host Workspace 目录。Remote Offline 仍可读缓存行并拒绝作曲器 mutation。附件 offer 复用配对范围的密封辅助函数与开发 capability；Desktop 通过 Host `session.prompt` 排队 `Attached: <fileName>`，不把附件明文放进 Relay。已确认的 Mobile Relay grant 按 Account 保存，刷新后在 activate 时恢复。`settleCompanionInteraction` 要求前台重连与该同步。会话卡片提供 Desktop 已授权的每一个结算选项，并仅在 Desktop 正在 streaming 时显示取消；原生 APNs/FCM 登记与真机投递仍不在此 shell 范围内。
