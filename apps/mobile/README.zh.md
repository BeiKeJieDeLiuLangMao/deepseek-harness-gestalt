...
打包后的 Mobile 入口把列表／详情导航留在 `apps/mobile`，并直接消费 Client Runtime 的 `SessionListState`、`WorkspaceView` 与按 Session id 索引的 `ConversationSnapshot` 权威投影；它既没有 Mobile Session 摘要模型，也没有 Mobile conversation-node router。Session 分组与行执行公共 `ui-workspace` presentation，终态 conversation node 则执行公共 `ui-conversation` keyed presentation。locale 与 light/dark theme 来自 Mobile 产品环境，并进入共享 conversation、Tool、attachment、question 与 Workspace 实现。图片一律通过 `ImageGallery` 渲染；普通 Tool 名称使用 Desktop 内置 keyed roster，只有未知名称使用 `GenericToolCard`。Mobile 绝不挂载 Desktop columns、Settings、model selection、plugin configuration 或 terminal input。

keyless snapshot 证据选择 development 产品环境，以 `VITE_MOBILE_PRESENTATION_EXAMPLE=1` 构建并加载该入口，通过拦截的 HTTPS 响应完成 Account lifecycle，再经同一生产 composition interface 打开仅用于开发的 Desktop 权威投影。它在 390 px 下覆盖英文／dark 与中文／light conversation、图片加载、专用与未知 Tool、Host failure、Approval、Ask User，以及调用本地证据回调的 InputBar 提交。即使该 flag 存在，production 也不会导入此 development projection。该 snapshot 不运行 model round，既不证明加密 Desktop transport，也不证明远端 mutation authority；live 产品证据仍受 #216 与 #217 阻塞。`apps/mobile/prototype-companion` 与开发端口 5173/5174 都不是 Mobile 产品验收 origin。
