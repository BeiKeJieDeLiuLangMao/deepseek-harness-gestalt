...
export class PostgresProjectMembershipPersistence implements ProjectMembershipPersistence {
  ...
  async transact<T>(operation: (transaction: ProjectMembershipTransaction) => Promise<T>): Promise<T> {
    return await this.locked(async (client) => {
      const result = await client.query<{ document: string }>('SELECT document FROM project_membership_documents WHERE namespace = $1', [this.namespace])
      ...
      if (staged !== undefined) {
        ...
        await client.query('UPDATE project_membership_documents SET document = $2 WHERE namespace = $1', [this.namespace, staged])
      }
      return value
    })
  }
  ...
  async importSnapshot(document: string, expectedDigest: string): Promise<boolean> {
    ...
    return await this.locked(async (client) => {
      const marker = await client.query<{ source_digest: string }>('SELECT source_digest FROM project_membership_imports WHERE namespace = $1', [this.namespace])
      if (marker.rows[0] !== undefined) {
        ...
        const retained = await client.query('SELECT namespace FROM project_membership_documents WHERE namespace = $1', [this.namespace])
        if (retained.rows.length !== 1) throw new Error('Membership import marker has no authoritative document')
        return false
      }
      const existing = await client.query('SELECT namespace FROM project_membership_documents WHERE namespace = $1', [this.namespace])
      if (existing.rows.length !== 0) throw new Error('Membership authority is not empty')
      await client.query('INSERT INTO project_membership_documents (namespace, document) VALUES ($1,$2)', [this.namespace, document])
      await client.query('INSERT INTO project_membership_imports (namespace, source_digest) VALUES ($1,$2)', [this.namespace, digest])
      return true
    })
  }
  ...
  private async locked<T>(operation: (client: PoolClient) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1, 0))', [`project-membership:${this.namespace}`])
