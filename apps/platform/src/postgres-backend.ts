...
ALTER TABLE account_attempts ADD COLUMN IF NOT EXISTS mobile_presentation jsonb;
ALTER TABLE account_sessions ADD COLUMN IF NOT EXISTS mobile_presentation jsonb;
`
  ...
  async createAttempt(record: LoginAttemptRecord): Promise<void> {
    await this.pool.query(
      `INSERT INTO account_attempts (
        ...
        mobile_presentation, public_key, state, code_verifier, expires_at, status, identity
    ...
    try {
      ...
      const sessionResult = await client.query<SessionRow>(
        `INSERT INTO account_sessions (
          ...
          mobile_presentation, public_key, revision, active, refresh_hash, refresh_expires_at
        ) VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7::jsonb,1,TRUE,$8,$9)
        RETURNING *`,
        [
          sessionId, attemptRow.identity_namespace, account.id, attemptRow.installation_id,
          attemptRow.installation_kind, attemptRow.mobile_presentation === null
            ? null
            : JSON.stringify(attemptRow.mobile_presentation),
...
function attemptFromRow(row: AttemptRow): LoginAttemptRecord {
  ...
  return {
    ...
    installationKind,
    ...mobilePresentationFromRow(installationKind, row.mobile_presentation),
...
function mobilePresentationFromRow(
  kind: InstallationKind,
  value: unknown,
): { presentation?: ReturnType<typeof parseMobileInstallationPresentation> } {
  if (kind === 'desktop') {
