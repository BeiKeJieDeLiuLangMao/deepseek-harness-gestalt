...
function pairingWriteReferences(state: PersonalPairingTransactionState): Map<string, PlatformAccountId> {
  ...
  for (const record of state.endpointMailbox.challenges) {
    references.set(`mailbox-challenge:${record.challengeId}`, parsePlatformAccountId(record.accountId))
  }
  for (const record of state.endpointMailbox.pending) {
    references.set(`mailbox-pending:${record.pendingPairingId}`, parsePlatformAccountId(record.accountId))
  }
  return references
}
