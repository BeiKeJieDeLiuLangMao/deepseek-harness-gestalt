...
import {
  Config as RemoteAccessHttpConfig,
  apply as applyRemoteAccessHttp,
  inject as remoteAccessHttpInject,
  name as remoteAccessHttpName,
} from '@deepseek-ai/dsh-remote-access-http'
import {
  Config as RemoteAccessRelayHttpConfig,
  apply as applyRemoteAccessRelayHttp,
  inject as remoteAccessRelayHttpInject,
  ...
  try {
    ...
    const relay = new RemoteRelayProvider(context, {
      ...
      config: {
        ...
        directoryTtlMs: config.relay.directoryTtlMs,
        heartbeatTimeoutMs: config.relay.heartbeatTimeoutMs,
        maxBufferedCiphertextBytes: config.relay.maxBufferedCiphertextBytes,
        maxConnections: config.relay.maxConnections,
    ...
    await context.plugin({
      ...
      inject: remoteAccessRelayHttpInject,
      name: remoteAccessRelayHttpName,
    }, {
