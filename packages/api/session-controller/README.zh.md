...
Client apply 会等待生成的 `remote.session`（与 `memberQuestion` 同一轮 Client Remote `$mount`，且更晚），并把 `ctx.receivingQuestions` 登记为 Cordis 服务。该书加载 `memberQuestion.snapshot`，经 `memberQuestion.settle` 结算，并随 fiber 卸载。该轮 mount 之后若缺少 `memberQuestion` 属性，apply 立即失败，既不会永远停在注入等待，也不会跳过 receiving 投影。
