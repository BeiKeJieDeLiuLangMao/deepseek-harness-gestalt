...
Client apply 还会等待生成的 `remote.memberQuestion` 命名空间，并把 `ctx.receivingQuestions` 登记为 Cordis 服务。该书加载 `memberQuestion.snapshot`，经 `memberQuestion.settle` 结算，并随 fiber 卸载。缺少该命名空间会让插件停在注入等待，而不会跳过 receiving 投影。
