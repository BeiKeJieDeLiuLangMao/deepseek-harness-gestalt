...
功能回调返回 `SessionAdmissionResult`：失败分支携带分布式 `code`、`message` 与 `details` 字段，不构造 `RemoteError`。Session Controller 为公共 `RemoteResult` 重建该 Error、保留已有结构标记的 `RemoteFailure`，并把其他回调 throw 归类为 `gateway/internal`；功能 `models` 与 `selectModel` 方法也由同一处归一。
