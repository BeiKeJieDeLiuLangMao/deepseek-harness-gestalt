...
When `memberQuestionReceiver` is composed, Host apply registers the single arrival Session materializer as a Cordis effect. Authenticated ingest creates or continues the receiver-owned Session identity, attaches the bound Workspace, records ignorable `member-question/received` metadata, injects the Decision Brief without starting a model turn, and flushes persistence. Unloading the controller withdraws that materializer so a later Host owner can replace it. Human-turn admission and terminal Session sync remain on the Host composition that still owns those adapters.
...
- Human-turn admission (`admitHumanTurn`) and terminal Session sync retry still live on the remaining Host composition (currently ApiProxy). This package owns only arrival Session materialization.
