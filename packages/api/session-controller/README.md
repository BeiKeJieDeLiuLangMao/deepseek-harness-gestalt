...
The Client apply also waits for the generated `remote.memberQuestion` namespace and registers `ctx.receivingQuestions` as a Cordis service. That book loads `memberQuestion.snapshot`, settles through `memberQuestion.settle`, and unloads with the fiber. Absence of the namespace parks the plugin; it does not skip receiving projection.
