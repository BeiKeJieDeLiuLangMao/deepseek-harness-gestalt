---
description: "提供项目创建、名册、邀请、在线状态与成员管理路由的 Project Membership HTTP Consumer。"
...
通过 HTTP 提供 Project 创建与恢复、成员清单、邀请、成员管理和 Installation presence。每条路由校验有界输入、鉴权 Account，并委托一个角色受控操作，不复制 membership 状态。应答禁止缓存，并将 membership、Account、冲突与容量失败映射为稳定 JSON 状态码和重试元数据。
