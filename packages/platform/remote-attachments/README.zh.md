---
description: "带过期一次性 capability 的配对范围加密 attachment blob store。"
...
无，因为该存储只携带加密 blob 与 consume capability，不创建发往模型的内容。

#### KV Cache 影响

该存储不增加模型请求内容，因此不影响提供方缓存复用。
