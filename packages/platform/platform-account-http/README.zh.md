...
通过 HTTP 提供 Platform Account 登录、GitHub 回调、轮询、刷新、Account 读取和 Installation 注销。路由禁止缓存，校验配置 origin 与有界请求 body，并通过正确的 429 重试元数据返回稳定 JSON 错误。访问 token 操作在专用 header 中携带 branded 单次证明 id。
