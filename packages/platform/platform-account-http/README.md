...
Expose Platform Account login, GitHub callback, polling, refresh, account, and installation sign-out over HTTP. Routes disable caching, validate configured origins and bounded request bodies, and return stable JSON errors with correct 429 retry metadata. Access-token operations carry branded single-use proof ids in dedicated headers.
