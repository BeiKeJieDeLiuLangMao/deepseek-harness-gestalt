...
`PlatformAccountHttpTransport` 只接受从已校验开发／生产环境对中选出的身份，并从 `unknown` 解析每种响应。`IndexedDbInstallationAccountStore` 解析持久化记录，并保存不可导出的 Mobile WebCrypto 密钥与账号会话；Desktop 复用相同传输，但使用 Electron Host 拥有的加密存储。一个 `AccountLifecycleTransitions` owner 串行化加载、登录、轮询、刷新、切换与退出，避免并发恢复清除或复活较新的会话。单个安装切换账号时，`accountStorageNamespace` 为配对密钥、缓存与回执提供按账号和环境隔离的前缀。
