...
通过基于浏览器和签名轮询的流程，将 Desktop 或 Mobile Installation 登录到 Platform Account。客户端显示保留说明、创建 P-256 密钥、准备五分钟尝试、校验已保存会话，并恢复仍有效的待处理登录。Installation 秘密与普通存储隔离；延迟的加载不会覆盖新准备的登录。
