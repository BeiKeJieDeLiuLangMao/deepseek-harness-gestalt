...
The implemented catalog contains a bounded transcript-page projection, a prompt-submission operation, an attachment-offer operation, a Desktop-confirmed result, and an attachment-rejection result. The attachment-offer operation is the bounded control message for encrypted attachment transfer: it carries only a one-time blob capability, the ciphertext SHA-256, the exact ciphertext byte count, the capability expiry, and a bounded file name. Every identifier is branded by this protocol rather than imported from a Harness domain package. Unsupported operations and projection fields fail during decoding.
...
- The current Companion catalog proves prompt submission, attachment offers, transcript projection, and two result kinds; discovery, creation, interaction, cancellation, and operation-receipt messages require later protocol additions before their adapters can expose them.
