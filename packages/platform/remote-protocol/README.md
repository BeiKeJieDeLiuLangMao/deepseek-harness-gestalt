...
Major 3 adds bounded Session and Workspace discovery, complete conversation-page projections, Session history, Workspace-owned or Ungrouped Session creation, prompt submission, cancellation, Approval and Ask User settlement, and content-addressed historical-image reads. Image bytes travel as ordered 32 KiB chunks with one shared digest and at most 512 chunks; a Mobile endpoint accepts them only for the originating operation, Session, attachment, media type, generation, index, count, and digest. The catalog retains attachment offers, authoritative `search-sessions`, reconnect `query-operation-status`, Desktop confirmations, attachment rejections, correlated `session-search`, typed `operation-failed`, and `status` answers carrying the original terminal mutation result or explicit absence. Successful `create-session` returns `session-created` with the branded Host Session id; a committed status answer replays that exact result. Mobile may select the id only for the connection generation that received the result and only after an authenticated Desktop resync contains the blank Session. It retains that selection across surface pages and repeated refreshes until the detail view commits or an authoritative row reports the Session non-blank; a replacement connection clears it. Host failures preserve one of four closed categories: HTTP status, invalid wire response, typed business error, or timeout. `foreground-sync` carries the positive physical-connection generation and Desktop revision after authenticated decryption; a raw byte cannot decode as synchronization authority. Unsupported operations, extra fields, malformed content-addressed attachment ids, and limit overflow fail during decoding.

Major 4 adds `observe-session` and unsolicited `session-live` replacements. One pairing observes at most one open Session. Its replacement may include a bounded conversation only while the operation's observation epoch remains current; every hidden changed Session carries only its authoritative summary, position in the archive-filtered Session list, and Workspace memberships. A removal carries only the Session id. Workspace and archive authority changes request a complete paged baseline, whose cursor is bound to its frozen Host snapshot and physical generation rather than later send revisions. A later foreground baseline on the same physical channel retains conversation, operation correlation, and observed focus; only attachment replacement creates a new receiver and reconciles operations. Each replacement names the physical generation and a monotonically increasing Desktop revision; Mobile applies a revision at most once, ignores older duplicates, and queues at most 32 distinct Session replacements until the paged baseline completes. Same-Session changes coalesce behind one ordered Snow sender. Before encryption, an oversized live conversation drops oldest nodes and then UTF-8-truncates one oversized latest text node until the complete major 4 message fits the 48 KiB projection ceiling. Every truncated conversation sets `hasMore` so Mobile exposes the correlated `load-history` recovery path from the oldest retained sequence. Queue overflow, Host stream failure, invalid bounded output, and projection failure retire the channel so reconnect establishes a new generation and full baseline.
...
| Complete encoded projection message | 49,152 bytes (48 KiB) |
