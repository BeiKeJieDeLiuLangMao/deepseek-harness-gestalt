...
- 当前 Companion catalog 只证明一个 mutation、一个 projection 与 operation-status 查询；discovery、creation、interaction、attachment 和 cancellation 消息必须在后续协议扩展中加入，adapter 才能暴露它们。
- 配对 handshake、凭据持久化、challenge lifecycle、加密 blob capability、token 分发与生产加密属于服务或经评审的 endpoint 集成，不属于这些 codec。
