...
Expose Personal Pairing operations over HTTP and authenticated Remote Relay attachments over one exact WSS path. Routes validate Account proof, bounded operation input, peer-address quotas, and declared blob sizes before delegating through the service. Capacity failures preserve retry metadata; untrusted or malformed connections never enter Relay protocol handling.
...
The HTTP Consumer requires a non-empty exact `origins` Config. Matching standard and custom tuple origins receive their configured value in `Access-Control-Allow-Origin`; paths, malformed values, unconfigured origins, and opaque `null` receive `ORIGIN_DENIED`.
