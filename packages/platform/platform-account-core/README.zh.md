...
通过五分钟 GitHub OAuth 尝试、S256 PKCE、签名轮询 token 和 P-256 证明对 Platform Installation 进行鉴权。该服务只保留公开 GitHub 身份字段、丢弃提供方 token，并在事务内执行实时 Installation 与连接配额。会话通过提供方自有状态轮换、撤销和过期；失败保留稳定的 Account 错误码。
