...
系统先提交终态，再清理提供方资源。挑战、待确认密钥或活跃密钥销毁失败时，资源会保留在可重试清理记录中；客户端重试仍观察原有的完成、确认、取消、拒绝或过期结果，不会重复握手或激活。每个已鉴权安装最多持有四个存活挑战和四个待确认配对；清理完成的幂等重试投影在五分钟后淘汰，清理失败的终态记录则保留到销毁成功。挑战创建时就调度过期任务；提供方释放资源时聚合尝试清理全部活跃资源与保留资源。生成的不透明 id 或密钥引用发生碰撞时会立即失败且不覆盖既有记录；任何已新分配的资源仍由清理记录持有。

`PairingHandshakeProvider` 是唯一的密码适配器。本包不实现 Noise，也不派生配对密钥。每次激活返回公开的带品牌密钥引用和一个独立的提供方私有分配句柄；碰撞回滚只销毁本次新分配，无法寻址既有配对密钥。生成的设备主体只有 `companion-surface` 权限。HTTP 消费方与共用 HTTP 传输把 `ctx.remoteAccess` 连接到 Desktop 设置和 Mobile 控制器。Mobile 会保留一次准备好的完成尝试，直到成功或邀请过期，因此响应丢失后的重试会复用完成 id 与握手字节。两个控制器都会在退出账号或卸载时停止计时器、排空进行中的工作，并拒绝之后的配对操作。loader 示例通过明确标记为未评审的无密钥提供方运行这条控制器／HTTP 路径；它不是产品密码实现。
