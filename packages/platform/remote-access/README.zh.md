...
无内容推送留在远程访问内部。`RemoteAccessService` 拥有 `registerPushToken`、`unregisterPushToken` 与 `publishPushHint`。提供方在这些入口重新解析提示与 token，因此额外字段到不了 outbox 或厂商载荷。`DesktopCompanionPushPublisher` 只在持久待处理或终态提交之后发布，并在该事件层丢弃流式分片。单独撤销按账号与安装删除 token，即使 Desktop route 已经不在。关闭手机访问会删除被撤销 route 上的全部 token。APNs 与 FCM 适配器通过注入的 transport 投影协议载荷，不会追加内容。每天 500 条提示的配额仍由开放注册准入计数器拥有，本扇出不执行该配额。
...
- 推送 token 登记、注销与提示发布是 `RemoteAccessService` 方法。配对 HTTP 消费方尚未暴露登记或发布。HTTP 客户端已发送 `unregister-push-token`；Platform 路由仍暂缓。原生 APNs/FCM 凭据与真机投递仍由部署拥有。
