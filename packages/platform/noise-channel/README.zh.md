...
`SnowPairingHandshakeProvider` 与 `SnowMobileHandshakeClient` 完成 XKpsk3 的全部三条消息。完成后的握手哈希只用于认证词。Relay authority 是已完成握手的首条 responder transport payload，不是由另行拼装的应用密码保护的 JSON。打开或密封该 payload 会销毁邀请 PSK、配对临时密钥与 transcript 状态；每个端点只保留自己独立生成的静态密钥和已认证的对端公钥。

provider 在非粘性 HTTP 状态转换后重建一次短期 XKpsk3 状态，方法是把 Snow 生成且仅使用一次的临时密钥交给 Snow 的 `fixed_ephemeral_key_for_testing_only` API。独立审查必须接受这一确切用法，产品 composition 才能选择该 provider。
...
- Relay peer discovery 与 Mobile attachment owner 已完成组装。首次配对与持久 static state 由 Desktop 拥有之前，Desktop 继续 fail-closed；Platform pairing provider 不会被作为端到端 endpoint 挂载。
