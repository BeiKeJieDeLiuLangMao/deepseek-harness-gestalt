  ...
  async sealMobileRelayAuthority(input: {
    ...
    try {
      const sealed = await sealPairingTransport({
        ...
        plaintext: encodeGrant(input.grant),
      })
...
function encodeGrant(grant: RelayCredentialGrant): Uint8Array {
  return new TextEncoder().encode(JSON.stringify(grant))
}

function decodeGrant(plaintext: Uint8Array): RelayCredentialGrant {
