...
/** Dispatch a focused document by filename extension. */
function documentKind(filename: string): 'markdown' | 'html' | 'file' {
...
function markdownLabels(t: DetailsSlotProps['t']): { code: { copyLabel: string; copiedLabel: string }; footnotes: string } {
  return {
...
/**
 * Focused-document body: the `conversation.details.document` seat over its
 * three-way markdown / restricted-html / bare-file fallback.
 */
      ...
      if (kind === 'markdown') {
        return (
          <section className={css.section}>
            <MarkdownText text={doc.content ?? ''} labels={markdownLabels(t)} />
          </section>
        )
      }
      if (kind === 'html') {
    ...
    <div className={css.root}>
      <div className={css.header}>
        <div className={css.title}>
          {documentFocus?.filename ?? (selection === null ? t('details.title') : material?.name ?? selection.toolName ?? t('details.title'))}
        </div>
        <button
