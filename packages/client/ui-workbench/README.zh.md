...
将 Browser Workspace 页面绑定到官方 Sidebar 的 Browser occurrence。Host 等待保留的设置命名空间、启用浏览器标签和链接拦截，并在激活前 dispose 时彻底取消。Client 复用匹配 Profile 的实例，并记录带初始 URL 的页面创建失败以便重试。
