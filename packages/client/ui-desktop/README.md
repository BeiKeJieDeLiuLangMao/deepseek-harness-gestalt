...
Use this Desktop-only plugin for Session Surface chrome, Mobile Pairing, and Sub2API Settings. The Desktop overlay adds brand, drag, update, Settings, and sidebar controls while keeping pairing keys outside the renderer. Browser `dsh web` does not load it, and account or pairing actions cross only the preload bridge.
