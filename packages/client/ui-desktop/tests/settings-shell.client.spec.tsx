...
describe('Desktop Settings shell Mobile Access placement', () => {
  it('places Mobile Access only in the 手机配对 Settings section', async () => {
    ...
    expect(screen.queryByRole('switch', { name: '手机访问' })).toBeNull()

    fireEvent.click(screen.getByRole('button', { name: '手机配对' }))
    const access = screen.getByRole('switch', { name: '手机访问' })
...
function bridge(account: DesktopAccountSnapshot, pairing: DesktopPairingSnapshot): DesktopBridge {
  return {
    ...
    accountPoolDismissLogin: vi.fn(() => Promise.resolve<DesktopAccountPoolSnapshot>({ state: 'starting', accounts: [] })),
    accountPoolOpenExternal: vi.fn(async () => {}),
    accountPoolSubmitCallback: vi.fn(() => Promise.resolve<DesktopAccountPoolSnapshot>({ state: 'starting', accounts: [] })),
    ...
    accountPoolRefreshAllQuota: vi.fn(() => Promise.resolve<DesktopAccountPoolSnapshot>({ state: 'starting', accounts: [] })),
    ...
    accountPoolPatchFields: vi.fn(() => Promise.resolve<DesktopAccountPoolSnapshot>({ state: 'starting', accounts: [] })),
