...
pi-ai profile 的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，上下文窗口、输出上限、接受的输入标签和思考档位标签收在该行的展开区内，右侧是两个无文字的操作——展开与删除。输入标签写入的就是 `settings.yaml` 里的 `text`/`image` 数组，界面文案跟随页面语言；省略它们（或清空列表）则继承目录，再回退到该路由的 `defaultInput`。思考档位标签写入该模型的 `reasoningEfforts` 字典（`high: high`，`off: null`）；省略它们则继承目录，自定义模型此时不提供思考档位。已存的 `false` 在点选档位之前会保留，并显示单独提示，而不是看起来像未设置。创建卡片和编辑卡片也用同一组标签暴露路由的输入回退。空列表意味着「使用该路由的内置 catalog」，因此每一行都只会被刻意添加；清空容量会丢弃它，而不是存入一个 schema 会拒绝的值，配置留空的部分由适配器的路由级回退值定尺寸——留空的容量以这些回退值的量级作为占位符，那只是提示而非镜像：该字段按 1000 计 `K`，且部署可以覆盖这些回退值。不是正整数的容量根本不会被存下。
...
- **凭据清理范围刻意保持狭窄**：删除一行时，仅当其引用与页面派生的 `<ROUTE>_API_KEY` 目标完全一致，或官方 DeepSeek 的 profile 使用 `DEEPSEEK_API_KEY` 时，才会清除已配置且可写的凭据。自定义引用、环境凭据和无法识别的目标会保留，因为该行无法证明自己拥有它们。官方 DeepSeek 删除后会离开行列表（残留的空用户层分节和该密钥被清掉）。它不会再出现在「添加提供方」里——该列表是 catalog 路由，例如 `deepseek`。官方适配器仍保持挂载。
