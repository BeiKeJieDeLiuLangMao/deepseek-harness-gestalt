...
export const en = {
  ...
  modelInputHint: 'Leave unselected to inherit the catalog, then this provider’s default. Selecting none is the same as leaving it unset.',
  ...
  modelReasoningHint: 'Leave unselected to inherit the catalog. A custom model then offers no thinking levels. Off plus at least one other level is required when any level is set.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  modelAdvanced: '容量',
  modelInput: '接受的输入',
  modelInputHint: '都不选则沿用模型目录，再使用本提供方的默认设置。都不选与未设置相同。',
  defaultInput: '默认接受的输入',
  defaultInputHint: '模型未单独设置时使用。适配器默认只接受文本。',
  modalityText: '文本',
  modalityImage: '图片',
  modelReasoning: '思考档位',
  modelReasoningHint: '都不选则沿用模型目录。自定义模型此时不提供思考档位。若要设置，除「关闭」外至少再选一档。',
