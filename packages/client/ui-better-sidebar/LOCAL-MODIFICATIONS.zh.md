...
11. **`src/client/index.ts`** — Desktop overlay 文档（`data-dsh-desktop-overlay` / `?dsh-desktop-overlay=1`）注册官方适配器，不挂 Host 打开路由或 Side Chat 准入。从不把 snapshot `Sidebar` 挂到 `document.body`。交互 Host 文档拦截 `sessions.open` / `sessions.openSubagent`，使 `Side: ` 目录 child 在普通 owner 上打开或聚焦其官方 Side Chat 标签页。
