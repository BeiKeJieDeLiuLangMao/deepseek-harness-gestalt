...
| 3 `SidebarContext` / Client augmentation / 准入 / `uiRenderer` / workspaces 归档 | **保留**；唯一的 `betterSidebar` Cordis augmentation **迁到** `src/client/context.ts`，并把 `openWorkspacePath` **迁到** `remote.session`。`SessionAdmissionAdapter` type-import `@deepseek-ai/dsh-api-session-controller/client`。`registerAdmissionAdapter` 为必填。 |
