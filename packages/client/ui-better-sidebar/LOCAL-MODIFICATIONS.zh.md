...
25. **`src/client/css-modules.d.ts`** — Host Typert 分析本包时会纳入 Client 半部。`css-modules.d.ts` 声明 `*.module.css` 与 `@xterm/xterm/css/xterm.css`。不声明全局 `*.css` 通配。未使用的 snapshot `layout.css` 副作用样式表省略。
...
9. **官方 Desktop overlay chrome** — 存在 `window.dshDesktop.chromeOverlayShow` 时，官方 Sidebar 的 `+` 菜单在 Desktop 原生 overlay 视图里打开。overlay 设置仍走 Host chrome 的设置席位。未使用的 snapshot `TabBar` / `Sidebar` 壳省略；官方 DockKit 持有标签 chrome。
10. **`src/client/state.ts` / `src/client/sidebar.module.css`** — snapshot store 类型与查看器 CSS 仍供官方 file/runtime/Changes/Side Chat 正文使用。面板推挤 CSS、自动 Files 首页以及未使用的 snapshot 壳省略。
11. **`src/client/index.ts`** — Desktop overlay 文档（`data-dsh-desktop-overlay` / `?dsh-desktop-overlay=1`）注册官方适配器，不挂 Host 打开路由或 Side Chat 准入。从不把 snapshot `Sidebar` 挂到 `document.body`。交互 Host 文档拦截 `sessions.open` / `sessions.openSubagent`，使 `Side: ` 目录 child 在普通 owner 上打开或聚焦其官方 Side Chat 标签页。公开 Client 入口从 `./context.ts` 再导出 `BetterSidebarService`，让 Typert Client catalog 发现能到达 `betterSidebar` Cordis augmentation。
...
仅导入允许路径：`dsh.plugin.json`、`src`、`tsdown.config.ts`。上游 `src/client/sidechat-transcript.ts` **未导入**（退役平行 transcript/轮询渲染器）。locale 字典仍在兄弟文件；新的 `src/client/chunks/locale.tsx` lazy chunk **采用**，`CHUNKS` 含 `locale`。`src/client/changes/` 的 split-sidebar 模块 **采用**。未使用的 snapshot `src/client/sidebar/` 壳省略。
...
| 25 Host 可达 CSS ambient | **仅保留 `css-modules.d.ts`** — `*.module.css` 与 `@xterm/xterm/css/xterm.css`；未使用的 snapshot `layout.css` 省略 |
...
| 9 Desktop overlay `+` 菜单 / windowChrome | **迁到** 官方 Sidebar / DockKit；snapshot `TabBar` 省略 |
| 10 面板缩放 / 不自动 Files 首页 | 随未使用的 snapshot 壳 **省略**；store 类型与查看器 CSS 保留 |
| 11 Desktop overlay 不挂 Sidebar | **保留** — overlay 仍跳过 Host 打开路由、Side Chat 准入与 `Side: ` 目录拦截 |
...
| 24 最后一个停靠标签收起面板 | **迁到** 官方 `ui-sidebar-right`；snapshot-store 测试省略 |
