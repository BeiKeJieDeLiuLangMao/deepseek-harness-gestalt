...
Use the pinned Better Sidebar snapshot to supply workbench capabilities. Its Host half serves sidebar data, media, previews, lazy chunks, and terminal WebSockets behind the trust fence; its Client half registers the retained capabilities on the official Sidebar. Upstream refresh instructions and repository-owned modifications remain documented separately.
...
Pinned source snapshot of [omdsh-dev/DSH-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar). The Host half mounts `/sidebar` JSON, media, HTML preview, lazy-chunk, and terminal WebSocket routes behind the webServer trust fence. The Client entry contributes Files, viewers, Changes, Tasks, Side Chat, Terminal, Browser fallback, settings, and open routing through `ctx.sidebarRight`, `ctx.sidebarRightTabs`, and keyed Slots. It creates no React root, layout store, tab registry, or layout pressure of its own. The shared snapshot mirror is named `SidebarContext` so Host type catalogs cannot mistake it for Cordis `Context`. SHA and refresh steps live in [UPSTREAM.md](UPSTREAM.md). Repository-owned edits are listed in [LOCAL-MODIFICATIONS.md](LOCAL-MODIFICATIONS.md).

Product composition mounts this package and [`dsh-client-ui-workbench`](../ui-workbench/README.md). The workbench package registers the higher-priority Browser Workspace definition and body through the official registry; the sandboxed iframe remains the standalone fallback. Do not edit snapshot sources to change product behavior.

The Client registers one builtin `file` type for canonical `dsh-resource://file/session/...` addresses through `ctx.sidebarRightTabs` and supplies its body through the keyed `sidebar.right.pane.tab` Slot. Every accepted address names a resource Session; the decoder reads the encoded components literally instead of applying WHATWG URL path normalization, so relative dot segments remain part of the resource path. The body resolves paths and performs reads, writes, viewer loads, tree operations, references, and conversation insertion through that Session. The official occurrence's Session remains the display host for replace, new-tab, split, payload, line navigation, tree reveal, editor retention, and mutation reconciliation. System path opens and queued `sidebar_open` requests use the same Session as resource owner and display host, while embedded Side Chat links and produced-file rows retain the child resource owner inside the parent's visible workbench. The tree retains search, reveal, rename, delete, upload, open-with, workspace-fence, and merged or separate editor behavior. Dirty drafts, editor mode, HTML unlock, and scroll state belong to the official occurrence until its signal aborts; a true close asks before discarding a dirty draft ([file host decision](../../../.agents/notes/implemented/feature/2026-09-09-official-sidebar-better-file-host.md), [embedded ownership](../../../.agents/notes/implemented/bug-fix/2026-09-10-side-chat-resource-display-ownership.md)).
...
A fresh Session opens the official guide, where the retained Changes, Tasks, Side Chat, Terminal, and Browser definitions contribute descriptions to cards derived from the same definition inventory as the official `+` menu. Selecting Files opens the same official `file` type used by resource navigation. Member Question material chips open a `fileAddressFor` resource through `ctx.sidebarRight.forSession(sessionId)` with the receiving Session id and a receiver-owned hidden Workspace path; Markdown and sandboxed HTML use the official file host, and a composition without a file type falls through to the Host system opener ([decision](../../../.agents/notes/implemented/architecture/2026-09-03-member-question-files-sidebar.md)).
