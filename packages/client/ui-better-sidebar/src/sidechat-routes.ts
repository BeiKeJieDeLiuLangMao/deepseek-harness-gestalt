...
export interface SidechatRoutes {
  ...
  'sidechat.start'(payload: unknown): Promise<{ childId: SessionId; accepted: true }>
  ...
  /** Wait for creation, report durable publication, and release the live Agent. */
  'sidechat.dispose'(payload: unknown): Promise<{ accepted: true; published: boolean }>
}
...
function updateThreadQueue(agent: Agent, itemId: MessageId, action: SidechatQueueAction): void {
  ...
  const message = messages?.find(candidate => candidate.id === itemId)
...
interface PersistedSidechatSetup {
 ...
 * migration.
 */
async function threadLogEvents(ctx: SidebarContext, childId: string): Promise<readonly SidechatLogEvent[]> {
...
export function buildSidechatApi(ctx: SidebarContext): SidechatApi {
  ...
  const threadSelections = new Map<SessionId, ModelSelectionRef>()
  /** First-prompt operations that a concurrent close must observe to completion. */
  const threadPublications = new Map<SessionId, Promise<void>>()
  ...
  const rawRoutes: SidechatRoutes = {
    ...
    'sidechat.prompt': async (payload: unknown) => {
      ...
      if (agent === undefined) {
        ...
        const persisted = await composePersistedSetup(ctx, childId)
        const selectionRef = threadSelections.get(childId)
          ?? { current: persisted.selection, assembled: undefined }
        try {
          const handle = await agents.resume({
            ...
            setup: withModelSelection(persisted.setup, selectionRef),
          })
          threadDisposers.set(childId, () => handle.dispose())
          threadSelections.set(childId, selectionRef)
          agent = handle.agent
    ...
    'sidechat.model': async (payload: unknown) => {
      ...
      const parentSessionId = typeof (payload as { parentSessionId?: unknown }).parentSessionId === 'string'
        ? SessionId((payload as { parentSessionId: string }).parentSessionId)
        : undefined
    ...
    'sidechat.updateQueue': async (payload: unknown) => {
      const childId = SessionId(requireString(payload, 'childId'))
      const itemId = MessageId(requireString(payload, 'itemId'))
      const agent = liveThreadAgent(ctx, childId)
      if (agent === undefined) {
        throw new SidebarError('queue-item-not-found', 'queued item is no longer pending', 409)
      }
    ...
    'sidechat.permission': async (payload: unknown) => {
      const childId = SessionId(requireString(payload, 'childId'))
      const parentSessionId = SessionId(requireString(payload, 'parentSessionId'))
    ...
    'sidechat.dispose': async (payload: unknown) => {
      const childId = SessionId(requireString(payload, 'childId'))
      await threadPublications.get(childId)
      let published = liveThreadAgent(ctx, childId) !== undefined
      if (!published) {
        const persistence = ctx.get('sessionPersistence') as SidebarSessionPersistenceService | undefined
        if (persistence === undefined) {
          throw new SidebarError('sidechat-error', 'persisted Side Chat lookup is unavailable', 503)
      ...
      if (dispose !== undefined) {
        await dispose()
        if (threadDisposers.get(childId) === dispose) threadDisposers.delete(childId)
      }
      threadSelections.delete(childId)
      return { accepted: true as const, published }
    },

  }
  const routes: SidechatRoutes = {
    'sidechat.start': (payload) => {
      const childId = SessionId(requireString(payload, 'childId'))
      const result = admit(() => rawRoutes['sidechat.start'](payload))
      const publication = result.then(() => undefined, () => undefined)
      threadPublications.set(childId, publication)
      void publication.finally(() => {
        if (threadPublications.get(childId) === publication) threadPublications.delete(childId)
      })
      return result
    },
