...
export const hongKongTraditionalChinese: Record<string, string> = {
  ...
  addPluginsTabDesc: '側邊欄頁面（Tab）可以由插件擴展。插件透過 ctx.sidebarRightTabs 服務註冊；點擊「安裝」複製安裝命令，貼到 DSH 所在環境的終端執行。',
  addPluginsViewerDesc: '檔案預覽器可以由插件擴展。插件透過 ctx.sidebarRightTabs 服務註冊；點擊「安裝」複製安裝命令，貼到 DSH 所在環境的終端執行。',
