...
export function sanitizeState(parsed: unknown): SidebarState | undefined {
  ...
  if (Array.isArray(record.floats)) {
    for (const entry of record.floats) {
      ...
      if (tab === undefined || tab === 'drop') continue
      if (
...
/**
 * Validate one persisted tab record.
 * @returns The clean tab, `'drop'` for a recoverable obsolete record, or
 * undefined when the record is malformed. A malformed float drops only that
 * window, while malformed split-tree data resets the persisted layout.
 */
function sanitizePersistedTab(tab: unknown): SidebarTab | 'drop' | undefined {
  ...
  if (candidate.type === 'diff' || isLegacySeededFilesTab(candidate)) return 'drop'
