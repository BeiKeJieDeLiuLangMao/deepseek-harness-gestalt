...
export function retargetPathTabs(ctx: SidebarContext, store: SidebarStore, oldPath: string, newPath: string): void {
...
export function closePathTabs(ctx: SidebarContext, store: SidebarStore, target: string): void {
  const service = ctx.get('betterSidebar')
  if (service === undefined) return
  for (const tab of pathTabsOf(store.getSnapshot())) {
    const path = tab.path
