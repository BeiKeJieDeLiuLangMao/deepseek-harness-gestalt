...
/**
 * Published chunk directory. Source launch resolves this module from `src/`;
 * the Host bundle resolves it from `lib/index.js`. Both must read
 * `lib/client-<name>.js`, never a sibling of the current file.
 */
const LIB_DIR = join(fileURLToPath(new URL('..', import.meta.url)), 'lib')
