...
describe('bottom panel', () => {
  it('tracks height and removes its track in fullscreen or while closed', () => {
    ...
    expect(store.getSnapshot().layoutInfo).toMatchObject({ bottombarShown: true, bottombarFullscreen: true })
    actions.closeBottombar()
    expect(store.getSnapshot().layoutInfo).toMatchObject({ bottombar: 260, bottombarShown: false, bottombarFullscreen: false })
  })
})
