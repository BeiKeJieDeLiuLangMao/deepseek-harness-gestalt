...
清单把占用设备标为未授权时，已连接内容渲染同一条警示臂（实时流优先于过期清单），并消费 Host `phone-stream` 的同源通道但不 import 它：`POST /phone/session` 铸造签名 H264 与 MJPEG 采集地址，并返回设备类别的首选编码；`/phone/ws/io` WebSocket 承载 JSON-RPC `tap` / `swipe` / `text` / `button`。tap 与 swipe 帧携带活跃采集身份、格式及 live `surfaceSize()` 的 `captureWidth`/`captureHeight`；H264 还携带当前帧精确旋转。Android 设备与 iOS 真机首先使用 H264；iOS 模拟器因 mobilecli 会拒绝该设备类别的 AVC 而直接使用 MJPEG。`PhoneH264Surface` 把 H264 拉取到 canvas；其播放模块根据 AUD 或完整的 primary-picture slice identity 解析未使用 data partition 的 Annex-B access unit，从 SPS 推导 codec，在背压时等待 `VideoDecoder` dequeue，并绘制、关闭每个 `VideoFrame`（遵守 `VideoFrame.rotation` 的 0/90/180/270，90/270 会交换 canvas 轴），把旋转后的显示尺寸作为触控坐标面。当占用 Android 清单行在 Host `GET /phone/devices` 上的 `logicalDisplay`（`dumpsys display` 的 `logicalFrame`，绝不用锁死的 `device.info.screenSize`）宽高数字变化时，`PhoneConnectedView` 把该字段转给 `PhoneConnectionController.noteLogicalDisplay`。live H264 会话经 `refresh` 重新铸造；live MJPEG（首选编码或 H264 fallback）只记录尺寸并保留同一份采集。相同轮询与隐藏或暂停的 tab 不重新铸造。画面框与 tap/swipe 的 x/y 以及 `source.captureWidth`/`captureHeight` 跟随当前采集的 H264 解码显示尺寸。Host 把 Android 采集源坐标映射到当前 `logicalDisplay`；Android 缺少该字段（dumpsys 未命中）时拦截 tap/swipe，按钮与文本仍可用。占用设备平台未确认时 tap/swipe 失败关闭，不当成 iOS。iOS 清单行不要求 `logicalDisplay`。仍为竖屏的解码不会被拉成横屏清单。没有横屏解码时，重新铸造不会填满 native-fallback 信箱像素。H264 拉取、协议、解析、支持、解码、绘制或上游 EOF 失败时，同一份已铸造 session 与 io socket 会切换到 MJPEG URL；当前绘制的 MJPEG JPEG（对 live `<img>` 调用 `createImageBitmap`）成为触控坐标面。Chromium 会把 `naturalWidth`/`naturalHeight` 锁在 `multipart/x-mixed-replace` 的第一帧，因此这两项不是尺寸来源。只有 MJPEG 也失败才进入有界重连。devbar 根据 `PhoneConnectionController` 与 `surfaceSize` 显示连接中、等待已解码首帧、已绘制编码、重连或错误；流契约没有 fps 字段，chip 从不编造实测帧率。画面框跟随实测画面宽高比——H264 旋转后的显示尺寸，或当前 MJPEG JPEG（因 Chromium 在后续 multipart JPEG 翻转方向时不更新 `naturalWidth`，按 500ms 轮询重新测量）——在横竖屏间实时翻转，锁稿的 1:2 比例仅作为首次测量前的占位；`object-fit: contain` 把帧映射进比例一致的框内而不拉伸像素。画面框在面板剩余空间居中，底部保留圆形 返回/主屏幕/最近任务/截图/刷新流 工具条与触控提示行。点击画面发送 tap。按下时捕获 pointer；移动超过 6px 后，按下起点与松开点形成一次语义 swipe（[精确旋转输入](../../../.agents/notes/implemented/bug-fix/2026-09-04-ios-semantic-input-rotation.zh.md)）。合并后的触控板滚动沿纵轴发送同一条 swipe。Pointer 取消时释放捕获且不发送不完整 swipe。可打印字符（Enter 为 `\n`）发送 text；「截图」保持禁用，直到会话附件存储就绪。

连接生命周期收敛在 `PhoneConnectionController`（无 React，占用设备一实例）：铸造 → io 打开 → 首选编码 live → 首选编码为 H264 时同 session MJPEG fallback；`visible: false` 暂停拉流，恢复时重新铸造——签名地址短时效。serial 变化会销毁上一份控制器；socket 中断或 MJPEG 失败进入有界自动重连（3 次线性退避），预算耗尽落到错误卡。`PhoneConnectedView` 在 live、fallback、suspended、reconnecting 与 error 阶段之间保留同一份 H264 播放 owner。surface cleanup 会同步请求关闭 fetch reader 与 decoder；新 H264 URL 或设备只在不会 reject 的 settlement 完成后启动，已过时的排队 replacement 永不启动。每次 teardown 与格式切换都会清空已学习的触控面，因此 tap 会等待当前 renderer 提供 H264 解码尺寸或当前 MJPEG JPEG 尺寸。设备离线（铸造 404 或 io `-32010`）、真机调试未授权（上游报文）与被拒绝（403）会跳过重试。托管会话上 tap / swipe 的 JSON-RPC 错误保持 live，不进入 checking-agent。Host `POST /phone/session` 会在签发前安装可恢复的缺失 iOS 真机 agent，因此打开面板不会先被 `PHONE_AGENT_MISSING` 挡住。`PhoneConnectionController.recoverAgent` 仍用于残留缺失、强制重装与受限失败；已托管会话耗尽画面重试后会复检 agent；已安装但无法产出画面时提供强制重装。`PHONE_REAL_DEVICE_ISSUE` 把设备锁定、证书未信任、profile 过期、tunnel 失败与设备断开分别保留为独立卡。未配置 provisioning 时使用单独的 `PHONE_AGENT_PROFILE_REQUIRED` 卡，引导设置 `provisioningProfilePath`；界面不会宣称自己能够创建签名 identity、profile、Developer Mode、解锁或信任。安装/重装是唯一主按钮，「重新检测」使用次级样式。渲染层只镜像阶段快照；全部决策留在 controller 内，fake gateway 的 spec 逐一证明迁移。

Host 半边注册持久化 `ui-phone` 命名空间（`enabled`，boolean，默认 `false`），并把该 gate 接到 `ctx.phoneEnvironment`。浏览器半边贡献顶层设置分区（`id: phone-devices`，导航标签「手机设备」 / Phone Devices）。方案 C 页面在共享 mobilecli 运行时下方分别显示 Android/iOS 平台卡。Android 卡展示 SDK 组件、来源、下载与磁盘信息、SDK/AVD 根目录、显式许可同意、进度、人工要求、重试和启动操作。iOS 卡在 Host 持有的一键准备期间提供取消入口，被动检测期间不提供；Windows 与 Linux 上会用一段同时说明 iOS Simulator 与 iPhone 真机控制限制的不可用内容替代准备组件和操作。设备清单在两张卡下方保持独立；`PHONE_UNRESOLVED` 的 fleet 拉取只在运行时快照未就绪时渲染「未找到 mobilecli」行，因为就绪快照证明 fleet 处于活动状态，此时落到「无设备」恢复行。如果运行时在该行渲染后变为就绪，已启用的卡片会重新拉取真实 fleet 清单；拉取成功后用当前设备清单替换这条陈旧错误，卡片关闭时则等待再次启用。每个在线设备行都提供主操作「打开面板」；Desktop 设置 overlay 会把所选设备转交给 Session Surface，由后者读取本次选择独享的 Host 清单、等待持久 gate 稳定、重新确认在线设备，再创建或聚焦单例「手机」tab 并展开右侧面板。离线行保持禁用，默认模拟器的启动操作仍归对应平台卡。不提供全局 npm、shell 命令或 `PATH` 步骤。本页不是「移动伴侣」：伴侣是人用手机连桌面，这里是设备被控调试。本包 client face 不 import Host phone 包。
