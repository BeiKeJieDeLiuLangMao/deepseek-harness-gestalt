...
The `./presentation` entry exposes the same `QuestionComposer` behind a small `QuestionPresentation` interface over the Runtime's authoritative `PendingWait<'question'>`. Non-Desktop Web compositions supply only that carrier and the shared question translator; response encoding and receipt checks remain owned here.
