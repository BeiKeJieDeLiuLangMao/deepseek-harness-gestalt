...
The `./presentation` entry exposes `QuestionPresentationView`, the owner-defined narrow question component used by both Desktop `QuestionComposer` and direct Web compositions. Callers supply the Runtime's authoritative `PendingWait<'question'>`, the shared translator, and optional mutation disablement; they do not construct a framework standard kit or fake Session hook. Response encoding and receipt checks remain owned here.
