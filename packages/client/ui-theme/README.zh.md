...
同一组样式表为不挂载动态 theme plugin 的 bundled Web 产品提供稳定 `./styles/*.css` export。暗色 alias 与 Shiki override 会在任意 `data-ds-dark-theme` owner 下生效，因此窄屏产品 root 可以限定自己选择的主题，而无需修改 Desktop 拥有的 `body`。
