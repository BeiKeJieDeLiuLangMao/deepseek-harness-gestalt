...
- **编辑是官方正文的可选子能力。** 官方标签页仍是唯一的文件加载与渲染器选择所有者。贡献的 keyed 编辑器只有在分页读取到达 EOF 后才能编辑纯文本、Markdown 或代码，因此不会把前缀误存为完整文件。写入成功后，所有者回到预览并通过 `workspaceFiles.read` 从第 1 行重新读取；超限或未完整加载的源码保持只读，并显示原因。目录地址仍以 `not-regular-file` 失败。
