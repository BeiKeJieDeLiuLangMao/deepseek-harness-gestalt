...
- **编辑是官方正文的可选子能力。** 官方标签页仍是唯一的文件加载与渲染器选择所有者。贡献的 keyed 编辑器只有在分页预览到达 EOF 后才可用于纯文本、Markdown 或代码，随后通过现有有界 `workspaceFiles.readAll` 端点取得逐字节准确的 UTF-8 文本；编辑源不会由显示页重建，因此保留 CRLF、文件末尾终止符和尾部空行。完整读取失败、无效 UTF-8/NUL 内容或版本变化都会让预览保持可见并说明原因，且不会写入文件。写入成功后，所有者回到预览并通过 `workspaceFiles.read` 从第 1 行重新读取。目录地址仍以 `not-regular-file` 失败。
