...
- **Editing is an optional official child.** The official tab remains the only file loader and renderer selector. A contributed keyed editor is offered for plain text, Markdown, and code only after paged preview reaches EOF, then receives byte-exact UTF-8 text from the existing bounded `workspaceFiles.readAll` endpoint; this preserves CRLF, a final terminator, and trailing empty lines instead of reconstructing source from display pages. A full-read failure, invalid UTF-8/NUL content, or a version change leaves preview active with a visible explanation and never writes the file. After a successful write the owner returns to Preview and rereads from line 1 through `workspaceFiles.read`. Directory addresses still fail with `not-regular-file`.
