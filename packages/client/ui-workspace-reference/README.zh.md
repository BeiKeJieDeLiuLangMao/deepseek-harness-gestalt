...
Web 上名为 `workspace` 的 `@` source。候选来自 Host 工作区引用索引 Remote，并在浏览器内排序。选中后插入纯文本 `@rel/path`（目录保留尾斜杠）。Host 的 pre-step 注入仅表示存在性的标记。picker 排序含有源自 [omdsh-dev/dsh-at-file](https://github.com/omdsh-dev/dsh-at-file) 0.6.3（MIT）的部分；见 [NOTICE](NOTICE)。决策记录：[工作区引用 Agent Note](../../../.agents/notes/implemented/feature/2026-08-19-workspace-reference.md)。
