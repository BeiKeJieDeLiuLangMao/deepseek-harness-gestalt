...
- **类型**：`ctx.sidebarRightTabs.register(...)`，kind 为 `files`，id 为 `@deepseek-ai/dsh-client-ui-sidebar-files`，档位 `builtin`，没有 patterns，order 为 10，并带一个 `files` 图标 token。它的引导元数据只提供描述；引导页从同一个 definition 读取卡片的顺序、标题与图标。
