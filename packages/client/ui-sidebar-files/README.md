...
- **The type** — `ctx.sidebarRightTabs.register(...)` with kind `files`, id `@deepseek-ai/dsh-client-ui-sidebar-files`, band `builtin`, no patterns, order 10, and one `files` icon token. Its guide metadata supplies the description; the guide derives the card's order, title, and icon from the same definition.
