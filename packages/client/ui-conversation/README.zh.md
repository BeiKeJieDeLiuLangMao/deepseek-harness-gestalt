...
聊天视图保留工具的消息流位置，但委托其展示。每个已排序的 `tool-call` Conversation Node 都通过 `conversation.chat.node` 的同名 key 分发；详情壳层则通过 `conversation.details.tool` 传递当前选中的调用。组装后的 Web bundle 为该 Chat Node key 注册 [`ui-tool`](../ui-tool/README.md)，由后者渲染运行时已投影的递归 root/child 树，并负责按名称分发、通用展示和 render-intent 卡片；只有详情席位会在该 renderer 缺席时保留 raw-result fallback。点击 `browser_*` 卡片会调用 `openDetails`：打开以原始形式显示已选中调用的详情，并在该标签页仍列在列表中时用列表修订号聚焦它，且不写入 `dockOpen`（[决策](../../../.agents/notes/implemented/feature/2026-08-20-chat-browser-tool-focus-dock.md)）。
...
无。该包是浏览器会话 UI；编译后的注释文案只会通过 Host 接纳的普通 `user/message` 到达模型。
...
- **注释草稿持久化保持在浏览器本地**：整份草稿（含问题文本，按会话一份 store 值）可在刷新与会话切换后恢复，跨标签页为确定性的后写覆盖且不做实时同步。未发送标注点所需的暂存图片字节持久化在 IndexedDB 中。
...
- **普通工具行仍然没有详情入口**：只有 `browser_*` 卡片会调用 `openDetails`。该手势会打开以原始形式显示已选中调用的详情，并在标签页仍列在列表中时聚焦它，且不写入 `dockOpen`。没有 Input/Output/Metadata 切换、Prev/Next 步进，也没有 trajectory 深链接。
