...
The package registers the optional-Session `conversation` shell, strict Session header/body entries, View list, composer chain and bar, input regions, Hero regions, queue dock, draft persistence, and phase calculation. `ctx.uiSession.provide()` materializes the Conversation and input sources from the same Session binding and supplies `inputActions` as a stable standard prop. The public presentation seam renders user and steering `ContentBlock` images through the Session-authorized `renderMessageImages` adapter and keeps unknown blocks on the JSON fallback. Shared Approval settlement exposes allow-once and reject through the wait's `answer` callback, keeps pending clicks from reentering, and surfaces transport failure for retry. Optional `snapshot` is accepted for Mobile call-site compatibility; command detail remains the Desktop `conversation.approval.detail` slot. The standalone composer owns a local draft undo/redo ring for its controlled textarea (Desktop SessionInput keeps Lexical history); a successful send cuts that ring so the sent draft cannot resurrect.
...
The package occupies the root-scoped `main` key `conversation`, whose wrapper declares the optional-Session `main.conversation` shell. `ConversationPresentationOwnerProps` lets another occurrence mount that content in compact `sidechat` mode, retarget descendant navigation, and name the Session whose workbench displays resources. The rendered Session remains the Conversation and resource owner; `ConversationDisplayOwnerProps` carries only the display host through the strict Session body to the selected View. An ordinary main occurrence omits these optional values. The package also registers the strict Session header/body entries, View list, composer chain and bar, input regions, Hero regions, queue dock, draft persistence, and phase calculation. `ctx.uiSession.provide()` materializes the Conversation and input sources from the same Session binding and supplies `inputActions` as a stable standard prop ([decision](../../../.agents/notes/implemented/bug-fix/2026-09-10-side-chat-resource-display-ownership.md)).
