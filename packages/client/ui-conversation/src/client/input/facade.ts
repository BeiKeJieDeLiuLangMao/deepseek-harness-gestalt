...
export class SessionInputShell implements SessionInput {
  ...
  submit(mode: InputSubmitMode = 'queue'): void {
    if (this.annotationReservation !== undefined) return
    if (this.snapshot.draft.trim() === '' && this.attachmentIds.length === 0 && this.annotations.length === 0) return
    if (this.annotations.length > 0 && this.snapshot.draft.trim() === '' && this.attachmentIds.length === 0) {
  ...
  private sinkSerialized(
    ...
    if (occurrences.length === 0) {
      this.settleSink(attempt, this.deps.defaultSink(compiled, attachmentIds, mode, attempt.signal))
      return
    }
    const inputTriggers = this.deps.inputTriggers?.()
