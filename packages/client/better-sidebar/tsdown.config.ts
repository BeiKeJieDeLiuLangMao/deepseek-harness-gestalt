...
/**
 * Emit published declarations without joining tsconfig.client.json. The
 * snapshot references host Context packages, so a client-aggregate project
 * would merge host and client `sessions` / `loader` keys.
 */
function emitSnapshotDts(): BuildPlugin {
  return {
    name: 'dsh-better-sidebar-dts',
    buildStart() {
      if (snapshotDtsEmitted) return
      ...
      if (result.status !== 0) {
        throw new Error(`better-sidebar dts emit failed:\n${result.stdout}${result.stderr}`)
      }
    },
  }
}
...
/**
 * Host pass emits the Node library; Client pass emits the Node library plus
 * the browser factory and lazy chunks.
 * @param inlineConfig - tsdown workspace env, including DSH_BUILD_FACE.
 * @returns selected configs for the current build face.
 */
export default ({ env }: { env?: { DSH_BUILD_FACE?: string } }): UserConfig[] => {
  const face = env?.DSH_BUILD_FACE
  if (face === 'host') return [nodeLibrary]
  return [nodeLibrary, ...clientBundles]
}
