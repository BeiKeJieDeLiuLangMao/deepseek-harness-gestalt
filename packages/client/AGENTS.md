...
Three declarations read like dependency edges and none is interchangeable: Cordis service `inject`, module-graph `external`, and `dsh.client.inject` — the informational package-name edges described by the [client plugin cookbook](../../docs/cookbook/adding-a-client-plugin.md).
...
## Creating client packages and components

Use the [client package and component cookbook](../../docs/cookbook/adding-a-client-plugin.md) when creating a package or component. This standing file retains the architecture and safety rules that apply to every client edit; the cookbook owns the conditional scaffolding sequence.
