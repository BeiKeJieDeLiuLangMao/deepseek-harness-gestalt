...
`ReceivingQuestionBook` is the only Client pending owner. It loads generated `memberQuestion.snapshot`, settles through `memberQuestion.settle`, and refreshes on `member-question-receiver/changed`. The dock reads that book's `PendingQuestion` and terminal records; React never receives `ctx`. A failed settle keeps the same `PendingQuestion` answerable so drafts remain.
...
- **Receiving Session faces stay session-controller-owned** — `ReceivingQuestionBook` maps Host snapshot rows onto `PendingQuestion`. This package does not keep a second Remote ledger.
