...
`ReceivingQuestionBook` still owns receiving Session faces and `PendingWait`. This package's apply fiber consumes generated `ctx.remote.memberQuestion.snapshot` / `settle` and `member-question-receiver/changed`; React never receives `ctx`. Settlement failures retain the request as a retry draft. Mapping Host snapshot rows onto `PendingWait` remains session-controller-owned and is not closed here.
...
- **Receiving Session faces stay session-controller-owned** — this package loads and settles through generated Remote, but does not mint `PendingWait` or replace `ReceivingQuestionBook`. Decision Brief rendering still reads the conversation snapshot the controller projects.
