...
`ReceivingQuestionBook` is the only Host snapshot owner. It loads generated `memberQuestion.snapshot`, settles through `memberQuestion.settle`, and refreshes on `member-question-receiver/changed`. The dock reads JSON pending rows and terminal records and declares `question.presentation` with those questions plus Host answer and cancel callbacks. `PendingQuestion` and drafts stay in ui-user-questions. A failed Host settle leaves the JSON row and the QuestionComposer drafts.
