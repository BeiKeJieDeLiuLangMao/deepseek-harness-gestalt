...
`ReceivingQuestionBook` 仍拥有 receiving Session face 与 `PendingWait`。本包 apply fiber 消费生成的 `ctx.remote.memberQuestion.snapshot` / `settle` 与 `member-question-receiver/changed`；React 从不接收 `ctx`。settlement 失败会把请求保留为可重试 draft。把 Host snapshot 行映射成 `PendingWait` 仍由 session-controller 拥有，本切片未闭合。
...
- **Receiving Session face 仍由 session-controller 拥有** —— 本包经生成 Remote 加载与结算，但不铸造 `PendingWait`，也不替换 `ReceivingQuestionBook`。Decision Brief 渲染仍读取 controller 投影的 conversation snapshot。
