...
Settings section `id: 'browser'` under namespace `ui-browser` holds a named persistent Profile roster and the default create identity (`shared` / `temporary` / `persistent`) that `browser_create` and sidebar `+ → Browser` use when the model or user omits `profile`. Roster names are partition keys: renaming one updates the roster and follows the persistent default when that name was selected. The page does not create Browser Workspaces or migrate Chromium partition data.
