...
The collapsed preview is a one-line layered summary of the same Dock, not a second Dock. It has no outer shell or footer. Clicking a back layer focuses that tab with its listed revision; clicking the current layer opens the Dock. A listed-revision `BROWSER_REVISION_CONFLICT` on a background chip observes that tab once and retries, or shows the failure. The preview is hidden while the Dock is visible. Ordinary MCP tool rows stay in conversation history.
