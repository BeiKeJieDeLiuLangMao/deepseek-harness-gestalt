...
The `./presentation` entry exposes `ToolPresentation` for compositions outside the Desktop slot tree. Its small interface accepts one authoritative `ToolCallBlock`, Workspace/home display roots, optional open/inspect callbacks, and the shared translator; the implementation reuses `ToolCallTree` and `GenericToolCard`, including recursive subcalls, unknown-tool fallback, and terminal, diff, read, search, and Web render intents.
