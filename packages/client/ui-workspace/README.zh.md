...
- **按工作区解析的云项目 Git 仍是 Host 缺口（#590）**：浏览区行菜单第一项始终是**工作区设置**，有 membership client 时会轮询待处理邀请；但 `createProject({ localWorkspaceId })`、`projectForWorkspace`、`localRemoteFor`、`cloneWorkspace` 尚无 Host 动词。注入的 gateway 会拒绝这四个调用，而不是编造空操作。
