...
- **Workspace-keyed Cloud Project Git is still a Host gap (#590)** — the browsing region always offers **Workspace settings** as the first row-menu item and polls inbound invitations when a membership client is present, but `createProject({ localWorkspaceId })`, `projectForWorkspace`, `localRemoteFor`, and `cloneWorkspace` have no Host verbs yet. The injected gateway rejects those four calls instead of inventing a no-op.
