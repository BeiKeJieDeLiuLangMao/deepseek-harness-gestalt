...
成员提问接收状态仍暴露 leftover `memberQuestion.snapshot` 与 `memberQuestion.settle` RPC，以及完整的 `host/member-question-snapshot` 基线／变更帧。生产 Client 的 snapshot 与 settle 使用生成的 `memberQuestion` Remote。仍有 Host 实现的剩余 carrier 方法是 `workspaceBinding`、`ensureWorkspaceBinding` 与 `bindWorkspace`；树内调用方只有 ApiProxy 测试与 runtime FakeApiClient。认证到达与人工轮次准入属于 Session Controller 与生成的 `memberQuestion.admitHumanTurn`。服务缺失、tuple 陈旧、Workspace 无法解析或 settlement Installation identity 缺失时，网关都会拒绝，不会虚构状态。可选的 Installation id 与设备名配置字段仅用于开发，必须同时配置且非空；生产组合在跨机器 settlement authority 暂缓期间不提供这些字段。存在已认证的当前 Installation id 时，帧会携带它，让各 Client 根据 canonical terminal winner 派生 answered-elsewhere 呈现。
