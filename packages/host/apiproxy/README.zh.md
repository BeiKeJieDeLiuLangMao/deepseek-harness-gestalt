...
成员提问接收状态使用精确的 `memberQuestion.snapshot`、`memberQuestion.settle` 与 `memberQuestion.admitHumanTurn` RPC，以及完整的 `host/member-question-snapshot` 基线／变更帧。认证到达会把 receiver Account 与 Project 解析为唯一的本地 Workspace，以 opaque receiving id 复用 durable Host Session id，关联 Workspace，记录可忽略的 `member-question/received`，并注入每条有界 brief，但不启动模型 turn。admission RPC 在该 Session 已存在后按 envelope `rpcId` 只 prompt 一次。terminal snapshot 变化会把对应的可忽略 `member-question/settled` 事件追加并 flush 到每个已 materialize 的 Session，Host 重启恢复时也会执行。同一 reservation 在失败后重放时会从 Session log 恢复，不会产生第二个 Session、record、message 或 prompt。服务缺失、tuple 陈旧、Workspace 无法解析或 settlement Installation identity 缺失时，网关都会拒绝，不会虚构状态。可选的 Installation id 与设备名配置字段仅用于开发，必须同时配置且非空；生产组合在跨机器 settlement authority 暂缓期间不提供这些字段。存在已认证的当前 Installation id 时，帧会携带它，让各 Client 根据 canonical terminal winner 派生 answered-elsewhere 呈现。
...
会话日志 ZIP 下载不是 ApiProxy RPC。`@deepseek-ai/dsh-session-log-export` 在 Connection 上拥有 `GET`/`HEAD /api/session.export`。
...
`session.fork` 将可选事件锚点映射到该锚点处或其后的首个 `turn/end`，使消息操作可包含该消息所在的完整轮次。锚点省略或超过末尾时，选择最后一个已完成轮次；若锚点已在日志中，而其所在轮次仍开放，则返回 `fork-unavailable`，不会向较早位置裁剪。发布前，宿主先把连续的源日志前缀交给 `clearGoalFromForkSeed`，再以继承的历史、cwd、日志中最新的 `ModelSelection` 和谱系原子地创建子会话；`header.seedLength` 仍是源前缀长度，因此尾部目标墓碑属于子会话，源会话保留自己的目标。发布后的子会话再加入源 Workspace。如果附加到 Workspace 失败，`workspace-attach-failed` 会携带已发布的子会话 id，供客户端对账。[SessionStore fork 决策](../../../.agents/notes/implemented/feature/2026-06-30-session-store-fork-api.zh.md)记录了为何锚点要映射到该 `turn/end`。

会话模型选择属于会话领域约定。`session.models` 将当前 `ModelSelection` 与按提供方分组的建议性模型、精确模型的推理元数据和逐提供方查询失败记录分开返回。该选择可能不在这些分组中，也绝不会作为合成行注入；客户端可以提示用户作出另一项选择，而无需把目录变成路由白名单。`session.selectModel` 校验由适配器持有的可选推理强度，并指定下次组装提示词时使用的完整选择。live Agent 把切换存在进程内；没有 live Agent 的 session-backed child 会追加一条自有 `request/header` 而不 resume，下一次 Activation 会先读这条 header，再读创建时的 descriptor。目录成员关系不构成校验：适配器可以解析未列出的模型，而不可用的提供方或不受支持的推理强度会返回 `model-unavailable`。`session.models` 还会报告 `routable`，即当前是否有适配器为所选提供方提供服务。该值刻意不从分组推导，因为适配器可以服务未公布的模型。`session.prompt` 会依据同一事实，在开启轮次之前以 `model-unavailable` 拒绝；客户端禁用 composer 只是提示性设计，这个方法始终可被调用。

`session.toolEligibility` 返回一个实时 Session 的正向配置并集和精确的合格 schema。`allow` 缺失表示未启用 allow-only 策略；空数组表示组合显式不允许任何末端工具。该目录来自模型请求组装和分发共同使用的同一份 `ctx.tools` 视图，因此 Host API 不会宣告 Session 实际无法调用的 schema。读取冷 Session 前会先经普通 agent 解析路径恢复它。
...
Workspace Git 操作留在 Host。`workspace.gitRemote({ workspaceId })` 与 `workspace.cloneGit({ remoteUrl, parentPath, directoryName })` 经由受管理的 subprocess tree 服务运行，使用凭据清理后的环境白名单、非交互 Git 配置、有界输出、调用方 cancellation，并等待整棵进程树退出。Clone 会在 Git 启动前原子创建原先不存在且经过校验的子目录；clone 或 Workspace 注册失败时，只会在报告失败前移除本次操作拥有的目标。`memberQuestion.workspaceBinding` 将精确 Account／Project association 分类为 missing、live 或 stale，且不替换它。`memberQuestion.ensureWorkspaceBinding` 通过 compare-and-bind 原子保留 live association，或创建／修复 missing／stale association；并发恢复不能替换第一个 live winner。Ensure 与显式 `memberQuestion.bindWorkspace` 的校验／提交和 Workspace registry 的 create／delete 共用 mutation order，因此 candidate 不会在 live check 与持久 binding 之间消失。Receiving-session materialization 要求该 binding service 存在，失败时关闭，而不会按 display title 猜测。
