...
/**
 * Hosts that serve Moonshot/Kimi dedicated search (`POST` the configured URL as-is
 * with `text_query`). A path that still contains `/anthropic` stays on DeepSeek
 * Messages even on these hosts.
 */
 ...
 *
 * `/anthropic` in the path always selects DeepSeek Messages (`POST {baseURL}/messages`).
 * Otherwise a known Moonshot/Kimi host, or a path that ends in `/search`, selects
 * Moonshot dedicated search (`POST` the URL as-is). Every other parseable URL,
 * and any unparseable value, stays on DeepSeek Messages.
 *
 * @param baseURL - the provider section's endpoint, which may be a DeepSeek
 *   Anthropic base or a full Moonshot search URL.
...
export function classifySearchEndpoint(baseURL: string): SearchWireFormat {
  ...
  if (path.includes('/anthropic')) return 'deepseek-messages'
