...
由 [DeepSeek](https://deepseek.com) 支持的 `WebSearchProvider`，用于 harness [web 能力 seam](../web/README.md)（`ctx.web`）。默认端点调用 DeepSeek 的 **Anthropic 兼容 Messages API**（`POST {baseURL}/messages`），启用原生 `web_search_20250305` 服务器工具，并把 DeepSeek 返回的结构化 `web_search_tool_result` 块映射为 seam 规范化的 `WebSearchResult`。若配置的是 Moonshot／Kimi 专用搜索 URL，则按原样 `POST` 该 URL，请求体为 `text_query`，并从 `search_results[]` 映射结果。
...
| `baseURL` | `https://api.deepseek.com/anthropic/v1` | 搜索端点。DeepSeek Anthropic 基址会追加 `/messages`。Moonshot／Kimi 专用搜索 URL 按原样 POST（需填写完整 URL，例如 `https://api.moonshot.cn/v1/search`）。缺省时回退到任一环境层中的 `$DEEPSEEK_SEARCH_BASE_URL`；禁止复用属于 chat-completions LLM 适配器的 `$DEEPSEEK_BASE_URL`。无法解析时提供方不可用。 |
...
两种协议格式返回的提供方生成答案均不被该提供方信任为 `content`，因此省略 `content`。在 DeepSeek 上，`sources[]` 来自 `web_search_result` 条目，这些条目位于 `web_search_tool_result` 块内：`url` ← `url`、`title` ← `title`、`publishedAt` ← `page_age`。`cited_text` 条目按 URL 标识，单独位于文本块的 `citations[]` 中；提供方会按 URL 将它们关联到相应结果，没有摘录时省略 `snippet`。在 Moonshot 上，`sources[]` 来自 `search_results[]`：`url` ← `url`、`title` ← `title`、`snippet` ← `snippet`、`publishedAt` ← `date`。抓取到的 `content` 会被丢弃。

结果按 URL 去重，因为一次请求可能在多次搜索中呈现同一页面。DeepSeek 公开 `maxUses` 而非结果数量旋钮，因此 seam 会强制执行 `maxResults`：截断 `sources[]` 并设置 `truncated`。Moonshot 会从 `maxResults` 接受 `limit`（1–20，默认 5）；若提供方返回更多结果，seam 仍会截断。

提供方失败变为 `WEB_PROVIDER_ERROR`；调用方取消变为 `WEB_ABORTED`。HTTP 重定向会在接触 `Location` 目标前被拒绝，并以 `WEB_PROVIDER_ERROR` 呈现。

## 请求日志

由 agent 发起的 DeepSeek Messages 搜索会在发出请求前一刻，向相应会话追加仅用于日志的 `web/deepseek-search-llm-request` 会话事件。其中包含已解析端点、API 版本，以及发送给 DeepSeek 且不含密钥的精确 JSON 请求体；不包含标头和凭据。发出请求前发生凭据处理失败或取消时不会创建事件；发出请求后才发生 HTTP 或响应失败时，本次请求尝试仍保留持久记录。Moonshot 专用搜索是检索而非辅助模型轮次，因此不写入会话事件。在 agent 之外通过程序直接调用提供方时，没有发起会话可供记录。
...
- **一次 DeepSeek 搜索需要完整的 Messages 模型轮次**：会产生延迟与生成 token，并且最多执行 `maxUses` 次服务器侧搜索；DeepSeek 不公开专用检索端点。Moonshot 专用搜索是检索，不会产生该辅助轮次。
...
- **超量返回的 DeepSeek 源仍消耗 token**：DeepSeek 协议没有结果数量旋钮，`maxResults` 只能由 seam 在事后截断。Moonshot 会接收最高 20 的 `limit`。
- **未引用的 DeepSeek 结果没有 `snippet`**：只有 `text` 块中的引用（`cited_text`）匹配其 URL 时，DeepSeek 源才会获得 snippet。Moonshot 源使用专用的 `snippet` 字段。
- **Moonshot `moonshot_fetch` 不在范围内**：Gestalt 默认禁用 `web_fetch`；本包只适配搜索。
