...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-desktop AccountPoolControl id \'sub2api\'',
  ...
  {
    key: 'sidebar.right.tab.document.editor',
    kind: 'keyed',
    scope: 'session',
    summary: 'Optional renderer-specific source editor controlled by the official document owner.',
    doc: 'Optional renderer-specific source editor controlled by the official document owner.',
    registerOptions: [
      {
        name: 'key',
        requirement: 'required',
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), none are taken yet',
    hookContext: 'UseSidebarRightTabInfo',
    slotInject: '{ hooks: { tabInfo: SlotHookFactory<\'sidebar.right.tab.document.editor\', UseSidebarRightTabInfo> } }',
    declaredBy: 'an entry in \'sidebar.right.pane.tab\' (client-ui-sidebar-documentpreview), so it exists while that entry is mounted',
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidebar.right.tab.document.editor\', () => ctx.slots.register(\n      { name: \'sidebar.right.tab.document.editor\', key: \'<one key the owner dispatches>\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-sidebar-documentpreview/src/client/document/contract.ts:43',
  },
  {
