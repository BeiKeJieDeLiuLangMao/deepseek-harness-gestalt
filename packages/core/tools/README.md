...
- `ctx.tools.schemas(scope?: ScopeKey): ToolSchema[]` Host catalog of everything the scope can see (without the `execute` functions), including deferred definitions and `tool_search` when it is usable. Model-request assembly applies the separate durable-discovery filter. The shipped tools' schemas are catalogued in [docs/tool-catalog.md](../../../docs/tool-catalog.md), generated by booting each tool plugin and harvesting this method (see [the tool-schema-catalog Agent Note](../../../.agents/notes/implemented/process/2026-07-02-tool-schema-catalog.md)).
...
- `ToolDefinition` — `ToolSchema` + mandatory `output { schema, render, presentationMeta? }` + `execute(args, exec)`, optional `deferLoading`, final-content and presentation callbacks, cooperative `timeoutMs`, and optional per-call `isConcurrencySafe(args)` classification. A deferred definition remains in the Host catalog but stays out of model requests and Code Mode bindings until durable history discovers it. A body returns only the canonical JSON value declared by the output schema and cooperatively stops through `exec.signal`. `finalizeContent(exec, result)` runs exactly once for every normalized result, including failures that bypass post-policy, and can replace only `content`; it must be synchronous and total.
...
- `ToolRunContext` — the execution passed to a tool body, extending `ToolExecution` with `deferContext(context)` and `discoverTools(schemas)`. The former defers one context until the tool's final result reaches the loop; the latter attaches schema discoveries to a successful result without mutating the registry.
- `ToolExecutionResult` — discriminated execution-local outcome. Success is `{ isError:false, value:JsonValue, content, meta?, additionalContexts?, discoveredTools? }`; failure is `{ isError:true, error:{ message, info? }, content, meta?, additionalContexts? }` and has no value or discoveries. Call identity stays on the immutable `ToolExecution`. The registry snapshots, validates, and freezes the canonical value before rendering, then materializes the durable presentation fields before final observation. `ToolFailure.info` carries an internal `{ name, code }` for a `HarnessError`; `additionalContexts` preserves every deferred or post-execute identified `UserMessage` for the loop's post-result FIFO, while `discoveredTools` persists loadable schemas for request reconstruction.
...
In normal mode the model sees each eager visible definition's exact name, description, and JSON schema; the shipped definitions are recorded in the generated [tool package map and schema sections](../../../docs/tool-catalog.md#tool-package-map). Eligible deferred definitions are represented by `tool_search` until one of its durable results discovers their schemas. Agent-scoped restrictions, shadows, allow-only eligibility, and extension registrations change that agent's end-tool set before search and again before every reuse.

#### Token effect

Fixed per-request cost proportional to eager and previously discovered definitions, plus the search transport while deferred candidates remain. Restrictions that hide tools remove their entire schema cost for that agent.
...
The loop retains model-emitted arguments and the registry's final content. A successful `tool_search` result also retains the discovered schemas, so replay and continuation reconstruct the same next request; adapters may translate that provider-neutral field into a provider-native tool-search history. Any thrown or denied call becomes exactly `Error: <message>`. Code Mode renders the outer program's printed lines and return value, `(run_code completed with no output)` when both are empty, or `Error: code run failed (<kind>): <message>` followed conditionally by `Captured output:` and the captured lines. Inner dispatch events stay log-only, while a successful image-bearing sub-result is appended after the outer result as source-attributed context; post-execute listeners may append other source-attributed context at the same boundary.
