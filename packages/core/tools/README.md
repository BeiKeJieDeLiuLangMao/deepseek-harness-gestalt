...
The initial request contains [`tool_search`](../../../docs/tool-catalog.md#deepseek-aidsh-tools) and every non-deferred eligible schema. A successful search result contains exact matching schemas within the configured count and durable-result byte limits; the next request validates and reconstructs them from that durable result and the current eligible registry. Malformed schemas restored from a Session file fail request assembly. This is schema discovery, not activation: no visible registry state changes, and a schema that current allow-only eligibility excludes is neither returned nor restored from older history. In Code Mode the nested discovery schemas travel on the outer `run_code` result so the next SDK is reconstructable. Provider-neutral adapters receive the ordinary direct search call/result plus the newly included request schemas; OpenAI Responses routes additionally project that direct-search fact as native `tool_search_call`/`tool_search_output` history.
