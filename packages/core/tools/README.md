...
- `ctx.tools.allowEligible(names)` contributes positive eligibility for the calling scope. Contributions along the preset-to-Session chain union; once any contribution exists, only named end tools enter `schemas()`, `get()`, or `execute()`. A registered definition excluded at call creation resolves to `UNKNOWN_TOOL` before policy; eligibility narrowed during pre-policy is rechecked before `tools/execute`. Both denials are terminal: they skip the around-dispatch wrapper, tool body, `tools/post-execute`, and the rejected definition's `finalizeContent`, while `tools/result` and the loop's durable `tool/result` still receive the canonical error. Unknown or not-yet-loaded names retain the ordinary pipeline for dynamic providers. Names may precede dynamic registration, an explicit empty contribution allows no end tool, and disposal removes that contribution. [`dsh-agent-tool-eligibility`](../agent-tool-eligibility/README.md) and [`dsh-tools-eligibility`](../tools-eligibility/README.md) own the user-facing preset and settings inputs.
...
- `ctx.tools.schemas(scope?: ScopeKey): ToolSchema[]` Initial model-request schemas, excluding `deferLoading` definitions and every host callback.
- `ctx.tools.catalogSchemas(scope?: ScopeKey): ToolSchema[]` Complete current eligible end-tool catalog, including deferred definitions and excluding reserved presentation infrastructure. The shipped schemas are catalogued in [docs/tool-catalog.md](../../../docs/tool-catalog.md), generated by booting each tool plugin and harvesting this method (see [the tool-schema-catalog Agent Note](../../../.agents/notes/implemented/process/2026-07-02-tool-schema-catalog.md)).
...
- `ToolExecutionResult` — discriminated execution-local outcome. Success is `{ isError:false, value:JsonValue, content, meta?, additionalContexts?, loadedTools? }`; failure is `{ isError:true, error:{ message, info? }, content, meta?, additionalContexts? }` and has no value. Call identity stays on the immutable `ToolExecution`. The registry snapshots, validates, and freezes the canonical value before rendering, then materializes the durable presentation fields before final observation. `loadedTools` carries schemas discovered by the final committed success and is cleared when around/post policy or a definition finalizer replaces its value or content. `ToolFailure.info` carries an internal `{ name, code }` for a `HarnessError`; `additionalContexts` preserves every deferred or post-execute identified `UserMessage` for the loop's post-result FIFO.
...
- `PostToolDecision` — accept may replace `content` or `value`, never both, and may attach `additionalContexts`; block turns feedback into a valueless failure. Content replacement preserves the canonical value and presentation metadata but clears discovery metadata. Value replacement is revalidated, rerenders content/presentation metadata, and also clears discovery metadata. Accept preserves tool-deferred contexts before decision contexts; block discards tool-deferred contexts and exposes only contexts explicitly supplied by the blocking decision.
...
The initial request contains [`tool_search`](../../../docs/tool-catalog.md#deepseek-aidsh-tools) and every non-deferred eligible schema. A successful search result contains exact matching schemas within the configured count and durable-result byte limits. Deferred parameter schemas may omit `$schema` for the Harness draft-07 default or declare draft-07 or the MCP JSON Schema 2020-12 dialect; unsupported dialect identifiers and malformed schemas fail discovery. For reconstruction, the registry rejects Proxies before record inspection, safely reads only an own enumerable string name from each stored candidate, and discards and deduplicates names against the current eligible deferred view. Retained candidates must be accessor-free lossless JSON before canonical serialization, so Proxy traps and getters never run; the registry then budgets the complete raw eligible set before validating and projecting its full schemas. Malformed, unsupported, or huge stale candidates are ignored; an invalid or oversized current candidate fails assembly. This is schema discovery, not activation: no visible registry state changes, and a schema that current allow-only eligibility excludes is neither returned nor included in reconstruction budgeting. In Code Mode the nested discovery schemas travel on the outer `run_code` result; its complete merged metadata is budgeted before the canonical result is logged. Provider-neutral adapters receive the ordinary direct search call/result plus the newly included request schemas; OpenAI Responses routes additionally project that direct-search fact as native `tool_search_call`/`tool_search_output` history.

#### Token effect

Deferred schemas cost no initial-request tokens. Search results and matched schemas add data-dependent tokens from the discovery step onward.

#### KV Cache effect

The initial prefix stays stable while the immediate eligible catalog is unchanged. A logged search result appends history and changes the request tool list at that point without rewriting earlier history.
