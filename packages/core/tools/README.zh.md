...
初始请求包含 [`tool_search`](../../../docs/tool-catalog.md#deepseek-aidsh-tools) 与所有非 deferred 的合资格 schema。成功搜索结果包含精确匹配的 schema；下一次请求会从该持久结果与当前合资格注册表重建它们。这是 schema 发现，而不是激活：不会产生可见的注册表状态变更；当前 allow-only 资格排除的 schema 既不会被返回，也不会从旧历史恢复。在 Code Mode 中，嵌套发现得到的 schema 会随外层 `run_code` 结果传递，使下一次 SDK 可以重建。provider-neutral 适配器收到普通的直接搜索调用／结果和新加入的请求 schema；OpenAI Responses 路径还会把该直接搜索事实投影为原生 `tool_search_call`／`tool_search_output` 历史。
