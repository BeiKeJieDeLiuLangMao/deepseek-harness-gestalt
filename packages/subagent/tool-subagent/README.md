...
Each plugin instance binds one `provider` to one `toolName`; the model receives no transport selector. Load another distinctly named instance to expose another backend. Optional per-call LLM `provider`/`model` are a different axis and appear only when that backend advertises `agentOptions`. The tool registers only while its provider exists, avoiding sibling load-order and provider-reload dependencies. Its description follows `provider.inheritsParentContext`: fresh children require standalone prompts, while forked children already see completed parent turns.
...
| `agentOptions` | Default child LLM `provider`, `model`, and positive `maxTokens`. Requires the `agentOptions` capability. Per-call `provider`/`model` override these fields; omitted call fields keep this default, then inherit the parent session route. |
...
- **Persona, tool filter, and depth cap stay per instance** — another persona, tool filter, or depth cap still requires another distinctly named tool. Child LLM `provider`/`model` are per-call on backends that advertise `agentOptions`.
