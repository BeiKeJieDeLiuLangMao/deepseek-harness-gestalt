...
| `modelSelectionSettings` | `false` | Sample the Host's enabled user exact-route authorization for each fresh top-level Session; requires the Settings owner and provider `agentOptions` support |
| `deploymentRoutePreauthorization` | `false` | Sample the current deployment registry snapshot once per fresh top-level Session from the Agent or preset scope; an absent Provider records `[]`; independent of Settings and requires provider `agentOptions` support |
...
Set `modelSelectionSettings: true` to sample the Host's enabled `subagent-model-selection` preference when each fresh top-level Session is composed. Set `deploymentRoutePreauthorization: true` to independently sample `ctx.subagentRoutePreauthorization` from that Agent or preset scope at the same moment; this mode requires no Settings service and does not inject the Provider. An absent Provider records an empty route list, and a later Provider cannot authorize that Session. When both are enabled, the tool records their sorted, deduplicated union. A restored Session without a recorded policy remains disabled, including an explicitly empty restore. An empty recorded policy disables route fields and `list_subagent_models`. Recorded policy is inherited by child Sessions and remains unchanged by later Settings edits, Provider detach, or Provider replacement. The tool exposes optional `provider`, `model`, and `reasoning_effort` fields and registers the shared `list_subagent_models` tool only when that recorded policy is non-empty. Either mode requires a backend that advertises `agentOptions`; both in-process backends and DSH SDK support it, while ACP, Codex, and Claude Code reject it rather than ignore it.
...
Direct unpublished setup calls `apply(ctx, config, agent)`. The Agent supplies both the Session policy and the lifetime of shared model discovery; standing compositions resolve each candidate from the Agent registry.
...
- [Background subagent tasks](../../../.agents/notes/archived/feature/2026-07-08-background-subagent-tasks.md) — the one-shot background route.
