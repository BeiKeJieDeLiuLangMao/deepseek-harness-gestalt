...
| `agentOptions` | 子 agent 默认的 LLM `provider`、`model` 和正整数 `maxTokens`。要求具备 `agentOptions` 能力。每次调用的 `provider`/`model` 会覆盖这些字段；调用中省略的字段保留该默认值，然后再继承父会话路由。 |
