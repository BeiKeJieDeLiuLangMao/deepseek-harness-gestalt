...
子 agent 会获得父 agent 的工作目录／会话谱系；除非 `request.agentOptions` 覆盖，否则还会继承父 agent 当前的提供方／模型路由——先是实时会话选择，否则是最新已记录的 request header，再否则是创建时的 `AgentOptions`——以及输出 token 上限。它获得全新的扁平注册作用域：父级所有权不会导入父 agent 的工具限制，也不会建立权限子集。
