...
- `snapshot()` returns the complete committed revision with pending questions and terminal records. `changes(listener)` publishes the same authoritative projection only after atomic durable replacement; one throwing listener cannot starve another. The generated Remote namespace `memberQuestion` exports `snapshot` and `settle`; `packages/api/remotes` forwards `member-question-receiver/changed` after each durable commit.
