...
- `snapshot()` 返回含 pending 提问与 terminal 记录的完整已提交 revision。`changes(listener)` 只在原子持久替换后发布同一权威投影；一个抛错 listener 不会阻塞其他 listener。生成的 Remote 命名空间 `memberQuestion` 导出 `snapshot` 与 `settle`；`packages/api/remotes` 在每次持久提交后转发 `member-question-receiver/changed`。
