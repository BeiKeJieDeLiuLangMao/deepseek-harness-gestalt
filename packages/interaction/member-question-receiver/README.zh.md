...
- `settle(questionId, settlement)` 通过已配置的 first-claim authority 提议显式 `answered` 或 `declined` 终态，或者应用 transport 提供的权威 claim。保留的终态始终 canonical，包括本地 claim 失败的情况；human terminal 保留类型化 Installation id、设备名、时间与 answered value，`expired`、`withdrawn`、`superseded` 仍是 system terminal。Remote `settle` 用 pending 行校验 `receivingSessionId`、`questionId` 与 `revision`，再从本 Host 填入 Installation identity 与 `settledAt`。包内 `memberQuestionRemoteSettleRequestSchema` 是严格 settle codec：不含 Installation identity 与 `settledAt`，多余键失败。仍携带这些字段的 SRC 调用在方法处失败。生成的 Typert codec 目前投影为 `z.object` 并剥掉未知键；解码后 Host identity 仍来自本 Host。
