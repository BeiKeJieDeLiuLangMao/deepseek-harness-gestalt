...
- `ingest(envelope)` 只在认证 endpoint 已建立的接收 Account authority 旁接收解码后的 `member-question` operation。可选 `documents` 是 `MemberQuestionDocumentAssembler` 重组 Companion `document-chunk` 帧后、按 reference path 关联的传输 bytes。同一提问重放幂等；authority 或内容冲突会失败。Host 为 `(originSessionId, receiver Account)` 创建并持久化 opaque `ReceivingSessionId`，随后调用注入的 Session materializer，使该 identity 成为邀请绑定 Workspace 中的普通 Host Session。materializer 把传输 bytes 写到 `.dsh/member-questions/<questionId>/`：先 unlink 预埋符号链接，再独占创建仅所有者可访问的普通文件，并返回 `{ path, reason, cachedPath }` 元数据；同名 Workspace 文件永不被替换。它绝不从 `mq-recv` 派生 id，也不信任明文中的收件人。
- `snapshot()` 返回含 pending 提问与 terminal 记录的完整已提交 revision。`changes(listener)` 只在原子持久替换后发布同一权威投影；一个抛错 listener 不会阻塞其他 listener。生成的 Remote 命名空间 `memberQuestion` 导出 `snapshot`、`settle` 与 `admitHumanTurn`；`packages/api/remotes` 在每次持久提交后转发 `member-question-receiver/changed`。
- `settle(questionId, settlement)` 通过已配置的 first-claim authority 提议显式 `answered` 或 `declined` 终态，或者应用 transport 提供的权威 claim。保留的终态始终 canonical，包括本地 claim 失败的情况；human terminal 保留类型化 Installation id、设备名、时间与 answered value，`expired`、`withdrawn`、`superseded` 仍是 system terminal。Remote `settle` 用 pending 行校验 `receivingSessionId`、`questionId` 与 `revision`，再从本 Host 填入 Installation identity 与 `settledAt`。两条都通过 pending 校验的 Remote 会分别入队：先持久化的获胜，后进入 `settle` 的返回同一终态；之后看不到 pending 行的 Remote 回答 `member-question/revision-stale`。包内 `memberQuestionRemoteSettleRequestSchema` 是 `z.strictObject`，拒绝额外的 Installation identity 与 `settledAt`。仍携带这些字段的 SRC 调用在方法处失败。生成的 Typert codec 目前投影为 `z.object` 并剥掉未知键；解码后 Host identity 仍来自本 Host。
- `admitHumanTurn({ receivingSessionId, revision, rpcId, content, mode })` 先持久保留稳定 `rpcId`、规范化 content、mode 与 digest，并投影该 reservation 的 id 与 mode 供 Client 重启恢复，再把解析出的精确绑定 Workspace id 放入 admission context，调用一次注入的高层 human-turn adapter，成功后提交。adapter 不会在 receiver transaction 活跃时回查 receiver service。adapter 失败或 admission 后文件提交失败都会保留可精确重试的 action；同一 `rpcId` 下的不同 content 会被拒绝。adapter 必须按 `rpcId` 幂等；调用方永远看不到 Session-create 与 prompt 两个独立操作。Remote `admitHumanTurn` 在 `requestId` 下接收编码后的浏览器上传，经 Host attachment store 提升图片，并且绝不在线上接受 attachment ref。
