...
- The owning Session's versioned `schedule/change` stream is the only durable Schedule state. Folds validate every durable JSON boundary and derive retained records including paused ones; timers, idle waiters, and tool values remain disposable projections.
- A normal Session folds its complete log. A fork derives Schedule state only from events at or after `Session.inheritedEventCount`; it never inherits a parent reminder.
