...
`dsh-schedule` gives future live root Agents three Session-scoped tools for durable reminders and exposes human pause, resume, and delete mutations through `ctx.schedules`. Version 1 accepts positive safe-integer `after_seconds` delays, explicit absolute `at` targets, and fixed-rate `every_seconds` intervals of at least five minutes. The Session event log owns reminder state; timers, tool values, the browser projection, and model follow-ups are disposable projections of that log.

## Composition

Load this Service plugin after `ctx.sessions`, `ctx.agents`, `ctx.tools`, `ctx.sessionPersistence`, and the persistence listener that implements Session flushes. Static injection makes a missing persistence service a composition error. It installs `ctx.schedules`, optionally contributes the `schedules` unit when `ctx.sessionProjections` exists, listens only to later `agent/created` events, installs on runtime roots, and registers all tools through the exact `agent.ctx`. Agents that already existed when the plugin loaded and runtime children do not receive Schedule.
...
One Schedule Service-owned queue serializes each accepted management transaction and the live owner's due transaction by Session id, from preflight through any post-append barrier. Its admission closes during plugin teardown, which waits for accepted work without coupling independent Contexts. `schedule_create` requires exactly one of `after_seconds`, `at`, or `every_seconds`, validates shape-only failures before entering the queue, then checkpoints, allocates a never-reused id, appends create, and checkpoints again. `schedule_list` returns retained active and paused records in creation order with `state: "scheduled" | "overdue" | "paused"` and `deliveryMode: "session-local"`. `schedule_delete` rejects an empty or whitespace-padded id before the queue and appends for an active or paused id; an unknown or terminal id returns `{ id, deleted: false, code: "schedule_not_found" }` after preflight.

The human-only `schedules` Remote namespace accepts a branded Session id, so cold management never goes through the Agent-resume lookup. An existing live root uses its normal persistence barriers and recomputes its runtime. A cold mutation reserves and enters the exact prepared Session without announcing it, flushes before reading its fold, appends and flushes the change, then detaches it without publishing Session or Agent lifecycle or starting delivery. If preparation or entry loses to a live root, the transaction recomputes against that exact owner inside the same FIFO. Pause and resume are deliberately absent from the model-facing tool catalog; the browser uses them to manage durable state without adding model agency.

## Browser projection and board

When session projections are present, Schedule contributes an `owned-suffix` `schedules` value containing every retained record plus its durable `paused` flag in creation order. The Host folds it from `schedule/change`; the Client never reconstructs state from tool calls or transcript content. The Client clock derives scheduled versus overdue presentation from `scheduledAt`.

The standard Web app bundle contains the Schedule header action, but it activates only when the Host exposes the `schedules` Remote namespace. In the Desktop composition the action sits immediately after background jobs, stays absent for an empty list, counts scheduled plus overdue records while excluding paused records, and opens a current-state board with pause, resume, and two-step inline delete controls. Creation remains model-facing through `schedule_create`; the board has no create form.
...
The follow-up opens a normal later turn after the Agent becomes fully idle; it never steers or interrupts the current conversation. Its assistant output appears through the ordinary transcript, with no independent delivery receipt. The browser board is a current-state management projection, not a dispatch or model-completion receipt. Dispatch means the follow-up was queued and recorded, not that the model succeeded or the user read the answer.
...
- **No browser creation form** — humans manage retained records in the board, while creation remains available only through `schedule_create`.
