...
The human-only `schedules` Remote namespace accepts pause, resume, and delete against the exact live root Agent resolved from the wire Session id. Each mutation shares the Agent FIFO and persistence barriers with tools and due work. Pause and resume are deliberately absent from the model-facing tool catalog; the browser uses them to manage durable state without adding model agency.
