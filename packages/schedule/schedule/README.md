...
One Session-scoped queue serializes each accepted management transaction and the live owner's due transaction from preflight through any post-append barrier. `schedule_create` requires exactly one of `after_seconds`, `at`, or `every_seconds`, validates shape-only failures before entering the queue, then checkpoints, allocates a never-reused id, appends create, and checkpoints again. `schedule_list` returns retained active and paused records in creation order with `state: "scheduled" | "overdue" | "paused"` and `deliveryMode: "session-local"`. `schedule_delete` rejects an empty or whitespace-padded id before the queue and appends for an active or paused id; an unknown or terminal id returns `{ id, deleted: false, code: "schedule_not_found" }` after preflight.

The human-only `schedules` Remote namespace accepts a branded Session id, so cold management never goes through the Agent-resume lookup. An existing live root uses its normal persistence barriers and recomputes its runtime. A cold mutation reserves the exact prepared Session, attaches it only for its append and flush, then detaches it without publishing an Agent or starting delivery. Each mutation shares the Session FIFO with tools and due work. Pause and resume are deliberately absent from the model-facing tool catalog; the browser uses them to manage durable state without adding model agency.
