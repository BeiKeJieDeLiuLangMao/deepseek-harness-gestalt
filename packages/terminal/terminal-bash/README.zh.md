...
`shellDialect` 选择 shell 栈（默认 `bash`，或 `pwsh`）：它决定默认的 `shellPath`／`shellArgs`（bash 为 `--noprofile --norc -i`；pwsh 经共享的 `dsh-pwsh-local` 解析器得到 `-NoLogo -NoProfile`）与启动契约。bash 方言通过环境安装提示符（`PS1` 加 OSC `133;D;` 终结的 `PROMPT_COMMAND`）。pwsh 无法从环境安装提示符。在 Linux PTY 上，于 `PS …>` 之后交互写入不会执行，而 `-NoExit -File` 虽能打出标记但后续发送会被丢掉，因此 spawn 把 `HOME` 指到隔离 profile（钉住 UTF-8、定义 `prompt` 函数，并用 stdin `ReadLine` 循环接管输入（`exit` 之后由 `finally` 结束进程），后续 PTY 写入不再经过 PSReadLine 或 `PSConsoleHostReadLine`），再等待 `Write-Output` 打出 `__DSH_PWSH_SETUP_DONE__` 以及受控 `dsh> ` 提示符。该 HOME 保留到会话关闭。这是正常的交互会话（去掉 `-NoProfile` 以便加载该隔离 profile）。提交的 pwsh 行仍在 Unix 上用 LF、在 Windows 上用 CR，因为 Linux 上的 PSReadLine 把 CR 当成光标回行首。该函数与该标记都在运行时拼接，PTY 回显源码无法匹配 `dsh> ` 或该标记。Linux PTY 常常不再打印 `dsh> `；该标记才是 spawn 就绪信号。会继续 follow-up 直到该标记可见。`session_exit`、单次 send 的 `timeout` 与 spawn 墙 `timeoutMs` 仍拒绝。其环境去掉 bash 专属标记并加 `NO_COLOR`。会话解码路径按 UTF-8 读取 PTY 字节；未钉住编码的控制台会以宿主代码页输出非 ASCII 内容。两种方言发出相同的 BEL 终结 OSC 标记，因此就绪机制与消费方与方言无关。
