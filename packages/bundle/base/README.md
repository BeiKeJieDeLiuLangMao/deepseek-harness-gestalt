...
- **Wangwang is not a shared-core row** — its adapter fails loud without an admitted merchant directory, so it stays an overlay with real merchant configuration; DingTalk mounts idle and does not spawn DWS until an explicit consumer start.
