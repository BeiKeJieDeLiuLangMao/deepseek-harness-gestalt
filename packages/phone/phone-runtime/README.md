...
- `io(request, signal?)` — one closed semantic tap, swipe, text, or button action. Capture-source `x`/`y` and `captureWidth`/`captureHeight` remain the decoded plane. Android capture-source taps and swipes scale both axes onto the current incarnation `logicalDisplay` (`dumpsys display` `logicalFrame`) before `device.io.tap` / `device.io.swipe`; missing logical bounds or a capture plane that fails the uniform full-frame aspect assumption fail with `PHONE_PROTOCOL` before RPC. A dumpsys miss leaves `logicalDisplay` absent without replacing the incarnation, so capture-source coordinate io fails closed. A missing last-known size against a later known size may keep an active grant; current bounds and aspect still validate. A→miss→B revokes through the retained known size. Same aspect assumes a uniformly scaled full frame: each reconstructed logical axis, rounded to an integer pixel, must land within 1 logical pixel of the known display. That bound is integer reconstruction, not a ratio epsilon, and is not evidence of bar-free pixels; crop and rotation are never inferred. Against 2248×1080, 1124×540 and even-coded 1078×518 are accepted; 400×192 reconstructs more than 1 logical pixel off and is refused. Android fresh-probe pixels pass through. iOS rotation 0 uses the same methods; rotation 90/180/270 inverse-transforms displayed capture points into portrait event coordinates, sends swipes through `device.io.swipe`, and sends a tap as a zero-distance `device.io.swipe`. Exact rotation comes from the current H264 frame's trusted `captureRotation`, a live MJPEG EXIF observation, or one bounded single-flight MJPEG probe for model-only calls; unknown rotation is never inferred from width and height. Rotation observations and probes are scoped to the current runtime generation and listed device. Button and text stay independent of coordinate conversion. Physical handsets are valid targets; only ids absent from the latest published listing fail locally with `PHONE_DEVICE_NOT_FOUND`.
- `startCapture(request)` — upstream `device.screencapture`. An iOS MJPEG browser capture carries a runtime-private `captureId`; the returned body passes every byte unchanged while a bounded rolling JPEG prefix observes EXIF 1/3/6/8 as exact rotation for current-generation input. `h264` maps onto upstream `avc`; Android AVC is syntax-checked before it reaches the renderer. If mobilecli returns an HTTP-200 error body, a malformed stream, or no complete key access unit before `h264ProbeTimeoutMs`, or `dumpsys display` reports a landscape `logicalFrame` while Android screenrecord otherwise stays at physical portrait, the runtime tries Android system `screenrecord --output-format=h264` (with `--size WxH` from that logical frame) from the selected SDK or PATH and preserves H264. Discovery and control remain on mobilecli. If both H264 sources fail, the renderer can enter its MJPEG policy. Both mobilecli answer shapes are accepted: the bare stream and the 1.0.5 `{ format, sessionUrl }` envelope, whose session URL is forced onto the loopback fence. `requestTimeoutMs` bounds response headers and `h264ProbeTimeoutMs` bounds each H264 source probe; the caller owns a published body. A generation or incarnation change after headers joins foreign body cancellation for at most `captureCleanupTimeoutMs`. Unknown ids fail with `PHONE_DEVICE_NOT_FOUND`.
