/**
 * Non-Service mobilecli Adapter. Each generation owns one server tree, RPC client,
 * committed listing, coordinate observations, command joins, and capture cleanup.
 * The facade installs validation and publication callbacks before startup.
 * @module @deepseek-ai/dsh-phone-runtime/mobilecli-phone-runtime
 */

import type { Context } from '@deepseek-ai/cordis'
import type { Config } from './index.ts'
import type { PhoneRuntimeAdapter, PhoneRuntimeGeneration, PhoneRuntimeSpawnRequest, PhoneRuntimeStart } from './runtime-pool.ts'
...
import type { PhoneRuntimeStateValidator } from './runtime-state.ts'
...
/** Complete validated deployment settings for every generation. */
export type ResolvedConfig = Omit<Required<Config>, 'executablePath' | 'provisioningProfilePath'> & Pick<Config, 'executablePath' | 'provisioningProfilePath'>
...
/**
 * Validate composition defaults and configured executable/profile paths.
 * @param config - Schema-resolved composition input.
 * @returns complete generation settings.
 */
export function resolveValidatedConfig(config: Config): ResolvedConfig {
  ...
  assertDurationField('cleanupTimeoutMs', values.cleanupTimeoutMs)
...
/** Facade-owned callbacks installed before startup; validation precedes listing commit. */
export interface GenerationPublication {
  readonly validate: (change: PhoneDeviceChange) => undefined
  changed(change: PhoneDeviceChange): void
  readiness(ready: boolean): void
}

/** External mobilecli generation factory; isolated acquisition is explicitly unavailable. */
export class MobilecliPhoneRuntime implements PhoneRuntimeAdapter {
  constructor(
    private readonly config: ResolvedConfig,
    private readonly logger: Context['logger'],
    private readonly admit: (generation: MobilecliGeneration) => GenerationPublication,
  ) {}

  async start(request: PhoneRuntimeSpawnRequest): Promise<PhoneRuntimeStart> {
    if (request.kind !== 'external') throw new PhoneDevicesError('PHONE_UNAVAILABLE', 'isolated phone runtimes are unavailable')
    const executable = request.config.executablePath
    if (executable === undefined) throw new PhoneDevicesError('PHONE_UNRESOLVED', 'the phone runtime has no selected executable')
    const generation = new MobilecliGeneration(this.logger, this.config, executable, request.config.environment ?? {})
    generation.connect(this.admit(generation))
    try {
      await generation.start(request.signal)
      return { generation, stop: () => generation.stop() }
    } catch (error) {
      try { await generation.stop() } catch (cleanup) {
        throw phoneFailureWithCleanup(normalizeOperationError(error), cleanup, 'phone generation startup cleanup failed')
      }
      throw error
    }
  }
 ...
 */
export class MobilecliGeneration implements PhoneRuntimeGeneration {
  private publication: GenerationPublication | undefined
  private readonly operations = new Set<Promise<unknown>>()
  private readonly cleanupFailures: unknown[] = []
  private readonly cleanupWork = new Set<Promise<unknown>>()

  private retainCleanup(work: Promise<unknown>): Promise<unknown> {
    this.cleanupWork.add(work)
    void work.then(
      () => { this.cleanupWork.delete(work) },
      (error: unknown) => {
        this.cleanupWork.delete(work)
  ...
  private readonly bodyCleanups = new Set<() => Promise<void>>()
  private retirementReason = new PhoneDevicesError('PHONE_ABORTED', 'the phone runtime generation was disabled')
  private readonly ctx: { readonly logger: Context['logger'] }

  /**
   * Install generation-tokened facade publication before startup.
   * @param publication - Validation, listing, and readiness callbacks.
   */
  connect(publication: GenerationPublication): void { this.publication = publication }
  /**
   * Read the authoritative listing for the facade invariant owner.
   * @returns the last committed listing, absent before baseline.
   */
  readListing(): PhoneDeviceList | undefined { return this.publishedList }
  /**
   * Spawn and establish baseline plus stable readiness for this generation.
   * @param signal - Pool-owned startup cancellation.
   * @returns completion after readiness publication.
   */
  ...
  /**
   * Close admission and join exact generation cleanup.
   * @returns memoized cleanup completion; rejection retains non-quiescent failure.
   */
  stop(): Promise<void> { return this.teardown() }
  /**
   * Retire facade publication before pool cancellation and tree termination.
   * @param reason - Disposal reason; disposal clears listing without notifying observers.
   */
  revoke(reason?: PhoneDevicesError): void {
    if (reason !== undefined) this.retirementReason = reason
    this.ready = false
    this.publishReadiness(false)
    try {
      if (reason?.code === 'PHONE_DISPOSED') this.publishedList = undefined
      else this.clearPublishedList()
    } finally {
  ...
  private track<T>(run: () => Promise<T>): Promise<T> {
    ...
    void operation.then(
      ...
      (error: unknown) => {
        this.operations.delete(operation)
        if (error instanceof PhoneDevicesError && error.cause instanceof AggregateError) this.cleanupFailures.push(error)
      },
    )
    return operation
  }
  ...
  /**
   * Allocate generation state without spawning; facade callbacks attach before start.
   * @param logger - Owning facade diagnostic sink.
   * @param config - Complete validated deployment settings.
   * @param executablePath - Resolved immutable executable.
   * @param environment - Non-sensitive environment pinned to this generation.
   */
  constructor(logger: Context['logger'], config: ResolvedConfig, executablePath: string, environment: Readonly<Record<string, string>>) {
    this.ctx = { logger }
   ...
   * @returns the immutable executable accepted by the facade resolver.
   */
  ...
  private async startup(signal: AbortSignal): Promise<void> {
    ...
    try {
      for (;;) {
        ...
        if (signal.aborted) throw new PhoneDevicesError('PHONE_ABORTED', 'phone runtime activation was cancelled')
  ...
  private async whenReady(signal: AbortSignal | undefined): Promise<void> {
    // The Adapter calls start before publishing this generation to any fleet operation.
    const startup = this.startupOutcome as Promise<void>
  ...
  listDevices(signal?: AbortSignal): Promise<PhoneDeviceList> {
    return this.track(() => this.listDevicesOwned(fuseCallerAndLifetime(signal, this.lifetime.signal)))
  }

  private async listDevicesOwned(signal: AbortSignal): Promise<PhoneDeviceList> {
    this.assertAccepting()
  ...
  boot(id: DeviceId, signal?: AbortSignal): Promise<void> {
    return this.track(() => this.bootOwned(id, fuseCallerAndLifetime(signal, this.lifetime.signal)))
  }

  private async bootOwned(id: DeviceId, signal: AbortSignal): Promise<void> {
    this.assertAccepting()
  ...
  shutdown(id: DeviceId, signal?: AbortSignal): Promise<void> {
    return this.track(() => this.shutdownOwned(id, fuseCallerAndLifetime(signal, this.lifetime.signal)))
  }

  private async shutdownOwned(id: DeviceId, signal: AbortSignal): Promise<void> {
    this.assertAccepting()
  ...
  io(request: PhoneIoRequest, signal?: AbortSignal): Promise<void> {
    return this.track(() => this.ioOwned(request, fuseCallerAndLifetime(signal, this.lifetime.signal)))
  }

  private async ioOwned(request: PhoneIoRequest, signal: AbortSignal): Promise<void> {
    this.assertAccepting()
    this.assertUsable()
    await this.whenReady(signal)
    this.assertAccepting()
  ...
  private async rotationFor(
    ...
    if (current === undefined) {
      ...
      void this.retainCleanup(settled)
  ...
  startCapture(request: PhoneCaptureRequest): Promise<PhoneCaptureStream> {
    return this.track(() => this.startCaptureOwned(request))
  }

  private async startCaptureOwned(request: PhoneCaptureRequest): Promise<PhoneCaptureStream> {
    this.assertAccepting()
    this.assertUsable()
    ...
    capture = { contentType: capture.contentType, body: this.ownBody(capture.body) }
    ...
    if (staleGeneration || staleIncarnation) {
      ...
      await joinForeignCleanup(
        this.retainCleanup(Promise.resolve().then(() => capture.body.cancel(stale))),
        this.resolved.captureCleanupTimeoutMs,
  ...
  private ownBody(body: ReadableStream<Uint8Array>): ReadableStream<Uint8Array> {
    const reader = body.getReader()
    let sourceFailure: unknown
    void reader.closed.catch((error: unknown) => { sourceFailure = error })
    let cleanup: Promise<void> | undefined
    let output: ReadableStreamDefaultController<Uint8Array> | undefined
    const forget = (): void => { this.bodyCleanups.delete(stop) }
    const stop = (): Promise<void> => {
      if (cleanup !== undefined) return cleanup
      const cancellation = Promise.resolve().then(() => reader.cancel(this.retirementReason))
      cleanup = this.retainCleanup(cancellation.then(() => {
        forget()
        output?.error(this.retirementReason)
      }, (error: unknown) => {
        forget()
        if (error === sourceFailure && error instanceof PhoneDevicesError
          && error.code === 'PHONE_ABORTED' && !(error.cause instanceof AggregateError)) return
        throw error
      })) as Promise<void>
      return cleanup
    }
    this.bodyCleanups.add(stop)
    return new ReadableStream<Uint8Array>({
      start(controller) { output = controller },
      async pull(controller) {
        try {
          const next = await reader.read()
          if (next.done) { forget(); controller.close() } else controller.enqueue(next.value)
        } catch (error) { forget(); controller.error(error) }
      },
      cancel: () => stop(),
    }, { highWaterMark: 0 })
  }
  ...
  screenshot(id: DeviceId, signal?: AbortSignal): Promise<PhoneScreenshot> {
    return this.track(() => this.screenshotOwned(id, fuseCallerAndLifetime(signal, this.lifetime.signal)))
  }

  private async screenshotOwned(id: DeviceId, signal: AbortSignal): Promise<PhoneScreenshot> {
    this.assertAccepting()
    await this.whenReady(signal)
    this.assertUsable()
    ...
    if (isSignalAborted(signal)) {
      const failure = normalizeOperationError(signal.reason)
      try { await unlink(path) } catch (cleanup) {
        throw phoneFailureWithCleanup(failure, cleanup, 'retired screenshot removal failed')
      }
      throw failure
    }
  ...
  private openNativeAndroidH264(
    ...
    return openAndroidSystemH264({
      ...options,
      ownTree: (stop) => { this.bodyCleanups.add(stop) },
    })
  }

  private async cancelCaptureBody(body: ReadableStream<Uint8Array>): Promise<void> {
    const cleanup = this.retainCleanup(Promise.resolve().then(() => body.cancel()))
    try { await cleanup } catch {
      // Cleanup remains recorded independently of usable fallback capture bytes.
    }
  }
  ...
  private async preferAndroidH264(
    ...
    if (signal.aborted) {
      await this.cancelCaptureBody(mobilecli.body)
    ...
    try {
      ...
      if (!native.recognizable) {
        await this.cancelCaptureBody(native.body)
      ...
      if (nativeBody !== undefined) await this.cancelCaptureBody(nativeBody)
      if (isSignalAborted(signal)) {
        await this.cancelCaptureBody(mobilecli.body)
        throw normalizeOperationError(signal.reason)
      }
  ...
  agentStatus(id: DeviceId, signal?: AbortSignal): Promise<PhoneAgentStatus> {
    return this.track(() => this.agentStatusOwned(id, fuseCallerAndLifetime(signal, this.lifetime.signal)))
  }

  private async agentStatusOwned(id: DeviceId, signal: AbortSignal): Promise<PhoneAgentStatus> {
    this.assertAccepting()
    await this.whenReady(signal)
    this.assertUsable()
  ...
  installAgent(id: DeviceId, options: PhoneAgentInstallOptions = {}): Promise<PhoneAgentInstallResult> {
    const owned = { ...options, signal: fuseCallerAndLifetime(options.signal, this.lifetime.signal) }
    return this.track(() => this.installAgentOwned(id, owned))
  }

  private async installAgentOwned(
    id: DeviceId,
    options: PhoneAgentInstallOptions & { signal: AbortSignal },
  ): Promise<PhoneAgentInstallResult> {
    this.assertAccepting()
    ...
    if (!reinstall) {
      const current = await this.agentStatusOwned(id, options.signal)
      if (options.signal.aborted) throw normalizeOperationError(options.signal.reason)
  ...
  private async enqueueDeviceAcquisition(signal: AbortSignal): Promise<PhoneDeviceList> {
    ...
    const admitted = this.queueTail.then(async () => {
      ...
      this.assertAccepting()
      if (signal.aborted) throw new PhoneDevicesError('PHONE_ABORTED', 'device listing acquisition was cancelled before admission')
  ...
  private publish(change: PhoneDeviceChange): boolean {
    const validator: PhoneRuntimeStateValidator | undefined = this.publication?.validate
    ...
    this.publication?.changed(change)
  ...
  private markLost(reason: PhoneDevicesError): void {
    ...
    const cleanup = this.child?.stop()
    if (cleanup !== undefined) {
      void this.retainCleanup(cleanup).catch((error: unknown) => {
  ...
  private teardown(): Promise<void> {
    ...
    this.closing = true
    this.publication = undefined
    this.lifetime.abort(this.retirementReason)
    this.subscribers.clear()
    this.readinessSubscribers.clear()
    this.disposed = true
    const operation = this.stopRuntime(this.retirementReason)
  ...
  private publishReadiness(next: boolean): void {
    ...
    this.publication?.readiness(next)
  ...
  private async stopRuntime(reason: PhoneDevicesError): Promise<void> {
    ...
    const bodies = [...this.bodyCleanups].map(stop => stop())
    const cleanup = Promise.allSettled([...probes, childStop, ...bodies])
    await Promise.allSettled([...this.operations])
    await Promise.allSettled([...this.cleanupWork])
    const joins = await cleanup
    const failures: unknown[] = []
    for (const result of joins) {
      if (result.status === 'rejected') failures.push(result.reason)
    }
    failures.push(...this.cleanupFailures)
    if (failures.length > 0) throw new AggregateError(failures, 'phone generation cleanup failed')
    this.clearPublishedList()
...
async function awaitProbeForCaller(probe: Promise<PhoneRotation>, signal: AbortSignal): Promise<PhoneRotation> {
