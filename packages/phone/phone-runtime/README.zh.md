...
- `listDevices(signal?)` — 返回分组清单 `{ android, ios: { simulators, reals } }`；每项为冻结的 `PhoneDeviceRef`（`id` 为 branded `DeviceId`、`name`、`kind: 'emulator' | 'simulator' | 'real'`、`platform`、`state` 原样保留、`online`，以及 Android 可选的 `logicalDisplay`，来自 `adb dumpsys display` 的 `logicalFrame`，绝不用锁死的 `device.info.screenSize`）。关机的模拟器/仿真器同样是合法 boot 目标，因此始终随查询发送 `includeOffline: true`。仅上游 `online` 状态映射为 `online: true`；其余一切上游状态——`offline`、`unauthorized` 等——在 `state` 上原样携带而不互相折叠，因此 `unauthorized` 真机在清单中始终可辨识（上游在其接受信任提示前拒绝其 io）。`devices.list` 结果的两种已发布形态——裸设备数组与 mobilecli 1.0.5 的 `{ devices: [...] }` 信封——均被接受。wire 解析器会验证每一行，再为每个 `(platform, id)` 组合保留首行，避免上游重复项进入设置、picker 或 badge。由于 operation 只接受 `deviceId`，同一 id 出现在两个平台时会以歧义 `PHONE_PROTOCOL` 失败。
...
- `io(request, signal?)` — 执行一个封闭的语义 tap、swipe、text 或 button 动作。采集源的 `x`/`y` 与 `captureWidth`/`captureHeight` 仍是解码平面。Android 采集源 tap 与 swipe 会把两轴缩放到当前 incarnation 的 `logicalDisplay`（`dumpsys display` 的 `logicalFrame`）后再发到 `device.io.tap` / `device.io.swipe`；缺少逻辑边界，或采集平面不满足整帧均匀缩放假设时，在 RPC 之前以 `PHONE_PROTOCOL` 失败。dumpsys 缺失会去掉 `logicalDisplay` 而不替换 incarnation，因此采集源坐标 io 失败关闭。缺失的上次已知尺寸对上之后的已知尺寸时，可以保留有效授权；当前边界与宽高比仍会校验。A→miss→B 通过保留的已知尺寸撤销旧授权。相同宽高比假定整帧均匀缩放：每条重建的逻辑轴取整后须落在已知显示的 1 个逻辑像素内。该界限来自整数重建，不是比例 epsilon，也不能证明无黑边；绝不从像素推断裁剪或旋转。相对 2248×1080，1124×540 与偶数编码 1078×518 被接受；400×192 重建偏差超过 1 个逻辑像素，予以拒绝。Android fresh-probe 像素原样转发。iOS 旋转 0 度时使用相同方法；旋转 90/180/270 度时，运行时把显示采集点逆变换到竖屏事件坐标，swipe 通过 `device.io.swipe` 发送，tap 则作为零距离 `device.io.swipe` 发送。精确旋转来自当前 H264 帧受信任的 `captureRotation`、实况 MJPEG EXIF 观察，或模型独立调用触发的一次有界单飞 MJPEG 探针；绝不从宽高猜测未知旋转。观察与探针绑定当前运行时 generation 和设备。button 与 text 不参与坐标转换。真机是合法目标；仅最新清单中不存在的 id 以 `PHONE_DEVICE_NOT_FOUND` 失败。
- `startCapture(request)` — 对应上游 `device.screencapture`。iOS MJPEG 浏览器采集携带运行时私有 `captureId`；返回 body 逐字节原样通过，同时用有界滚动 JPEG 前缀观察 EXIF 1/3/6/8，作为当前 generation 输入的精确旋转。`h264` 映射为上游 `avc`；Android AVC 在到达 renderer 前先经过语法识别。mobilecli 若以 HTTP 200 返回错误正文、畸形码流，或在 `h264ProbeTimeoutMs` 内没有产出完整 key access unit，或 `dumpsys display` 报告横屏 `logicalFrame` 而 Android screenrecord 仍按物理竖屏编码，runtime 会从已选择的 SDK 或 PATH 调用 Android 系统 `screenrecord --output-format=h264`（带该逻辑帧的 `--size WxH`），继续保留 H264；设备发现与控制仍由 mobilecli 承担。两条 H264 源都失败时，renderer 可以进入 MJPEG 策略。mobilecli 的裸流和 1.0.5 `{ format, sessionUrl }` 信封都会被接受，信封会话 URL 必须留在回环栅栏内。`requestTimeoutMs` 约束响应头，`h264ProbeTimeoutMs` 约束每条 H264 源的识别；已发布 body 的取消由调用方持有。响应头到达后若 generation 或 incarnation 已过期，runtime 会在 `captureCleanupTimeoutMs` 内等待外部 body 取消。未知 id 以 `PHONE_DEVICE_NOT_FOUND` 失败。
- `screenshot(id, signal?)` — 通过 `mobilecli screenshot --format png` 捕获一张 PNG 静帧，并持久化到 `$DSH_HOME/phone/screenshots/<deviceId>-<stamp>.png`（目录 `0700`、文件 `0600`）。应答为 `{ mediaType: 'image/png', path }`，携带该绝对路径，不含 PNG 字节或 base64。真机是合法目标；仅最新清单中不存在的 id 在本包内以 `PHONE_DEVICE_NOT_FOUND` 失败。实况 MJPEG/H264 采集仍由 `startCapture` 负责。
