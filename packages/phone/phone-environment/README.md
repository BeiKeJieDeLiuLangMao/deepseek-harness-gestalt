...
| `androidRuntimeVerifyTimeoutMs` | `180000` | Ceiling in milliseconds for mobilecli listing the prepared Android device online before the H264 picture probe; a cold Emulator boot can outlast the picture budget. |
| `iosRuntimeVerifyTimeoutMs` | `25000` | Overall ceiling in milliseconds for the online Simulator listing, device-agent preparation, and recognizable MJPEG/JPEG picture. |
| `iosAgentSettleDelayMs` | `2000` | Delay in milliseconds after device-agent installation before opening the first Simulator capture. |
| `iosAgentCaptureRetryDelayMs` | `1000` | Delay in milliseconds before retrying the first unsuccessful Simulator capture after device-agent installation. |
| `iosAgentFirstCaptureTimeoutMs` | `5000` | Ceiling in milliseconds for the first Simulator capture opened after device-agent installation. |
