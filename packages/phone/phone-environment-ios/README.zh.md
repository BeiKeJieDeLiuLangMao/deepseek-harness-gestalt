---
description: "用于配置或调试 mobilecli iOS 设备的 Xcode 运行时和托管 iPhone 模拟器准备。"
...
在 macOS 上通过完整 Xcode 安装准备并启动产品自有的 iPhone 模拟器。它检测 Xcode、许可、首次启动状态、运行时、设备类型与现有模拟器；Windows 和 Linux 返回稳定的不可用状态。Xcode 安装、许可、Apple ID、系统权限、设备信任、Developer Mode 和签名仍须手动完成。
