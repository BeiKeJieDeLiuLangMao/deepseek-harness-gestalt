...
| [`browser-runtime-tandem/`](browser-runtime-tandem/README.md) | 用于一个临时 Profile 的托管式 Tandem Browser HTTP Provider | 提供 `ctx.browserRuntime` |
| [`tool-browser/`](tool-browser/README.md) | 面向模型的延迟 Consumer | 注册到 `ctx.tools` |

子系统参考见 [docs/subsystems/browser-runtime.md](../../docs/subsystems/browser-runtime.md)。[临时浏览器运行时 Agent Note](../../.agents/notes/implemented/feature/2026-08-18-temporary-browser-runtime-tracer.md)记录生命周期、发现与持久化决策；[Tandem provider Agent Note](../../.agents/notes/implemented/feature/2026-08-18-tandem-browser-runtime-provider.md)记录托管子进程设计与不可用/重连状态模型。
