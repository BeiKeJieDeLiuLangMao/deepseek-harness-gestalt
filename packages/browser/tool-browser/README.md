---
description: "Deferred model-facing Browser Runtime tools."
...
The initial tool list omits all seven Browser tools and includes the ordinary `tool_search` schema. A search for browser capabilities returns the exact schemas in a durable result. Later requests revalidate those names against current eligible deferred definitions. Each operation renders its complete action-specific result as JSON text; across the result family, those fields cover Profile, Workspace, browser, tab, revision, page, screenshot, focus, close, availability, chrome, and storage facts, including unlabeled temporary Profiles, the reserved shared Profile, and `unavailable` states with their reason and reconnect flag. Omitting `profile` on `browser_create` uses the `ui-browser` settings default, which is the shared Profile until that page changes it. With the Session Binder composed, an omitted attach reuses an open browser instance on the matching retained Profile.
...
No runtime invariant companion is published because this package contributes tools through `dsh-tools`, which owns their registration, execution, and disposal, and this package retains no independently observed state.
