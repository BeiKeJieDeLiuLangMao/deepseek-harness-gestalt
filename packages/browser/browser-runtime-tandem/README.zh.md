---
description: "Browser Runtime 能力的 Tandem 形态 HTTP Service Provider。"
...
fixture 子进程意外退出或 HTTP `BROWSER_RUNTIME_UNAVAILABLE` 响应会提交一个 reason 为 `crashed` 或 `unhealthy` 的 `BrowserUnavailableState`。fixture 子进程最多尝试 `reconnectAttempts` 次重启。配置 `sidecar: false` 时，客户端改为在 `startupTimeoutMs` 内轮询外部 Host 的同一 target；恢复成功后以下一修订号重新提交打开页面状态，恢复耗尽则提交 `reason: 'reconnect-failed'` 且 `reconnecting: false`。该投影是真实的：不可用期间，针对该 target 的操作会以 `BROWSER_RUNTIME_UNAVAILABLE` 拒绝，而不是报告过期的页面事实。格式错误的 HTTP 响应、超限响应体与字段校验失败会以 `BROWSER_PROTOCOL` 拒绝。
