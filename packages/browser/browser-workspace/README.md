...
`browser/workspace` is a log-only, last-wins `SessionEventMap` member. The Binder folds `Session.snapshotEvents()` and writes each committed snapshot with `{ ignorable: true }`. Forked Sessions fold child-owned events through `Session.ownEvents()`. When `ctx.sessionProjections` is composed, the package registers the `browserWorkspace` unit. Cross-Session page transfer is not supported.
