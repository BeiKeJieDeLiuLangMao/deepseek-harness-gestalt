...
`browser/workspace` is a log-only, last-wins `SessionEventMap` member. The Binder folds `Session.snapshotEvents()`, including a fork-inherited prefix, and writes each committed snapshot with `{ ignorable: true }`. Live Runtime verbs reject a target that more than one live Session currently owns. When `ctx.sessionProjections` is composed, the package registers the `browserWorkspace` unit. Cross-Session page transfer is not supported.
