...
`browser/workspace` is a log-only, last-wins `SessionEventMap` member. `snapshot` folds `Session.snapshotEvents()` — the complete Session log, including a fork-inherited prefix — and writes each committed snapshot with `{ ignorable: true }`. Live Runtime verbs reject a target another live Session currently owns. After fork, parent and child both reconstruct that Workspace, so exclusive live Runtime authority is not claimed here. When `ctx.sessionProjections` is composed, the package registers the `browserWorkspace` unit. Cross-Session page transfer is not supported.
...
- After fork, parent and child both fold the inherited last-wins Workspace. `assertOwned` then treats each as the other's owner, so live Runtime verbs refuse the shared target. Exclusive live ownership after fork is out of this event-API slice.
