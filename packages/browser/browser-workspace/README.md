...
`browser/workspace` is a log-only, last-wins `SessionEventMap` member. `snapshot` folds `Session.snapshotEvents()` — the complete Session log, including a fork-inherited prefix — and writes each committed snapshot with `{ ignorable: true }`. Live Runtime verbs still reject a target that more than one live Session currently owns; a fork does not relax Session ownership. When `ctx.sessionProjections` is composed, the package registers the `browserWorkspace` unit. Cross-Session page transfer is not supported.
