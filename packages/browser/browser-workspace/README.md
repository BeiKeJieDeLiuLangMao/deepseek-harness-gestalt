...
`create`, `navigate`, `observe`, `screenshot`, `focus`, and `close` require the owning `Session`. A missing Session ownership rejects with `BROWSER_SESSION_MISMATCH`. A target already owned by another live Session rejects with `BROWSER_TRANSFER_UNSUPPORTED`. `setDock` records whether the Dock is open and the preferred width as Session facts. `snapshot` and `foldBrowserWorkspace` return the last logged whole Workspace, or the empty Workspace before the first change. `cleanup` closes leftover live Runtime tabs for one Session; Session disposal also runs it.
