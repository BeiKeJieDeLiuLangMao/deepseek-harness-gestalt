...
`create`、`navigate`、`observe`、`screenshot`、`focus`、`input`、`takeover`、`returnControl` 与 `close` 都要求提供所属 `Session`。缺少 Session 所有权会以 `BROWSER_SESSION_MISMATCH` 拒绝。已被另一个 live Session 拥有的 target 会以 `BROWSER_TRANSFER_UNSUPPORTED` 拒绝。`create` 附加到另一 live Session 的 Workspace 或实例也会以 `BROWSER_TRANSFER_UNSUPPORTED` 拒绝，附加到本 Session 未知的层级则以 `BROWSER_SESSION_MISMATCH` 拒绝。`input` 与 `takeover` 会在 Session 快照上持久化报告的 `controlOwner: human`；`returnControl` 与 Agent 写入会持久化 `agent`。锁是修订号；每条标签页记录还会保存最近一次提交的 Runtime 修订号。Binder 监听 `browser/runtime-state`，并为已持有且未关闭的标签页写入这些事实，包括从未进入 Binder 动词的修订号前进。对已关闭标签页的 `observe` 会遗忘该列表行。`setDock` 把 Dock 是否打开以及首选宽度记录为 Session 事实。`snapshot` 与 `foldBrowserWorkspace` 返回最后记录的完整 Workspace；在首次变更前返回空 Workspace。`cleanup` 会关闭遗留的 live Runtime 标签页、从 Session 快照中遗忘它们，并作为 `session/disposed` 返回的工作。
...
- Dock chrome 见 [`dsh-client-ui-browser`](../../client/ui-browser/README.md)。本包会持久化 Dock 打开状态、宽度、`userCollapsed`、每个标签页的当前控制权所有者，以及每个标签页最近一次提交的修订号。
