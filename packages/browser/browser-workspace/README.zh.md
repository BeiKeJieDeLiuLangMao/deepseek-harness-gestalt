...
`browser/workspace` 是仅日志、后写覆盖的 `SessionEventMap` 成员。`snapshot` 折叠 `Session.snapshotEvents()`——完整 Session 日志，含 fork 继承前缀——并以 `{ ignorable: true }` 写入每次提交的快照。live Runtime 动词仍拒绝当前被多于一个 live Session 持有的 target；fork 不放宽 Session 所有权。当组合挂载 `ctx.sessionProjections` 时，本包注册 `browserWorkspace` 投影单元。不支持跨 Session 页面转移。
