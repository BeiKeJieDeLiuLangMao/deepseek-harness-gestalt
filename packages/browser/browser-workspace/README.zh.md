...
`browser/workspace` 是仅日志、后写覆盖的 `SessionEventMap` 成员。Binder 折叠 `Session.snapshotEvents()`，并以 `{ ignorable: true }` 写入每次提交的快照。Fork 后的 Session 通过 `Session.ownEvents()` 折叠子会话自有事件。当组合挂载 `ctx.sessionProjections` 时，本包注册 `browserWorkspace` 投影单元。不支持跨 Session 页面转移。
