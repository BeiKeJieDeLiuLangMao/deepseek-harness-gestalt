...
A renderer-process crash commits `BrowserUnavailableState` with reason `crashed` and recreates the hidden window for the same target. A navigation or page observation that loses the Electron runtime commits reason `unhealthy`, releases the serialized operation queue, and recreates the hidden window at its last committed URL. Exhausted recovery commits `reason: 'reconnect-failed'`. Malformed Chromium results reject with `BROWSER_PROTOCOL`.
