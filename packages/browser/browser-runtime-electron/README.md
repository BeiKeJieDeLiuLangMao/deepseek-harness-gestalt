...
In-process Electron Browser Runtime Provider for temporary and named persistent Profiles. It implements `ctx.browserRuntime` with this process's `session.fromPartition` and hidden offscreen `webContents`. Screenshots use `webContents.capturePage`; page text uses `executeJavaScript`. Named Profiles restore `persist:…` partitions; temporary Profiles use unique partitions and leave no reusable identity. Partition files stay under Electron `userData` and never write `~/Library/Application Support/Tandem Browser`.

The plugin loads only when `process.versions.electron` is set or a test injects Electron APIs. Composing it on Node fails at load. Desktop Host owns the hidden windows; the Dock remains a native pane of screenshot, title, and text and does not embed a second BrowserView.
...
`listenElectronBrowserHttp` binds a loopback HTTP server that copies Tandem's session, tab, navigate, page-content, screenshot, focus, and destroy operations so the Web Host can drive this engine without embedding a second Electron application.
...
- Real Chromium e2e runs only when `process.versions.electron` is set; Node unit tests inject Electron APIs and never spawn Tandem.app.
