...
When an entered Session emits `session/detached`, the coordinator waits for its controller, serializes a final drain, then releases state owned by that exact `Session` object. This includes unpublished preparations attached for an infrastructure transaction; it does not require a public created/disposed lifecycle. Failed retirement leaves the controller in the live-session map, so backend teardown can retry it. Backend teardown stops event admission first, flushes every remaining controller, awaits per-id operations, and only then closes the storage handle.
