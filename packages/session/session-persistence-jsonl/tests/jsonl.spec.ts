...
import { KNOWN_SESSION_EVENT_TYPES, SESSION_FORMAT_VERSION, SessionLogOffset, SessionSeq, SessionId } from '@deepseek-ai/dsh-session'
...
describe('JsonlSessionPersistence: official vocabulary portability', () => {
  it('retains an ignorable unknown event across reopen and continued writes', async () => {
    ...
    const firstSeq = oneTurnLog().length
    const unknown = {
      type: 'official/unknown-event', seq: SessionSeq(firstSeq), time: 7007,
      data: { payload: ['retained', 2], nested: { exact: true } }, ignorable: true,
    } as unknown as SessionEvent
    const second = [
      unknown,
      { type: 'turn/start', seq: SessionSeq(firstSeq + 1), time: 8008, data: { turn: 2 } },
      { type: 'turn/end', seq: SessionSeq(firstSeq + 2), time: 8009, data: { turn: 2, reason: { kind: 'completed' } } },
    ] as SessionEvent[]
    ...
    expect((await writer.read()).events).toEqual([...oneTurnLog(), ...second])
    const third = [
      { type: 'turn/start', seq: SessionSeq(firstSeq + 3), time: 9009, data: { turn: 3 } },
      { type: 'turn/end', seq: SessionSeq(firstSeq + 4), time: 9010, data: { turn: 3, reason: { kind: 'completed' } } },
    ] as SessionEvent[]
