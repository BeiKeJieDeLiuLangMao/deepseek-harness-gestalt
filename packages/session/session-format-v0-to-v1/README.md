...
Restore released v0 Session JSONL by decoding each physical row into the shared-layout v1 format. The migration preserves validated headers, events, exact legacy payloads, ignorable flags, and the empty model allowlist while applying only accepted v0 normalizations. Unknown, malformed, or unsupported historical records fail before current restoration and retain their source for recovery.
...
The frozen inventory also includes Gestalt browser-workspace snapshots, member-question sender and receiver records, and admitted attachment references. Their exact payloads and ignorable flags survive migration; the default empty model allowlist remains valid. Unknown event types still refuse migration.
