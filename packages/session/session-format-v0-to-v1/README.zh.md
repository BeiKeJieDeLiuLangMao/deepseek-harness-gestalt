...
将每个物理行解码为共享布局 v1 格式，以恢复已发布的 v0 会话 JSONL。迁移保留经校验的 header、事件、精确旧 payload、ignorable 标志和空模型 allowlist，仅应用 v0 接受的归一化。未知、格式错误或不受支持的历史记录会在当前恢复器运行前失败，并保留源数据以便恢复。
...
冻结清单也包含 Gestalt 浏览器工作区快照、成员提问发送端与接收端记录，以及已接纳的附件引用。迁移保留其精确载荷和 ignorable 标记，默认的空模型允许列表仍然有效。未知事件类型仍拒绝迁移。
