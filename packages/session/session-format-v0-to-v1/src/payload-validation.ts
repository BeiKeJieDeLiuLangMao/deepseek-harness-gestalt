...
export function assertReleasedPayloadSemantics(event: SessionFormatEvent, version: number): void {
  ...
  switch (event.type) {
    case 'browser/workspace':
      browserWorkspaceValue(data, label)
      return
    case 'member-question/asked':
      stringMembers(data, ['questionId', 'toProjectMember', 'projectId', 'originSessionId'], label)
      stringValue(data['background'], `${label} background`)
      memberQuestionsValue(data['questions'], label, false)
      return
    case 'member-question/outcome':
      nonEmptyString(data['questionId'], `${label} questionId`)
      literalValue(data['outcome'], ['answered', 'declined', 'expired', 'withdrawn', 'superseded', 'offline', 'revoked'], `${label} outcome`)
      if (data['answers'] !== undefined) {
        literalValue(data['outcome'], ['answered'], `${label} outcome with answers`)
        memberAnswersValue(data['answers'], label)
      }
      return
    case 'member-question/received': {
      stringMembers(data, ['questionId', 'projectId', 'originSessionId'], label)
      countValue(data['arrivedAt'], `${label} arrivedAt`)
      countValue(data['expiresAt'], `${label} expiresAt`)
      stringValue(data['background'], `${label} background`)
      const origin = exactRecord(data['origin'], `${label} origin`, ['projectName', 'originSessionTitle', 'askerAccountId', 'askerRole', 'askerDisplayName', 'askerAvatarUrl'])
      for (const key of ['projectName', 'originSessionTitle', 'askerAccountId', 'askerDisplayName', 'askerAvatarUrl']) stringValue(origin[key], `${label} origin ${key}`)
      literalValue(origin['askerRole'], ['owner', 'admin', 'member'], `${label} askerRole`)
      memberQuestionsValue(data['questions'], label, true)
      memberReferencesValue(data['references'], label, false)
      if (data['cachedReferences'] !== undefined) memberReferencesValue(data['cachedReferences'], label, true)
      return
    }
    case 'member-question/settled': {
      literalValue(data['type'], ['member-question-settled'], `${label} type`)
      stringMembers(data, ['operationId', 'questionId'], label)
      countValue(data['settledAt'], `${label} settledAt`)
      literalValue(data['outcome'], ['answered', 'declined', 'expired', 'withdrawn', 'superseded'], `${label} outcome`)
      const human = data['outcome'] === 'answered' || data['outcome'] === 'declined'
      const required = ['type', 'operationId', 'questionId', 'settledAt', 'outcome']
      if (human) required.push('settledByInstallationId', 'settledByDeviceName')
      if (data['outcome'] === 'answered') required.push('answers')
      assertReleasedV0Keys(data, required, [], label)
      if (human) {
        nonEmptyString(data['settledByInstallationId'], `${label} settledByInstallationId`)
        stringValue(data['settledByDeviceName'], `${label} settledByDeviceName`)
      }
      if (data['outcome'] === 'answered') memberAnswersValue(data['answers'], label)
      return
    }
    case 'session/attachment-admitted': {
      nonEmptyString(data['operationId'], `${label} operationId`)
      literalValue(data['source'], ['companion'], `${label} source`)
      const attachment = exactRecord(data['attachment'], `${label} attachment`, ['attachmentId', 'mediaType', 'bytes', 'sha256'], ['name'])
      stringMembers(attachment, ['attachmentId', 'mediaType', 'sha256'], label)
      countValue(attachment['bytes'], `${label} bytes`)
      if (typeof attachment['sha256'] !== 'string' || !/^[a-f0-9]{64}$/.test(attachment['sha256'])) throw new SessionFormatError(`${label} sha256 must be a lowercase SHA-256 digest`)
...
function stringMembers(data: JsonRecord, keys: readonly string[], label: string): void {
  for (const key of keys) nonEmptyString(data[key], `${label} ${key}`)
}

function memberQuestionsValue(value: SessionFormatJsonValue | undefined, label: string, rich: boolean): void {
  arrayValue(value, `${label} questions`, (value, itemLabel) => {
    const question = exactRecord(value, itemLabel, ['id', 'question'], rich ? ['header', 'options', 'multiSelect'] : [])
    nonEmptyString(question['id'], `${itemLabel} id`)
    stringValue(question['question'], `${itemLabel} question`)
    if (question['header'] !== undefined) stringValue(question['header'], `${itemLabel} header`)
    if (question['multiSelect'] !== undefined) booleanValue(question['multiSelect'], `${itemLabel} multiSelect`)
    if (question['options'] !== undefined) arrayValue(question['options'], `${itemLabel} options`, (value, optionLabel) => {
      const option = exactRecord(value, optionLabel, ['label'], ['description'])
      stringValue(option['label'], `${optionLabel} label`)
      if (option['description'] !== undefined) stringValue(option['description'], `${optionLabel} description`)
    })
  })
}

function memberAnswersValue(value: SessionFormatJsonValue | undefined, label: string): void {
  arrayValue(value, `${label} answers`, (value, itemLabel) => {
    const answer = exactRecord(value, itemLabel, ['id', 'selected'], ['custom'])
    nonEmptyString(answer['id'], `${itemLabel} id`)
    arrayValue(answer['selected'], `${itemLabel} selected`, stringValue)
    if (answer['custom'] !== undefined) stringValue(answer['custom'], `${itemLabel} custom`)
  })
}

function memberReferencesValue(value: SessionFormatJsonValue | undefined, label: string, cached: boolean): void {
  arrayValue(value, `${label} references`, (value, itemLabel) => {
    const reference = exactRecord(value, itemLabel, cached ? ['path', 'reason', 'cachedPath'] : ['path', 'reason'])
    stringMembers(reference, cached ? ['path', 'cachedPath'] : ['path'], itemLabel)
    stringValue(reference['reason'], `${itemLabel} reason`)
  })
}

function browserWorkspaceValue(data: JsonRecord, label: string): void {
  browserCollectionValue(data['workspaces'], data['activeWorkspaceId'], 'workspaceId', label, (workspace, workspaceLabel) => {
    assertReleasedV0Keys(workspace, ['workspaceId', 'profileId', 'browsers', 'activeBrowserId'], [], workspaceLabel)
    nonEmptyString(workspace['profileId'], `${workspaceLabel} profileId`)
    browserCollectionValue(workspace['browsers'], workspace['activeBrowserId'], 'browserId', workspaceLabel, (browser, browserLabel) => {
      assertReleasedV0Keys(browser, ['browserId', 'tabs', 'activeTabId'], [], browserLabel)
      browserCollectionValue(browser['tabs'], browser['activeTabId'], 'tabId', browserLabel, (tab, tabLabel) => {
        assertReleasedV0Keys(tab, ['tabId', 'revision'], ['url'], tabLabel)
        countValue(tab['revision'], `${tabLabel} revision`)
        if (tab['url'] !== undefined) nonEmptyString(tab['url'], `${tabLabel} url`)
      })
    })
  })
}

function browserCollectionValue(
  value: SessionFormatJsonValue | undefined,
  active: SessionFormatJsonValue | undefined,
  idKey: string,
  label: string,
  validate: (record: JsonRecord, label: string) => void,
): void {
  const ids = new Set<string>()
  arrayValue(value, `${label} ${idKey} collection`, (value, itemLabel) => {
    const record = releasedV0Record(value, itemLabel)
    const id = record[idKey]
    nonEmptyString(id, `${itemLabel} ${idKey}`)
    if (ids.has(id)) throw new SessionFormatError(`${itemLabel} has duplicate ${idKey}`)
    ids.add(id)
    validate(record, itemLabel)
  })
  if (active !== null) {
    nonEmptyString(active, `${label} active ${idKey}`)
    if (!ids.has(active)) throw new SessionFormatError(`${label} active ${idKey} must belong to its collection`)
  }
}
