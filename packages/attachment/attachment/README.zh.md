...
将图像附加到提示词和命令，并在同一会话的后续轮次中复用。该服务准入并归一化图像、保存不会进入模型历史的不透明 Companion 文件，并且只记录与提供方无关的引用。交付组合无需设置；图像可跨重启，草稿在提交前留在浏览器中，已存对象不会自动删除。
...
你可以把图片附加到提示词和命令中，harness 会持久保存提供方无关的规范化版本：每张源图都会在你的消息被处理前准入并规范化，重新出现在对话历史中，并在同一会话的后续轮次投影到所选模型路由。同一存储还可以把 Companion 的精确不透明文件持久化为内容寻址字节对象，且这些对象绝不会进入模型历史。随附的 `dsh` 组合无需任何配置即可支持这一点。已附加的图片在重启后依然存在，而浏览器路径、提供方 URL、本地存储路径与 base64 绝不会进入持久会话事件。提示词图片只接受光栅格式（PNG、JPEG、WebP、GIF），未发送的输入区草稿在提交前仍留在浏览器中。已存储对象永远不会被自动删除。音频和视频仍没有产品路径。
...
服务族运行同一条准入与存储流程：每个图片入口都强制执行源批次限制与规范 base64，在发布任何成员前准备提供方无关的规范化附件，再按输入顺序持久提交而不产生部分结果。`saveBytes` 与 `readBytes` 在同一套内容寻址对象上持久化并校验精确的不透明文件，既不会把它们当图片解码，也不会投影给模型。`readImageRequest` 派生确定性的路由尺寸变体，其身份包含附件 id、变换版本、像素与字节预算及编码参数。纯函数导出 `requestImageDimensions` 会按总像素预算计算每个投影保持宽高比的尺寸，使提供方与请求定价共享同一套几何计算。`imageHostPath` 只向需要执行世界映射的受信任同进程消费方暴露实现拥有的宿主位置。调用方组合有序批次，而实现拥有压缩并发、缓存与 singleflight。读取和投影保留调用方的取消语义。失败带有稳定且机器可读的错误码，运行时即可识别可由调用方修正的准入子集，让每个协议适配器映射自己的词汇；各操作的确切约定见 [`src/index.ts`](src/index.ts) 与 [`src/error.ts`](src/error.ts)。
...
- **提示词图片仍仅支持光栅**——PNG、JPEG、WebP 与 GIF 作为图片准入；不透明 Companion 文件走 `saveBytes`，绝不会变成 `ImageBlock`。音频和视频仍没有产品路径。
...
#### 未来：助手侧图片输出以及文件之外的媒体类型

不透明 Companion 文件已经通过 `saveBytes` 持久化。音频、视频与助手侧图片输出仍需要单独的生命周期与提供方契约——当前生产适配器声明只输出文本，因此只有用户内容携带图片。
