...
Store attachments locally under `DSH_HOME` with content-addressed deduplication and separate cached image request variants. The backend validates and normalizes prompt images while preserving opaque Companion files byte-for-byte. Objects survive restarts and admission-limit changes, remain local to one machine, and are never deleted automatically.
