...
/** One `llm-pi-ai` models[] entry written for Gestalt Account Pool. */
export interface AccountPoolPiAiModel {
  readonly id: string
  readonly name?: string
  readonly contextWindow?: number
  readonly maxTokens?: number
...
export function parseAccountPoolCatalog(value: unknown): readonly AccountPoolCatalogModel[] {
  ...
  for (const entry of rows) {
    ...
    const contextWindow = capacity(record.context_window, record.context_length, record.max_context_length)
    const maxTokens = capacity(record.max_output_tokens, record.max_tokens, record.max_completion_tokens)
    const input = modalities(record.input_modalities)
...
/**
 * Project catalog rows onto the `llm-pi-ai` models list.
 * @param models - Current authenticated core catalog.
 * @returns JSON-compatible model profiles for settings.
 */
export function toPiAiModels(models: readonly AccountPoolCatalogModel[]): AccountPoolPiAiModel[] {
...
function modalities(value: unknown): readonly ('text' | 'image')[] | undefined {
  ...
  for (const entry of value) {
    if (entry === 'text' || entry === 'image') input.add(entry)
  }
