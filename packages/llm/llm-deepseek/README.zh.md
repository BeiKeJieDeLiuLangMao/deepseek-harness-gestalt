...
当组合需要通过 harness LLM 服务流式调用 DeepSeek 模型时挂载本插件。它拥有唯一的 `deepseek-official` 路由，并按请求解析连接事实，因此组合条目加可选用户设置分节即可驱动整个适配器。没有 settings 的组合以及从未写入用户分节的首次运行会保留该路由。settings 已挂载时，非空用户分节或已配置凭据会占用该路由；显式空用户分节且没有凭据时，可配置提供方目录仍可用，但 Session 选择器不再获得该路由及其模型。
...
连接事实通过可选 settings 与凭据 seam 每次操作重新读取一次。用户设置文档中的 `llm-deepseek:` 分节无需重启即可覆盖任何字段；未通过超 schema 上限的快照会保留最后有效事实并记录失败。settings 文档与凭据引用提交也会重新核对路由占用。凭据描述是异步操作：较新的提交会取代较旧结果，描述失败会保留当前路由状态。API 密钥从提供端点、图片与 Files 策略及空闲预算的同一快照按流调用解析，因此被拒绝的设置代际不会贡献其中任何事实。图片请求在请求时解析附件服务，因此加载顺序不会冻结图片可用性。
...
插件建立在一个显式解析步骤与一个可替换注册之上。`resolveAdapterOptions()` 是从原始配置到已校验连接事实的唯一路径，适配器通过 thunk 每次操作重新读取这些事实——基址、目录、请求默认值、图片与 Files 策略及空闲预算都会作用于下一个请求，而进行中的流保持其启动时的事实。重试策略与路由占用由注册捕获；任一变化都会原子替换注册的路由集合，因此任何请求都观察不到释放再注册的空档。
