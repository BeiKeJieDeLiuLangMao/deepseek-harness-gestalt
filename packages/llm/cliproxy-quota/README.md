...
Every retained value is bounded: a probe response over 1 MiB UTF-8 bytes fails the observation (raw text is measured by encoded bytes, so multibyte bodies cannot slip under a character count, and an already-decoded body is measured after re-serialization), and a payload claiming more than 256 quota windows fails loud instead of emitting an unbounded array. The account reference is a branded `QuotaAccountRef`; an empty reference is rejected at the observe boundary.
...
| Codex | `GET chatgpt.com/backend-api/wham/usage`, `GET …/rate-limit-reset-credits` | windows classified by `limit_window_seconds` (5h / weekly / monthly keys); payloads without durations keep positional `primary`/`secondary` keys with `periodHours: null`; `plan_type`; read-only reset-credit counts |
