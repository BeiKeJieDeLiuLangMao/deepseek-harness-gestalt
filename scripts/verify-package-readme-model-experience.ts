...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/platform/remote-access-http': { kind: 'none', reason: 'The pairing HTTP Consumer handles state outside model requests.' },
  'packages/platform/remote-access-redis': { kind: 'none', reason: 'Redis Relay coordination carries transport metadata and ciphertext outside model requests.' },
  'packages/platform/noise-channel': { kind: 'none', reason: 'Handshake bytes and pairing keys never enter a model request.' },
  'packages/platform/remote-attachments': { kind: 'none', reason: 'Attachment ciphertext and capabilities never enter a model request.' },
  ...
  'packages/client/ui-conversation': { kind: 'none', reason: 'Browser conversation UI; compiled annotation prose reaches a model only through the ordinary user/message the Host admits.' },
  'packages/client/ui-workspace-reference': { kind: 'none', reason: 'Browser-side @ picker; the host workspace-reference plugin owns the model-visible marker.' },
